...
* `route_rules` - (Required) (Updatable) The collection of rules used for routing destination IPs to network devices.
	...
	* `destination` - (Optional) (Updatable) The destination service cidrBlock or destination IP address range in CIDR notation. Matching packets will be routed to the indicated network entity (the target).  Examples: `10.12.0.0/16`           `oci-phx-objectstorage` 
