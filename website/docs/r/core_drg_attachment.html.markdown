...
* `network_details` - (Optional) (Updatable) 
	...
	* `vcn_route_type` - (Optional) (Updatable) Indicates whether the VCN CIDR(s) or the individual Subnet CIDR(s) are imported from the attachment.  Routes from the VCN Ingress Route Table are always imported. 
