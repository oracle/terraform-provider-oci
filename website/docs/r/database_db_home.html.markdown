...
* `database` - (Required) (Updatable) 
	...
	* `db_backup_config` - (Applicable when source=NONE | VM_CLUSTER_NEW) (Updatable) 
		...
		* `backup_destination_details` - (Applicable when source=NONE | VM_CLUSTER_NEW) (Updatable) Backup destination details.
			...
			* `type` - (Required when source=NONE | VM_CLUSTER_NEW) Type of the database backup destination. Supported values: `NFS`.
