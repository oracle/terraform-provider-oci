...
layout: "oci"
page_title: "Oracle Cloud Infrastructure: oci_identity_domains_network_perimeters"
sidebar_current: "docs-oci-datasource-identity_domains-network_perimeters"
description: |-
  Provides the list of Network Perimeters in Oracle Cloud Infrastructure Identity Domains service
---

# Data Source: oci_identity_domains_network_perimeters
...
* `network_perimeter_count` - (Optional) OPTIONAL. An integer that indicates the desired maximum number of query results per page. 1000 is the largest value that you can use. See the Pagination section of the System for Cross-Domain Identity Management Protocol specification for more information. (Section 3.4.2.4).
* `network_perimeter_filter` - (Optional) OPTIONAL. The filter string that is used to request a subset of resources. The filter string MUST be a valid filter expression. See the Filtering section of the SCIM specification for more information (Section 3.4.2.2). The string should contain at least one condition that each item must match in order to be returned in the search results. Each condition specifies an attribute, an operator, and a value. Conditions within a filter can be connected by logical operators (such as AND and OR). Sets of conditions can be grouped together using parentheses.
...
* `network_perimeters` - The list of network_perimeters.

### NetworkPerimeter Reference
...
* `delete_in_progress` - A boolean flag indicating this resource in the process of being deleted. Usually set to true when synchronous deletion of the resource would take too long.
	...
	* type: boolean
	* uniqueness: none
* `description` - NetworkPerimeter Description
...
* `idcs_prevented_operations` - Each value of this attribute specifies an operation that only an internal client may perform on this particular resource.
	...
	* type: string
	* uniqueness: none
* `ip_addresses` - IPAddresses or Ranges assigned to the NetworkPerimeter
	...
	* type: complex
	* uniqueness: none
	* `type` - type of the ip address value
		...
		* type: string
		* uniqueness: none
	* `value` - Value of exact ipaddress or the range in CIDR or the range with start and end ip addresses
		...
		* type: string
		* uniqueness: none
	* `version` - Indicates the type of Ip Address example, IPV4 or IPV6
