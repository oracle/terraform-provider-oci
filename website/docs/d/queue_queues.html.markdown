...
* `capabilities` - The list of capabilities enabled on the queue
	* `is_primary_consumer_group_enabled` - (Applicable when type=CONSUMER_GROUPS) Specifies if the primary consumer group should be automatically enabled after adding the capability.
	...
	* `primary_consumer_group_display_name` - (Applicable when type=CONSUMER_GROUPS) Name of the primary consumer group. If omitted, it will be named "Primary Consumer Group".
	...
	* `type` - The type of the capability. Could be CONSUMER_GROUPS and/or LARGE_MESSAGES
