...
layout: "oci"
page_title: "Oracle Cloud Infrastructure: oci_identity_domains_approval_workflow_assignments"
sidebar_current: "docs-oci-datasource-identity_domains-approval_workflow_assignments"
description: |-
  Provides the list of Approval Workflow Assignments in Oracle Cloud Infrastructure Identity Domains service
---

# Data Source: oci_identity_domains_approval_workflow_assignments
...
* `approval_workflow_assignment_count` - (Optional) OPTIONAL. An integer that indicates the desired maximum number of query results per page. 1000 is the largest value that you can use. See the Pagination section of the System for Cross-Domain Identity Management Protocol specification for more information. (Section 3.4.2.4).
* `approval_workflow_assignment_filter` - (Optional) OPTIONAL. The filter string that is used to request a subset of resources. The filter string MUST be a valid filter expression. See the Filtering section of the SCIM specification for more information (Section 3.4.2.2). The string should contain at least one condition that each item must match in order to be returned in the search results. Each condition specifies an attribute, an operator, and a value. Conditions within a filter can be connected by logical operators (such as AND and OR). Sets of conditions can be grouped together using parentheses.
...
* `approval_workflow_assignments` - The list of approval_workflow_assignments.

### ApprovalWorkflowAssignment Reference

The following attributes are exported:

* `approval_workflow` - Details of the Approval Workflow
	...
	* type: complex
	* uniqueness: none
	* `ref` - URI of the approval workflow
		...
		* type: reference
		* uniqueness: none
	* `display` - Display name of the approval workflow
		...
		* type: string
		* uniqueness: none
	* `ocid` - Unique Oracle Cloud Infrastructure Identifier of the approval workflow
		...
		* type: string
		* uniqueness: none
	* `type` - Indicates type of the entity that is associated with this assignment (for ARM validation)
		...
		* type: string
		* uniqueness: none
	* `value` - Identifier of the approval workflow
		...
		* type: string
		* uniqueness: none
* `assigned_to` - Details of resource for which Approval Workflow is assigned
	...
	* `description` - Description
		...
		* type: string
		* uniqueness: none
	* `display` - Display name of the resource for which Approval Workflow is assigned
		...
		* type: string
		* uniqueness: none
	* `ocid` - Ocid of the resource for which Approval Workflow is assigned
		...
		* type: string
		* uniqueness: none
	* `type` - Type of the resource (stripe and non-stripe) for which Approval Workflow is assigned
		...
		* type: string
		* uniqueness: none
	* `value` - Identifier of the resource for which Approval Workflow is assigned
		...
		* type: string
		* uniqueness: none
* `assignment_type` - Type of approval workflow assignment to the resource. Resources like Group have only one workflow associated (default assignment type is membership), but resources like TemporalBusinessRole can have atleast 2 AWs attached, one for membership to TPR, another for activation of that TPR membership
