...
layout: "oci"
page_title: "Oracle Cloud Infrastructure: oci_identity_domains_policies"
sidebar_current: "docs-oci-datasource-identity_domains-policies"
description: |-
  Provides the list of Policies in Oracle Cloud Infrastructure Identity Domains service
---
...
* `policy_count` - (Optional) OPTIONAL. An integer that indicates the desired maximum number of query results per page. 1000 is the largest value that you can use. See the Pagination section of the System for Cross-Domain Identity Management Protocol specification for more information. (Section 3.4.2.4).
...
* `active` - If true, Policy is active.
...
* `ocid` - Unique Oracle Cloud Infrastructure identifier for the SCIM Resource.
	...
	* type: string
	* uniqueness: global
* `policy_groovy` - The Groovy script that is run instead of the policy, if the policy type allows the policy to be a Groovy script.
...
* `policy_type` - PolicyType on which the policy is based
	...
	* `value` - PolicyType identifier
		...
		* type: string
		* uniqueness: none
* `rules` - Rules assigned to this policy
	...
	* `name` - Rule name
		...
		* type: string
		* uniqueness: none
	* `sequence` - Position of the rule in evaluation order. No duplicates allowed.
