...
* `granular_maintenance_history` - The list of granular maintenance history details.
	...
	* `execution_window` - Details of an execution window. 
		...
		* `total_time_taken_in_mins` - The total time taken by corresponding resource activity in minutes.
		* `window_details` - A message that gives a description on how and why the window was created.
...
* `maintenance_run_details` - Details of a maintenance run. 
	...
	* `window_type_descriptions` - A list of key-value pairs where the key will contain the window type and value contains all the windowDetails of that window type.
		...
		* `window_type` - The execution window type. Either "PLANNED" or "UNPLANNED".
