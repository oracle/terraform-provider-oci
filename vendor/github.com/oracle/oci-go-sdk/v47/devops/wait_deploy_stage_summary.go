...
func (m WaitDeployStageSummary) MarshalJSON() (buff []byte, e error) {
	type MarshalTypeWaitDeployStageSummary WaitDeployStageSummary
	s := struct {
		DiscriminatorParam string `json:"deployStageType"`
		MarshalTypeWaitDeployStageSummary
	}{
		"WAIT",
		(MarshalTypeWaitDeployStageSummary)(m),
	}
...
func (m *WaitDeployStageSummary) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		WaitCriteria                     waitcriteriasummary               `json:"waitCriteria"`
	}{}
	...
	if nn != nil {
		m.WaitCriteria = nn.(WaitCriteriaSummary)
	} else {
		m.WaitCriteria = nil
	}
