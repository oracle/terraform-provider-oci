...
func (m ReplicationTarget) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingReplicationTargetLifecycleStateEnum[string(m.LifecycleState)]; !ok && m.LifecycleState != "" {
	...
	if _, ok := mappingReplicationTargetDeltaStatusEnum[string(m.DeltaStatus)]; !ok && m.DeltaStatus != "" {
