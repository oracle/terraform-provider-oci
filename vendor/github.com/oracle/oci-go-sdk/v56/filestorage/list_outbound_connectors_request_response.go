...
func (request ListOutboundConnectorsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListOutboundConnectorsLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListOutboundConnectorsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListOutboundConnectorsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
