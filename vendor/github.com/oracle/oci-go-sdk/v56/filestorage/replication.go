...
func (m Replication) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingReplicationLifecycleStateEnum[string(m.LifecycleState)]; !ok && m.LifecycleState != "" {
	...
	if _, ok := mappingReplicationDeltaStatusEnum[string(m.DeltaStatus)]; !ok && m.DeltaStatus != "" {
