...
func (request ListOperationsInsightsPrivateEndpointsRequest) ValidateEnumValue() (bool, error) {
	...
	for _, val := range request.LifecycleState {
		if _, ok := mappingOperationsInsightsPrivateEndpointLifecycleStateEnum[string(val)]; !ok && val != "" {
	...
	if _, ok := mappingListOperationsInsightsPrivateEndpointsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListOperationsInsightsPrivateEndpointsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
