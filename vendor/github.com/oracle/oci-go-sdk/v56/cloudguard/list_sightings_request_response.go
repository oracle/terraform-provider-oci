...
func (request ListSightingsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListSightingsAccessLevelEnum[string(request.AccessLevel)]; !ok && request.AccessLevel != "" {
	...
	if _, ok := mappingListSightingsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListSightingsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
