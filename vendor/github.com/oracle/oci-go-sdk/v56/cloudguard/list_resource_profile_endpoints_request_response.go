...
func (request ListResourceProfileEndpointsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListResourceProfileEndpointsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListResourceProfileEndpointsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
