...
func (request ListResourceProfilesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListResourceProfilesAccessLevelEnum[string(request.AccessLevel)]; !ok && request.AccessLevel != "" {
	...
	if _, ok := mappingListResourceProfilesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListResourceProfilesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
