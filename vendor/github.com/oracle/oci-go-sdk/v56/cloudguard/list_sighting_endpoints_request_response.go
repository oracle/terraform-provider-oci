...
func (request ListSightingEndpointsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListSightingEndpointsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListSightingEndpointsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
