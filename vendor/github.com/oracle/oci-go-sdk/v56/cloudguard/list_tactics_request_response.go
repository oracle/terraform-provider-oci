...
func (request ListTacticsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListTacticsLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListTacticsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListTacticsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
