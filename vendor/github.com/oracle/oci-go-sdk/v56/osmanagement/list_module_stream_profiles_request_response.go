...
func (request ListModuleStreamProfilesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListModuleStreamProfilesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListModuleStreamProfilesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
