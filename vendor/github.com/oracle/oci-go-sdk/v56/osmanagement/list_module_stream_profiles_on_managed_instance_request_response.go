...
func (request ListModuleStreamProfilesOnManagedInstanceRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListModuleStreamProfilesOnManagedInstanceProfileStatusEnum[string(request.ProfileStatus)]; !ok && request.ProfileStatus != "" {
	...
	if _, ok := mappingListModuleStreamProfilesOnManagedInstanceSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListModuleStreamProfilesOnManagedInstanceSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
