...
func (request ListFoldersRequest) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingListFoldersFavoritesQueryParamEnum(string(request.FavoritesQueryParam)); !ok && request.FavoritesQueryParam != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for FavoritesQueryParam: %s. Supported values are: %s.", request.FavoritesQueryParam, strings.Join(GetListFoldersFavoritesQueryParamEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingListFoldersFavoritesQueryParamEnum = map[string]ListFoldersFavoritesQueryParamEnum{
	"FAVORITES_ONLY":     ListFoldersFavoritesQueryParamFavoritesOnly,
	"NON_FAVORITES_ONLY": ListFoldersFavoritesQueryParamNonFavoritesOnly,
	"ALL":                ListFoldersFavoritesQueryParamAll,
}

var mappingListFoldersFavoritesQueryParamEnumLowerCase = map[string]ListFoldersFavoritesQueryParamEnum{
	"favorites_only":     ListFoldersFavoritesQueryParamFavoritesOnly,
	"non_favorites_only": ListFoldersFavoritesQueryParamNonFavoritesOnly,
	"all":                ListFoldersFavoritesQueryParamAll,
}
...
func GetListFoldersFavoritesQueryParamEnumValues() []ListFoldersFavoritesQueryParamEnum {
	values := make([]ListFoldersFavoritesQueryParamEnum, 0)
	for _, v := range mappingListFoldersFavoritesQueryParamEnum {
		values = append(values, v)
	}
	return values
}
...
func GetMappingListFoldersFavoritesQueryParamEnum(val string) (ListFoldersFavoritesQueryParamEnum, bool) {
	enum, ok := mappingListFoldersFavoritesQueryParamEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
