...
func (m DrgUpgradeState) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingDrgUpgradeStateStateEnum(string(m.State)); !ok && m.State != "" {
...
// GetMappingDrgUpgradeStateStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingDrgUpgradeStateStateEnum(val string) (DrgUpgradeStateStateEnum, bool) {
	enum, ok := mappingDrgUpgradeStateStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
