...
func (request ListModuleStreamsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListModuleStreamsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListModuleStreamsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
