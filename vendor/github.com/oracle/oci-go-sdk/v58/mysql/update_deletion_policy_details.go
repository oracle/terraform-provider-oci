...
func (m UpdateDeletionPolicyDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingUpdateDeletionPolicyDetailsAutomaticBackupRetentionEnum[string(m.AutomaticBackupRetention)]; !ok && m.AutomaticBackupRetention != "" {
	...
	if _, ok := mappingUpdateDeletionPolicyDetailsFinalBackupEnum[string(m.FinalBackup)]; !ok && m.FinalBackup != "" {
