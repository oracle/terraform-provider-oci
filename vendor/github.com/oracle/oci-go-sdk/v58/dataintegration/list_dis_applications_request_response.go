...
func (request ListDisApplicationsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListDisApplicationsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListDisApplicationsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
