...
func (request ListExcludedObjectsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListExcludedObjectsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListExcludedObjectsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListExcludedObjectsReasonCategoryEnum[string(request.ReasonCategory)]; !ok && request.ReasonCategory != "" {
