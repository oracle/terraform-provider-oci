...
func (request GetNetworkConnectivityStatusCollectionRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingGetNetworkConnectivityStatusCollectionSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingGetNetworkConnectivityStatusCollectionSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
