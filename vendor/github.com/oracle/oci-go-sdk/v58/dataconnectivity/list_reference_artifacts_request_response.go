...
func (request ListReferenceArtifactsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListReferenceArtifactsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListReferenceArtifactsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListReferenceArtifactsFavoritesQueryParamEnum[string(request.FavoritesQueryParam)]; !ok && request.FavoritesQueryParam != "" {
