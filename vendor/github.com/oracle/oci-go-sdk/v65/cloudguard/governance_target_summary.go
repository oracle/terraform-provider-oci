...
// GovernanceTargetSummary Summary information for a governance target.
type GovernanceTargetSummary struct {
	...
	// Type of scope for governance that allows specifying all subject tenancies or a custom list.
	GovernanceScopeType ScopeTypeEnum `mandatory:"true" json:"governanceScopeType"`
...
func (m GovernanceTargetSummary) String() string {
...
func (m GovernanceTargetSummary) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingScopeTypeEnum(string(m.GovernanceScopeType)); !ok && m.GovernanceScopeType != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for GovernanceScopeType: %s. Supported values are: %s.", m.GovernanceScopeType, strings.Join(GetScopeTypeEnumStringValues(), ",")))
	}
