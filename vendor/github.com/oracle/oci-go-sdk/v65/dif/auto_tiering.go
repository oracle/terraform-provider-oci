...
var mappingAutoTieringEnum = map[string]AutoTieringEnum{
	"DISABLED":         AutoTieringDisabled,
	"INFREQUENTACCESS": AutoTieringInfrequentaccess,
}

var mappingAutoTieringEnumLowerCase = map[string]AutoTieringEnum{
	"disabled":         AutoTieringDisabled,
	"infrequentaccess": AutoTieringInfrequentaccess,
}
...
func GetAutoTieringEnumValues() []AutoTieringEnum {
	values := make([]AutoTieringEnum, 0)
	for _, v := range mappingAutoTieringEnum {
...
func GetMappingAutoTieringEnum(val string) (AutoTieringEnum, bool) {
	enum, ok := mappingAutoTieringEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
