...
// BatchJobPool A batch job pool represents an abstract group for jobs.
...
func (m BatchJobPool) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingBatchJobPoolLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for LifecycleState: %s. Supported values are: %s.", m.LifecycleState, strings.Join(GetBatchJobPoolLifecycleStateEnumStringValues(), ",")))
	}
...
var mappingBatchJobPoolLifecycleStateEnum = map[string]BatchJobPoolLifecycleStateEnum{
	"ACTIVE":          BatchJobPoolLifecycleStateActive,
	"INACTIVE":        BatchJobPoolLifecycleStateInactive,
	"UPDATING":        BatchJobPoolLifecycleStateUpdating,
	"NEEDS_ATTENTION": BatchJobPoolLifecycleStateNeedsAttention,
	"DELETED":         BatchJobPoolLifecycleStateDeleted,
}

var mappingBatchJobPoolLifecycleStateEnumLowerCase = map[string]BatchJobPoolLifecycleStateEnum{
	"active":          BatchJobPoolLifecycleStateActive,
	"inactive":        BatchJobPoolLifecycleStateInactive,
	"updating":        BatchJobPoolLifecycleStateUpdating,
	"needs_attention": BatchJobPoolLifecycleStateNeedsAttention,
...
func GetBatchJobPoolLifecycleStateEnumValues() []BatchJobPoolLifecycleStateEnum {
	values := make([]BatchJobPoolLifecycleStateEnum, 0)
	for _, v := range mappingBatchJobPoolLifecycleStateEnum {
...
func GetMappingBatchJobPoolLifecycleStateEnum(val string) (BatchJobPoolLifecycleStateEnum, bool) {
	enum, ok := mappingBatchJobPoolLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
