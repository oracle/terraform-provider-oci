...
func (m SmtpCredential) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingSmtpCredentialStatusEnum(string(m.Status)); !ok && m.Status != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Status: %s. Supported values are: %s.", m.Status, strings.Join(GetSmtpCredentialStatusEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
var mappingSmtpCredentialStatusEnum = map[string]SmtpCredentialStatusEnum{
	"ACTIVE":   SmtpCredentialStatusActive,
	"INACTIVE": SmtpCredentialStatusInactive,
}

var mappingSmtpCredentialStatusEnumLowerCase = map[string]SmtpCredentialStatusEnum{
	"active":   SmtpCredentialStatusActive,
	"inactive": SmtpCredentialStatusInactive,
}
...
func GetSmtpCredentialStatusEnumValues() []SmtpCredentialStatusEnum {
	values := make([]SmtpCredentialStatusEnum, 0)
	for _, v := range mappingSmtpCredentialStatusEnum {
...
func GetMappingSmtpCredentialStatusEnum(val string) (SmtpCredentialStatusEnum, bool) {
	enum, ok := mappingSmtpCredentialStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
