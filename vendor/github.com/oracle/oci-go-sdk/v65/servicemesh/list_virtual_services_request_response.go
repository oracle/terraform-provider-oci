...
func (request ListVirtualServicesRequest) RetryPolicy() common.OCIRetry {
...
func (request ListVirtualServicesRequest) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingVirtualServiceLifecycleStateEnum(string(request.LifecycleState)); !ok && request.LifecycleState != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for LifecycleState: %s. Supported values are: %s.", request.LifecycleState, strings.Join(GetVirtualServiceLifecycleStateEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
