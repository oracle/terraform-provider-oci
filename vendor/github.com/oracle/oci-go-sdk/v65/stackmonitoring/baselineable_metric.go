...
func (m BaselineableMetric) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingBaselineableMetricLifeCycleStatesEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for LifecycleState: %s. Supported values are: %s.", m.LifecycleState, strings.Join(GetBaselineableMetricLifeCycleStatesEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
