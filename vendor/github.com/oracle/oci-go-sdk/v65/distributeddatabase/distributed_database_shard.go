...
func (m *distributeddatabaseshard) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {
	...
	case "EXADB_XS":
		...
		return mm, err
	case "BASE_DB":
		mm := DistributedDatabaseShardWithBaseDb{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	default:
...
const (
	...
	DistributedDatabaseShardSourceBaseDb             DistributedDatabaseShardSourceEnum = "BASE_DB"
)
...
var mappingDistributedDatabaseShardSourceEnum = map[string]DistributedDatabaseShardSourceEnum{
	...
	"BASE_DB":               DistributedDatabaseShardSourceBaseDb,
}
