...
// ValidateOpensearchClusterPipeline Validate Pipeline configuration provided by the customer.
func (client OpensearchClusterPipelineClient) ValidateOpensearchClusterPipeline(ctx context.Context, request ValidateOpensearchClusterPipelineRequest) (response ValidateOpensearchClusterPipelineResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.NoRetryPolicy()
	if client.RetryPolicy() != nil {
		policy = *client.RetryPolicy()
	...
	ociResponse, err = common.Retry(ctx, request, client.validateOpensearchClusterPipeline, policy)
	if err != nil {
		if ociResponse != nil {
			if httpResponse := ociResponse.HTTPResponse(); httpResponse != nil {
				opcRequestId := httpResponse.Header.Get("opc-request-id")
				response = ValidateOpensearchClusterPipelineResponse{RawResponse: httpResponse, OpcRequestId: &opcRequestId}
			} else {
				response = ValidateOpensearchClusterPipelineResponse{}
			}
		}
		return
	}
	if convertedResponse, ok := ociResponse.(ValidateOpensearchClusterPipelineResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into ValidateOpensearchClusterPipelineResponse")
	}
	return
}

// validateOpensearchClusterPipeline implements the OCIOperation interface (enables retrying operations)
func (client OpensearchClusterPipelineClient) validateOpensearchClusterPipeline(ctx context.Context, request common.OCIRequest, binaryReqBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (common.OCIResponse, error) {

	httpRequest, err := request.HTTPRequest(http.MethodPost, "/opensearchClusterPipelines/actions/validatePipeline", binaryReqBody, extraHeaders)
	if err != nil {
		return nil, err
	}

	var response ValidateOpensearchClusterPipelineResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
	if err != nil {
		apiReferenceLink := "https://docs.oracle.com/iaas/api/#/en/opensearch/20180828/OpensearchPipelineValidationResponse/ValidateOpensearchClusterPipeline"
		err = common.PostProcessServiceError(err, "OpensearchClusterPipeline", "ValidateOpensearchClusterPipeline", apiReferenceLink)
		return response, err
	}
