...
func (m InboundAuthConfig) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingInboundAuthConfigInboundAuthConfigTypeEnum(string(m.InboundAuthConfigType)); !ok && m.InboundAuthConfigType != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for InboundAuthConfigType: %s. Supported values are: %s.", m.InboundAuthConfigType, strings.Join(GetInboundAuthConfigInboundAuthConfigTypeEnumStringValues(), ",")))
	}
...
func GetInboundAuthConfigInboundAuthConfigTypeEnumValues() []InboundAuthConfigInboundAuthConfigTypeEnum {
	values := make([]InboundAuthConfigInboundAuthConfigTypeEnum, 0)
	for _, v := range mappingInboundAuthConfigInboundAuthConfigTypeEnum {
...
func GetMappingInboundAuthConfigInboundAuthConfigTypeEnum(val string) (InboundAuthConfigInboundAuthConfigTypeEnum, bool) {
	enum, ok := mappingInboundAuthConfigInboundAuthConfigTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
