...
func (request DeleteIdpGroupMappingRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["mappingId"] != nil {
		templateParam := mandatoryParamMap["mappingId"]
		for _, template := range templateParam {
			replacementParam := *request.MappingId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
