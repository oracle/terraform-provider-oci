...
func (request DeleteCustomerSecretKeyRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["customerSecretKeyId"] != nil {
		templateParam := mandatoryParamMap["customerSecretKeyId"]
		for _, template := range templateParam {
			replacementParam := *request.CustomerSecretKeyId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
