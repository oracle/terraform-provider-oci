...
func (request DeleteApiKeyRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["fingerprint"] != nil {
		templateParam := mandatoryParamMap["fingerprint"]
		for _, template := range templateParam {
			replacementParam := *request.Fingerprint
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
