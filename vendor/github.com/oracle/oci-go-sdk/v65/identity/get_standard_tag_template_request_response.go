...
func (request GetStandardTagTemplateRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["standardTagNamespaceName"] != nil {
		templateParam := mandatoryParamMap["standardTagNamespaceName"]
		for _, template := range templateParam {
			replacementParam := *request.StandardTagNamespaceName
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
