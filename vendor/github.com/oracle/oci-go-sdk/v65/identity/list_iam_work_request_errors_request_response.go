...
func (request ListIamWorkRequestErrorsRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	if mandatoryParamMap["iamWorkRequestId"] != nil {
		templateParam := mandatoryParamMap["iamWorkRequestId"]
		for _, template := range templateParam {
			replacementParam := *request.IamWorkRequestId
