...
func (request GetAccountByEntitlementIdRequest) HTTPRequest(method, path string, binaryRequestBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (http.Request, error) {
...
func (request GetAccountByEntitlementIdRequest) BinaryRequestBody() (*common.OCIReadSeekCloser, bool) {
...
func (request GetAccountByEntitlementIdRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["entitlementId"] != nil {
		templateParam := mandatoryParamMap["entitlementId"]
		for _, template := range templateParam {
			replacementParam := *request.EntitlementId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
...
func (request GetAccountByEntitlementIdRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if len(errMessage) > 0 {
