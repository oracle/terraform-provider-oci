...
func (request DeleteAuthTokenRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["authTokenId"] != nil {
		templateParam := mandatoryParamMap["authTokenId"]
		for _, template := range templateParam {
			replacementParam := *request.AuthTokenId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
