...
func (request GetVirtualNodeRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["virtualNodeId"] != nil {
		templateParam := mandatoryParamMap["virtualNodeId"]
		for _, template := range templateParam {
			replacementParam := *request.VirtualNodeId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
