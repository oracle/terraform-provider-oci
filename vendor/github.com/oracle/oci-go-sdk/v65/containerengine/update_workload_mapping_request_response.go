...
func (request UpdateWorkloadMappingRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["workloadMappingId"] != nil {
		templateParam := mandatoryParamMap["workloadMappingId"]
		for _, template := range templateParam {
			replacementParam := *request.WorkloadMappingId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
