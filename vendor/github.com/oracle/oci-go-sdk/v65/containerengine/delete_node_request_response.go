...
func (request DeleteNodeRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	if mandatoryParamMap["nodePoolId"] != nil {
		templateParam := mandatoryParamMap["nodePoolId"]
		for _, template := range templateParam {
			replacementParam := *request.NodePoolId
	...
	if mandatoryParamMap["nodeId"] != nil {
		templateParam := mandatoryParamMap["nodeId"]
		for _, template := range templateParam {
			replacementParam := *request.NodeId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
