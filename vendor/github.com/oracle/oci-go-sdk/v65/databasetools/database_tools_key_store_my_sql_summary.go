...
func (m DatabaseToolsKeyStoreMySqlSummary) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingKeyStoreTypeMySqlEnum(string(m.KeyStoreType)); !ok && m.KeyStoreType != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for KeyStoreType: %s. Supported values are: %s.", m.KeyStoreType, strings.Join(GetKeyStoreTypeMySqlEnumStringValues(), ",")))
	}
	if len(errMessage) > 0 {
...
func (m *DatabaseToolsKeyStoreMySqlSummary) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		KeyStoreType     KeyStoreTypeMySqlEnum                     `json:"keyStoreType"`
		KeyStoreContent  databasetoolskeystorecontentmysqlsummary  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordmysqlsummary `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentMySqlSummary)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordMySqlSummary)
	} else {
