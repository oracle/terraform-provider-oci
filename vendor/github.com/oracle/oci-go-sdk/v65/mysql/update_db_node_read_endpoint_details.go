...
// UpdateDbNodeReadEndpointDetails Details used to update the read endpoint for a DB node.
// Each DB node has its own read endpoint in addition to the shared-storage DB cluster-level read endpoint.
type UpdateDbNodeReadEndpointDetails struct {
...
func (m UpdateDbNodeReadEndpointDetails) String() string {
...
func (m UpdateDbNodeReadEndpointDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
