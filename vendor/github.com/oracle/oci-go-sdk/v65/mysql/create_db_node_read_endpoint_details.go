...
// CreateDbNodeReadEndpointDetails Details used to configure the read endpoint for a DB node.
// Each DB node has its own read endpoint in addition to the shared-storage DB cluster-level read endpoint.
type CreateDbNodeReadEndpointDetails struct {
...
func (m CreateDbNodeReadEndpointDetails) String() string {
...
func (m CreateDbNodeReadEndpointDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
