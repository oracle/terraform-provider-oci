...
func (request GetBackendOperationalStatusRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["backendName"] != nil {
		templateParam := mandatoryParamMap["backendName"]
		for _, template := range templateParam {
			replacementParam := *request.BackendName
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
