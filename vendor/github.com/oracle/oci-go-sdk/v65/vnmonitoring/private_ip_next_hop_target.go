...
type PrivateIpNextHopTarget struct {
	...
	// The OCID (https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the nextHop target entity.
...
func (m PrivateIpNextHopTarget) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingPrivateIpNextHopTargetTargetTypeEnum(string(m.TargetType)); !ok && m.TargetType != "" {
...
// GetMappingPrivateIpNextHopTargetTargetTypeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingPrivateIpNextHopTargetTargetTypeEnum(val string) (PrivateIpNextHopTargetTargetTypeEnum, bool) {
	enum, ok := mappingPrivateIpNextHopTargetTargetTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
