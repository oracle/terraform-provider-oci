...
func (m ClientVpnUser) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingClientVpnUserLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
...
var mappingClientVpnUserLifecycleStateEnumLowerCase = map[string]ClientVpnUserLifecycleStateEnum{
	"creating": ClientVpnUserLifecycleStateCreating,
	"active":   ClientVpnUserLifecycleStateActive,
	"inactive": ClientVpnUserLifecycleStateInactive,
	"failed":   ClientVpnUserLifecycleStateFailed,
...
// GetMappingClientVpnUserLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingClientVpnUserLifecycleStateEnum(val string) (ClientVpnUserLifecycleStateEnum, bool) {
	enum, ok := mappingClientVpnUserLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
