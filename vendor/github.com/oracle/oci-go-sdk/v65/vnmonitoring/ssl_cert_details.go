...
func (m *sslcertdetails) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {
	...
	case "BASE64ENCODED":
		...
		return mm, err
	default:
		common.Logf("Received unsupported enum value for SslCertDetails: %s.", m.ContentType)
		return *m, nil
	}
}
...
var mappingSslCertDetailsContentTypeEnumLowerCase = map[string]SslCertDetailsContentTypeEnum{
	"base64encoded": SslCertDetailsContentTypeBase64encoded,
}
...
// GetMappingSslCertDetailsContentTypeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingSslCertDetailsContentTypeEnum(val string) (SslCertDetailsContentTypeEnum, bool) {
	enum, ok := mappingSslCertDetailsContentTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
