...
func (request UpdateQuotaRuleRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["quotaRuleId"] != nil {
		templateParam := mandatoryParamMap["quotaRuleId"]
		for _, template := range templateParam {
			replacementParam := *request.QuotaRuleId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
