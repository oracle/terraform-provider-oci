...
// MatchingRules An object that defines the set of rules that identifies the target instances in a dynamic set.
type MatchingRules struct {

	// Include either any or all attributes.
	Include IncludeTypeEnum `mandatory:"true" json:"include"`

	// The list of compartment details.
	Compartments []CompartmentDetails `mandatory:"true" json:"compartments"`
	...
	OsNames []string `mandatory:"false" json:"osNames"`

	// The list of managed instance OS version.
	OsVersions []string `mandatory:"false" json:"osVersions"`
...
func (m MatchingRules) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingIncludeTypeEnum(string(m.Include)); !ok && m.Include != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Include: %s. Supported values are: %s.", m.Include, strings.Join(GetIncludeTypeEnumStringValues(), ",")))
	}
...
func (m *MatchingRules) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		Tags                    []tag                         `json:"tags"`
		...
		Include                 IncludeTypeEnum               `json:"include"`
		Compartments            []CompartmentDetails          `json:"compartments"`
	}{}
	...
	m.OsNames = make([]string, len(model.OsNames))
	copy(m.OsNames, model.OsNames)
	m.OsVersions = make([]string, len(model.OsVersions))
	copy(m.OsVersions, model.OsVersions)
	...
	m.Compartments = make([]CompartmentDetails, len(model.Compartments))
	copy(m.Compartments, model.Compartments)
	return
}
