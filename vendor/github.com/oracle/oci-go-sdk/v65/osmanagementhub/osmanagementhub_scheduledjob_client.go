...
// PauseScheduledJob Triggers an already created recurring scheduled job to be paused immediately or within the future.
// This operation only applies to recurring jobs, not one-time scheduled jobs.
// A default retry strategy applies to this operation PauseScheduledJob()
func (client ScheduledJobClient) PauseScheduledJob(ctx context.Context, request PauseScheduledJobRequest) (response PauseScheduledJobResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.DefaultRetryPolicy()
	if client.RetryPolicy() != nil {
		policy = *client.RetryPolicy()
	...
	ociResponse, err = common.Retry(ctx, request, client.pauseScheduledJob, policy)
	if err != nil {
		if ociResponse != nil {
			if httpResponse := ociResponse.HTTPResponse(); httpResponse != nil {
				opcRequestId := httpResponse.Header.Get("opc-request-id")
				response = PauseScheduledJobResponse{RawResponse: httpResponse, OpcRequestId: &opcRequestId}
			} else {
				response = PauseScheduledJobResponse{}
			}
		}
		return
	}
	if convertedResponse, ok := ociResponse.(PauseScheduledJobResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into PauseScheduledJobResponse")
	}
	return
}

// pauseScheduledJob implements the OCIOperation interface (enables retrying operations)
func (client ScheduledJobClient) pauseScheduledJob(ctx context.Context, request common.OCIRequest, binaryReqBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (common.OCIResponse, error) {

	httpRequest, err := request.HTTPRequest(http.MethodPost, "/scheduledJobs/{scheduledJobId}/actions/pause", binaryReqBody, extraHeaders)
	if err != nil {
		return nil, err
	}

	var response PauseScheduledJobResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
	if err != nil {
		apiReferenceLink := "https://docs.oracle.com/iaas/api/#/en/osmh/20220901/ScheduledJob/PauseScheduledJob"
		err = common.PostProcessServiceError(err, "ScheduledJob", "PauseScheduledJob", apiReferenceLink)
		return response, err
	}
...
// ResumeScheduledJob Triggers an already created recurring scheduled job to be resumed immediately or within the future.
// This operation only applies to recurring jobs, not one-time scheduled jobs.
// A default retry strategy applies to this operation ResumeScheduledJob()
func (client ScheduledJobClient) ResumeScheduledJob(ctx context.Context, request ResumeScheduledJobRequest) (response ResumeScheduledJobResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.DefaultRetryPolicy()
	if client.RetryPolicy() != nil {
		policy = *client.RetryPolicy()
	...
	ociResponse, err = common.Retry(ctx, request, client.resumeScheduledJob, policy)
	if err != nil {
		if ociResponse != nil {
			if httpResponse := ociResponse.HTTPResponse(); httpResponse != nil {
				opcRequestId := httpResponse.Header.Get("opc-request-id")
				response = ResumeScheduledJobResponse{RawResponse: httpResponse, OpcRequestId: &opcRequestId}
			} else {
				response = ResumeScheduledJobResponse{}
			}
		}
		return
	}
	if convertedResponse, ok := ociResponse.(ResumeScheduledJobResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into ResumeScheduledJobResponse")
	}
	return
}

// resumeScheduledJob implements the OCIOperation interface (enables retrying operations)
func (client ScheduledJobClient) resumeScheduledJob(ctx context.Context, request common.OCIRequest, binaryReqBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (common.OCIResponse, error) {

	httpRequest, err := request.HTTPRequest(http.MethodPost, "/scheduledJobs/{scheduledJobId}/actions/resume", binaryReqBody, extraHeaders)
	if err != nil {
		return nil, err
	}

	var response ResumeScheduledJobResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
	if err != nil {
		apiReferenceLink := "https://docs.oracle.com/iaas/api/#/en/osmh/20220901/ScheduledJob/ResumeScheduledJob"
		err = common.PostProcessServiceError(err, "ScheduledJob", "ResumeScheduledJob", apiReferenceLink)
		return response, err
	}
