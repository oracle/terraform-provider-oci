...
func (m *hostdumptransferdetails) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {
	...
	case "CURL":
		...
		return mm, err
	default:
		common.Logf("Recieved unsupported enum value for HostDumpTransferDetails: %s.", m.Kind)
		return *m, nil
	}
}
...
var mappingHostDumpTransferDetailsKindEnumLowerCase = map[string]HostDumpTransferDetailsKindEnum{
	"curl":    HostDumpTransferDetailsKindCurl,
	"oci_cli": HostDumpTransferDetailsKindOciCli,
}
...
func GetMappingHostDumpTransferDetailsKindEnum(val string) (HostDumpTransferDetailsKindEnum, bool) {
	enum, ok := mappingHostDumpTransferDetailsKindEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
