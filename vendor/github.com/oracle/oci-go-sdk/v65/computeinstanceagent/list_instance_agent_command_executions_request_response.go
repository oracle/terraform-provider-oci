...
func (request ListInstanceAgentCommandExecutionsRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["instanceId"] != nil {
		templateParam := mandatoryParamMap["instanceId"]
		for _, template := range templateParam {
			replacementParam := *request.InstanceId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
