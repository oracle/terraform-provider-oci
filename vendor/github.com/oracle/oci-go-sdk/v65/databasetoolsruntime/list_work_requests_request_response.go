...
func (request ListWorkRequestsRequest) ValidateEnumValue() (bool, error) {
	...
	for _, val := range request.OperationType {
		if _, ok := GetMappingOperationTypeEnum(string(val)); !ok && val != "" {
			errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for OperationType: %s. Supported values are: %s.", val, strings.Join(GetOperationTypeEnumStringValues(), ",")))
		}
	}
