...
func (m Translator) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingTranslationServiceEnum(string(m.Type)); !ok && m.Type != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for Type: %s. Supported values are: %s.", m.Type, strings.Join(GetTranslationServiceEnumStringValues(), ",")))
	}
