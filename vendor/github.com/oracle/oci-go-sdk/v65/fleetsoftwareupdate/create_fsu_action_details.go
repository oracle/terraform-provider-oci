...
func (m *createfsuactiondetails) UnmarshalPolymorphicJSON(data []byte) (interface{}, error) {
	...
	case "STAGE":
		...
		err = json.Unmarshal(data, &mm)
		return mm, err
	case "SINGLE_TARGET_PRECHECK":
		mm := CreateSingleTargetPrecheckDetails{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	...
	case "CLEANUP":
		...
		err = json.Unmarshal(data, &mm)
		return mm, err
	case "SINGLE_TARGET_APPLY":
		mm := CreateSingleTargetApplyDetails{}
		err = json.Unmarshal(data, &mm)
		return mm, err
	case "PRECHECK":
		...
		return mm, err
	default:
		common.Logf("Received unsupported enum value for CreateFsuActionDetails: %s.", m.Type)
		return *m, nil
	}
}
