...
func (request ListIPSecConnectionTunnelSecurityAssociationsRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["tunnelId"] != nil {
		templateParam := mandatoryParamMap["tunnelId"]
		for _, template := range templateParam {
			replacementParam := *request.TunnelId
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
