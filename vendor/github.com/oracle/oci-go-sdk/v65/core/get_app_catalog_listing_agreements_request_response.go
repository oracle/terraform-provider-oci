...
func (request GetAppCatalogListingAgreementsRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	if mandatoryParamMap["listingId"] != nil {
		templateParam := mandatoryParamMap["listingId"]
		for _, template := range templateParam {
			replacementParam := *request.ListingId
	...
	if mandatoryParamMap["resourceVersion"] != nil {
		templateParam := mandatoryParamMap["resourceVersion"]
		for _, template := range templateParam {
			replacementParam := *request.ResourceVersion
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
