...
type LocalPeeringConnection struct {
	...
	// The OCID (https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the compartment containing the local peering connection.
	...
	// The local peering connection's Oracle ID (OCID (https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm)).
	...
	// The OCID (https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the VCN the local peering connection belongs to.
...
func (m LocalPeeringConnection) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingLocalPeeringConnectionLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
	...
	if _, ok := GetMappingLocalPeeringConnectionPeeringStatusEnum(string(m.PeeringStatus)); !ok && m.PeeringStatus != "" {
...
var mappingLocalPeeringConnectionLifecycleStateEnumLowerCase = map[string]LocalPeeringConnectionLifecycleStateEnum{
	"provisioning": LocalPeeringConnectionLifecycleStateProvisioning,
	"available":    LocalPeeringConnectionLifecycleStateAvailable,
	"terminating":  LocalPeeringConnectionLifecycleStateTerminating,
	"terminated":   LocalPeeringConnectionLifecycleStateTerminated,
...
// GetMappingLocalPeeringConnectionLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingLocalPeeringConnectionLifecycleStateEnum(val string) (LocalPeeringConnectionLifecycleStateEnum, bool) {
	enum, ok := mappingLocalPeeringConnectionLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
...
var mappingLocalPeeringConnectionPeeringStatusEnumLowerCase = map[string]LocalPeeringConnectionPeeringStatusEnum{
	"invalid": LocalPeeringConnectionPeeringStatusInvalid,
	"new":     LocalPeeringConnectionPeeringStatusNew,
	"peered":  LocalPeeringConnectionPeeringStatusPeered,
	"pending": LocalPeeringConnectionPeeringStatusPending,
...
// GetMappingLocalPeeringConnectionPeeringStatusEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingLocalPeeringConnectionPeeringStatusEnum(val string) (LocalPeeringConnectionPeeringStatusEnum, bool) {
	enum, ok := mappingLocalPeeringConnectionPeeringStatusEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
