...
func (request DeleteRuleSetRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["ruleSetName"] != nil {
		templateParam := mandatoryParamMap["ruleSetName"]
		for _, template := range templateParam {
			replacementParam := *request.RuleSetName
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
