...
func (request DeleteCertificateRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["certificateName"] != nil {
		templateParam := mandatoryParamMap["certificateName"]
		for _, template := range templateParam {
			replacementParam := *request.CertificateName
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
