...
func (request GetRoutingPolicyRequest) ReplaceMandatoryParamInPath(client *common.BaseClient, mandatoryParamMap map[string][]common.TemplateParamForPerRealmEndpoint) {
	...
	if mandatoryParamMap["routingPolicyName"] != nil {
		templateParam := mandatoryParamMap["routingPolicyName"]
		for _, template := range templateParam {
			replacementParam := *request.RoutingPolicyName
			if template.EndsWithDot {
				replacementParam = replacementParam + "."
			}
			client.Host = strings.Replace(client.Host, template.Template, replacementParam, -1)
