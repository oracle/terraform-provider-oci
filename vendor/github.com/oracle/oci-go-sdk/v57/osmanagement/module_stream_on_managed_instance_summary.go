...
type ModuleStreamOnManagedInstanceSummary struct {
	...
	Profiles []ModuleStreamProfileSummary `mandatory:"false" json:"profiles"`
...
func (m ModuleStreamOnManagedInstanceSummary) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingModuleStreamOnManagedInstanceSummaryStatusEnum[string(m.Status)]; !ok && m.Status != "" {
