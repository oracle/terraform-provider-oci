...
func (request ListReplicationTargetsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListReplicationTargetsLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListReplicationTargetsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListReplicationTargetsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
