...
func (m CreateDisApplicationDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingCreateDisApplicationDetailsModelTypeEnum[string(m.ModelType)]; !ok && m.ModelType != "" {
	...
	if _, ok := mappingCreateDisApplicationDetailsLifecycleStateEnum[string(m.LifecycleState)]; !ok && m.LifecycleState != "" {
