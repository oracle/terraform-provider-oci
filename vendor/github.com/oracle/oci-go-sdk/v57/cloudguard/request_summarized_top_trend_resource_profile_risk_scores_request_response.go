...
func (request RequestSummarizedTopTrendResourceProfileRiskScoresRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingRequestSummarizedTopTrendResourceProfileRiskScoresSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingRequestSummarizedTopTrendResourceProfileRiskScoresAccessLevelEnum[string(request.AccessLevel)]; !ok && request.AccessLevel != "" {
