...
func (request ListDedicatedVantagePointsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListDedicatedVantagePointsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListDedicatedVantagePointsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListDedicatedVantagePointsStatusEnum[string(request.Status)]; !ok && request.Status != "" {
