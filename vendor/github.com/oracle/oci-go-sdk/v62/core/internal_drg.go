...
func (m InternalDrg) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingInternalDrgLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
...
var mappingInternalDrgLifecycleStateEnumLowerCase = map[string]InternalDrgLifecycleStateEnum{
	"provisioning": InternalDrgLifecycleStateProvisioning,
	"available":    InternalDrgLifecycleStateAvailable,
	"terminating":  InternalDrgLifecycleStateTerminating,
	"terminated":   InternalDrgLifecycleStateTerminated,
...
// GetMappingInternalDrgLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingInternalDrgLifecycleStateEnum(val string) (InternalDrgLifecycleStateEnum, bool) {
	enum, ok := mappingInternalDrgLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
...
var mappingInternalDrgDrgTypeEnumLowerCase = map[string]InternalDrgDrgTypeEnum{
	"drg_classical":   InternalDrgDrgTypeClassical,
	"drg_transit_hub": InternalDrgDrgTypeTransitHub,
}
