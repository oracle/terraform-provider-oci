...
func (m EndpointService) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingEndpointServiceLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
...
var mappingEndpointServiceLifecycleStateEnumLowerCase = map[string]EndpointServiceLifecycleStateEnum{
	"provisioning": EndpointServiceLifecycleStateProvisioning,
	"available":    EndpointServiceLifecycleStateAvailable,
	"terminating":  EndpointServiceLifecycleStateTerminating,
	"terminated":   EndpointServiceLifecycleStateTerminated,
...
// GetMappingEndpointServiceLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingEndpointServiceLifecycleStateEnum(val string) (EndpointServiceLifecycleStateEnum, bool) {
	enum, ok := mappingEndpointServiceLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
