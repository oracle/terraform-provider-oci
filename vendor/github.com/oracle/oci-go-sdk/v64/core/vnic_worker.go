...
func (m VnicWorker) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingVnicWorkerLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
...
// GetMappingVnicWorkerLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingVnicWorkerLifecycleStateEnum(val string) (VnicWorkerLifecycleStateEnum, bool) {
	enum, ok := mappingVnicWorkerLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
