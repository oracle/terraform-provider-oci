...
func (m *DatabaseToolsKeyStoreMySql) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		KeyStoreContent  databasetoolskeystorecontentmysql  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordmysql `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentMySql)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordMySql)
	} else {
