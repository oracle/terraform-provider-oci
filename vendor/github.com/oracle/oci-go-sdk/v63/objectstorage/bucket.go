...
func (m Bucket) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingBucketObjectLevelAuditModeEnum(string(m.ObjectLevelAuditMode)); !ok && m.ObjectLevelAuditMode != "" {
...
var mappingBucketObjectLevelAuditModeEnumLowerCase = map[string]BucketObjectLevelAuditModeEnum{
	"disabled":  BucketObjectLevelAuditModeDisabled,
	"write":     BucketObjectLevelAuditModeWrite,
	"readwrite": BucketObjectLevelAuditModeReadwrite,
}
...
// GetMappingBucketObjectLevelAuditModeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingBucketObjectLevelAuditModeEnum(val string) (BucketObjectLevelAuditModeEnum, bool) {
	enum, ok := mappingBucketObjectLevelAuditModeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
