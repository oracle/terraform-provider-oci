...
func (m *DatabaseToolsKeyStoreMySqlDetails) UnmarshalJSON(data []byte) (e error) {
	model := struct {
		...
		KeyStoreContent  databasetoolskeystorecontentmysqldetails  `json:"keyStoreContent"`
		KeyStorePassword databasetoolskeystorepasswordmysqldetails `json:"keyStorePassword"`
	}{}
	...
	if nn != nil {
		m.KeyStoreContent = nn.(DatabaseToolsKeyStoreContentMySqlDetails)
	} else {
	...
	if nn != nil {
		m.KeyStorePassword = nn.(DatabaseToolsKeyStorePasswordMySqlDetails)
	} else {
