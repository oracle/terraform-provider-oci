...
func (m ExcludedObjectSummary) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingReasonKeywordsEnum(string(m.ReasonCategory)); !ok && m.ReasonCategory != "" {
		errMessage = append(errMessage, fmt.Sprintf("unsupported enum value for ReasonCategory: %s. Supported values are: %s.", m.ReasonCategory, strings.Join(GetReasonKeywordsEnumStringValues(), ",")))
	}
