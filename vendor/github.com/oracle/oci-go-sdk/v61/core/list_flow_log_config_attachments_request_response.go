...
func (request ListFlowLogConfigAttachmentsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingListFlowLogConfigAttachmentsTargetEntityTypeEnum(string(request.TargetEntityType)); !ok && request.TargetEntityType != "" {
...
// GetMappingListFlowLogConfigAttachmentsTargetEntityTypeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingListFlowLogConfigAttachmentsTargetEntityTypeEnum(val string) (ListFlowLogConfigAttachmentsTargetEntityTypeEnum, bool) {
	enum, ok := mappingListFlowLogConfigAttachmentsTargetEntityTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
