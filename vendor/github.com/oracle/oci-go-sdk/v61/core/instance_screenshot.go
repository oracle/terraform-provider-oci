...
func (m InstanceScreenshot) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := GetMappingInstanceScreenshotLifecycleStateEnum(string(m.LifecycleState)); !ok && m.LifecycleState != "" {
...
var mappingInstanceScreenshotLifecycleStateEnumLowerCase = map[string]InstanceScreenshotLifecycleStateEnum{
	"creating": InstanceScreenshotLifecycleStateCreating,
	"active":   InstanceScreenshotLifecycleStateActive,
	"failed":   InstanceScreenshotLifecycleStateFailed,
	"deleted":  InstanceScreenshotLifecycleStateDeleted,
...
// GetMappingInstanceScreenshotLifecycleStateEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingInstanceScreenshotLifecycleStateEnum(val string) (InstanceScreenshotLifecycleStateEnum, bool) {
	enum, ok := mappingInstanceScreenshotLifecycleStateEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
