...
func (m CreateDavDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := GetMappingCreateDavDetailsLabelTypeEnum(string(m.LabelType)); !ok && m.LabelType != "" {
...
var mappingCreateDavDetailsLabelTypeEnumLowerCase = map[string]CreateDavDetailsLabelTypeEnum{
	"mpls": CreateDavDetailsLabelTypeMpls,
}
...
// GetMappingCreateDavDetailsLabelTypeEnum performs case Insensitive comparison on enum value and return the desired enum
func GetMappingCreateDavDetailsLabelTypeEnum(val string) (CreateDavDetailsLabelTypeEnum, bool) {
	enum, ok := mappingCreateDavDetailsLabelTypeEnumLowerCase[strings.ToLower(val)]
	return enum, ok
}
