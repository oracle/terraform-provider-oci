...
func (m ManagementAgentImage) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingPackageTypesEnum[string(m.PackageType)]; !ok && m.PackageType != "" {
	...
	if _, ok := mappingArchitectureTypesEnum[string(m.PackageArchitectureType)]; !ok && m.PackageArchitectureType != "" {
