...
func (request ListModuleStreamsOnManagedInstanceRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListModuleStreamsOnManagedInstanceStreamStatusEnum[string(request.StreamStatus)]; !ok && request.StreamStatus != "" {
	...
	if _, ok := mappingListModuleStreamsOnManagedInstanceSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListModuleStreamsOnManagedInstanceSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
