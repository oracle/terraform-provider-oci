...
// KskDnssecKeyVersion A KSK DnssecKeyVersion. This contains timing and configuration data corresponding to the KSK that is used to
...
func (m KskDnssecKeyVersion) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingDnssecSigningAlgorithmEnum[string(m.Algorithm)]; !ok && m.Algorithm != "" {
