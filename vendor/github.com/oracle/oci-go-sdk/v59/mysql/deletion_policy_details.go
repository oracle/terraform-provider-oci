...
func (m DeletionPolicyDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingDeletionPolicyDetailsAutomaticBackupRetentionEnum[string(m.AutomaticBackupRetention)]; !ok && m.AutomaticBackupRetention != "" {
	...
	if _, ok := mappingDeletionPolicyDetailsFinalBackupEnum[string(m.FinalBackup)]; !ok && m.FinalBackup != "" {
