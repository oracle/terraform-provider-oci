...
func (m CreateDeletionPolicyDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingCreateDeletionPolicyDetailsAutomaticBackupRetentionEnum[string(m.AutomaticBackupRetention)]; !ok && m.AutomaticBackupRetention != "" {
	...
	if _, ok := mappingCreateDeletionPolicyDetailsFinalBackupEnum[string(m.FinalBackup)]; !ok && m.FinalBackup != "" {
