...
func (m UpdateVtapDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingUpdateVtapDetailsTargetTypeEnum[string(m.TargetType)]; !ok && m.TargetType != "" {
	...
	if _, ok := mappingUpdateVtapDetailsSourceTypeEnum[string(m.SourceType)]; !ok && m.SourceType != "" {
