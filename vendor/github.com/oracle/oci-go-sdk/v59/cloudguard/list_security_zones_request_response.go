...
func (request ListSecurityZonesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListSecurityZonesLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListSecurityZonesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListSecurityZonesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
