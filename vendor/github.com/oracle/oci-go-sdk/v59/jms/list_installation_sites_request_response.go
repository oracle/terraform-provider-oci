...
func (request ListInstallationSitesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListInstallationSitesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListInstallationSitesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListInstallationSitesJreSecurityStatusEnum[string(request.JreSecurityStatus)]; !ok && request.JreSecurityStatus != "" {
