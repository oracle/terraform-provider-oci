...
func (request ListBlocklistsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListBlocklistsOperationEnum[string(request.Operation)]; !ok && request.Operation != "" {
	...
	if _, ok := mappingListBlocklistsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListBlocklistsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
