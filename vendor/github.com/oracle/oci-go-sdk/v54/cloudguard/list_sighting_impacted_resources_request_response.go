...
func (request ListSightingImpactedResourcesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListSightingImpactedResourcesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListSightingImpactedResourcesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
