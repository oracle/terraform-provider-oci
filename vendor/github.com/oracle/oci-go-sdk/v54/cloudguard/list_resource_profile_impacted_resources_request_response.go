...
func (request ListResourceProfileImpactedResourcesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListResourceProfileImpactedResourcesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListResourceProfileImpactedResourcesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
