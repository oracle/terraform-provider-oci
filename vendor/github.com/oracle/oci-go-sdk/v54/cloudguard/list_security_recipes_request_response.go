...
func (request ListSecurityRecipesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListSecurityRecipesLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListSecurityRecipesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListSecurityRecipesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
