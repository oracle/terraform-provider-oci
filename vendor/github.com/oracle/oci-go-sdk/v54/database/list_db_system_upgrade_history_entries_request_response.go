...
func (request ListDbSystemUpgradeHistoryEntriesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListDbSystemUpgradeHistoryEntriesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListDbSystemUpgradeHistoryEntriesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingDbSystemUpgradeHistoryEntrySummaryActionEnum[string(request.UpgradeAction)]; !ok && request.UpgradeAction != "" {
	...
	if _, ok := mappingDbSystemUpgradeHistoryEntrySummaryLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
