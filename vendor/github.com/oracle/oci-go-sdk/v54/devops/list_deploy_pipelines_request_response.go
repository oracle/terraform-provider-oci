...
func (request ListDeployPipelinesRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingDeployPipelineLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListDeployPipelinesSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingListDeployPipelinesSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
