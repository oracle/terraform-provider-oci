...
func (request ListContinuousQueryRequest) RetryPolicy() *common.RetryPolicy {
...
func (request ListContinuousQueryRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingContinuousQuerySeverityEnum[string(request.Severity)]; !ok && request.Severity != "" {
	...
	if _, ok := mappingContinuousQueryLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
	...
	if _, ok := mappingListContinuousQuerySortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListContinuousQuerySortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
