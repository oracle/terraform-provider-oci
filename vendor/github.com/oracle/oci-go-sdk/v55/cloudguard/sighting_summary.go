...
func (m SightingSummary) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingClassificationStatusEnum[string(m.ClassificationStatus)]; !ok && m.ClassificationStatus != "" {
	...
	if _, ok := mappingSeverityEnum[string(m.Severity)]; !ok && m.Severity != "" {
	...
	if _, ok := mappingConfidenceEnum[string(m.Confidence)]; !ok && m.Confidence != "" {
