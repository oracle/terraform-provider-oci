...
func init() {
	RegisterDatasource("oci_data_safe_user_assessments", DataSafeUserAssessmentsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"user_assessments": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DataSafeUserAssessmentResource()),
			},
			"ignored_targets": {
				Type:     schema.TypeList,
				Computed: true,
...
func (s *DataSafeUserAssessmentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataSafeUserAssessmentsDataSource-", DataSafeUserAssessmentsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			userAssessment["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataSafeUserAssessmentsDataSource().Schema["user_assessments"].Elem.(*schema.Resource).Schema)
	}
