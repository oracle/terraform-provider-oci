...
func init() {
	RegisterResource("oci_data_labeling_service_dataset", DataLabelingServiceDatasetResource())
}
...
func (s *DataLabelingServiceDatasetResourceCrud) Create() error {
	...
	request.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service")
	...
	return s.getDatasetFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service"), oci_data_labeling_service.ActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}
...
func datasetWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	...
	return func(response oci_common.OCIOperationResponse) bool {
		...
		if shouldRetry(response, false, "data_labeling_service", startTime) {
			return true
		}
...
func datasetWaitForWorkRequest(wId *string, entityType string, action oci_data_labeling_service.ActionTypeEnum,
	...
	retryPolicy := GetRetryPolicy(disableFoundRetries, "data_labeling_service")
...
func (s *DataLabelingServiceDatasetResourceCrud) updateCompartment(compartment interface{}) error {
	...
	changeCompartmentRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service")
	...
	workId := response.OpcWorkRequestId
	return s.getDatasetFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service"), oci_data_labeling_service.ActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
}
...
func (s *DataLabelingServiceDatasetResourceCrud) updateLabelSet(oldLabelSet, newLabelSet oci_data_labeling_service.LabelSet) error {
	...
	if len(labelSetDiffAdd) > 0 {
		...
		addDatasetLabelsRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service")
		...
		workId := response.OpcWorkRequestId
		err = s.getDatasetFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service"), oci_data_labeling_service.ActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
		if err != nil {
			return err
		}
	}
	...
	if len(labelSetDiffRemove) > 0 {
		...
		removeDatasetLabelsRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "data_labeling_service")
