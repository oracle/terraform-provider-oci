...
func init() {
	RegisterDatasource("oci_devops_repository_refs", DevopsRepositoryRefsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsRepositoryRefResource()),
						},
					},
				},
			},
...
func (s *DevopsRepositoryRefsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoryRefsDataSource-", DevopsRepositoryRefsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoryRefsDataSource().Schema["repository_ref_collection"].Elem.(*schema.Resource).Schema)
		repositoryRef["items"] = items
	}
