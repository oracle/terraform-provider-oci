...
func (s *MysqlMysqlDbSystemResourceCrud) Create() error {
	...
	return s.getMysqlDbSystemFromWorkRequest(workId, getRetryPolicy(s.DisableNotFoundRetries, "mysql"), oci_mysql.WorkRequestResourceActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}

func (s *MysqlMysqlDbSystemResourceCrud) getMysqlDbSystemFromWorkRequest(workId *string, retryPolicy *oci_common.RetryPolicy,
	actionTypeEnum oci_mysql.WorkRequestResourceActionTypeEnum, timeout time.Duration) error {

	// Wait until it finishes
	mysqlDbSystemId, err := mysqlDbSystemWaitForWorkRequest(workId, "mysql",
		actionTypeEnum, timeout, s.DisableNotFoundRetries, s.WorkRequestClient)

	if err != nil {
		return err
	}
	s.D.SetId(*mysqlDbSystemId)

	return s.Get()
}

func mysqlDbSystemWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	startTime := time.Now()
...
func mysqlDbSystemWaitForWorkRequest(wId *string, entityType string, action oci_mysql.WorkRequestResourceActionTypeEnum,
	...
	retryPolicy.ShouldRetryOperation = mysqlDbSystemWorkRequestShouldRetryFunc(timeout)
	...
	if identifier == nil || response.Status == oci_mysql.WorkRequestOperationStatusFailed || response.Status == oci_mysql.WorkRequestOperationStatusCanceled {
		return nil, getErrorFromMysqlMysqlDbSystemWorkRequest(client, wId, retryPolicy, entityType, action)
	}

	return identifier, nil
}

func getErrorFromMysqlMysqlDbSystemWorkRequest(client *oci_mysql.WorkRequestsClient, workId *string, retryPolicy *oci_common.RetryPolicy, entityType string, action oci_mysql.WorkRequestResourceActionTypeEnum) error {
...
func (s *MysqlMysqlDbSystemResourceCrud) Update() error {
	...
	workId := response.OpcWorkRequestId
	return s.getMysqlDbSystemFromWorkRequest(workId, getRetryPolicy(s.DisableNotFoundRetries, "mysql"), oci_mysql.WorkRequestResourceActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
}
...
func (s *MysqlMysqlDbSystemResourceCrud) Delete() error {
	...
	workId := response.OpcWorkRequestId
	// Wait until it finishes
	_, delWorkRequestErr := mysqlDbSystemWaitForWorkRequest(workId, "mysql",
