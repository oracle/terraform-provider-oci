...
func init() {
	RegisterDatasource("oci_opsi_host_insights", OpsiHostInsightsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(OpsiHostInsightResource()),
						},
					},
				},
			},
...
func (s *OpsiHostInsightsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OpsiHostInsightsDataSource-", OpsiHostInsightsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OpsiHostInsightsDataSource().Schema["host_insight_summary_collection"].Elem.(*schema.Resource).Schema)
		hostInsight["items"] = items
	}
