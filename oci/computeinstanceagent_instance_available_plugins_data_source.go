...
func init() {
	RegisterDatasource("oci_computeinstanceagent_instance_available_plugins", ComputeinstanceagentInstanceAvailablePluginsDataSource())
}
...
func readComputeinstanceagentInstanceAvailablePlugins(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).pluginconfigClient()
...
func (s *ComputeinstanceagentInstanceAvailablePluginsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ComputeinstanceagentInstanceAvailablePluginsDataSource-", ComputeinstanceagentInstanceAvailablePluginsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, ComputeinstanceagentInstanceAvailablePluginsDataSource().Schema["available_plugins"].Elem.(*schema.Resource).Schema)
	}
