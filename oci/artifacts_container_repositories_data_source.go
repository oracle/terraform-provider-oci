...
func init() {
	RegisterDatasource("oci_artifacts_container_repositories", ArtifactsContainerRepositoriesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(ArtifactsContainerRepositoryResource()),
						},
...
func (s *ArtifactsContainerRepositoriesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ArtifactsContainerRepositoriesDataSource-", ArtifactsContainerRepositoriesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ArtifactsContainerRepositoriesDataSource().Schema["container_repository_collection"].Elem.(*schema.Resource).Schema)
		containerRepository["items"] = items
	}
