...
func init() {
	RegisterDatasource("oci_log_analytics_namespaces", LogAnalyticsNamespacesDataSource())
}
...
func (s *LogAnalyticsNamespacesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("LogAnalyticsNamespacesDataSource-", LogAnalyticsNamespacesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, LogAnalyticsNamespacesDataSource().Schema["namespace_collection"].Elem.(*schema.Resource).Schema)
		namespace["items"] = items
	}
