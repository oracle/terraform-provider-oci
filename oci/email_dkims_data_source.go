...
func init() {
	RegisterDatasource("oci_email_dkims", EmailDkimsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(EmailDkimResource()),
						},
					},
				},
			},
...
func (s *EmailDkimsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("EmailDkimsDataSource-", EmailDkimsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, EmailDkimsDataSource().Schema["dkim_collection"].Elem.(*schema.Resource).Schema)
		dkim["items"] = items
	}
