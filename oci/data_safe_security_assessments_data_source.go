...
func init() {
	RegisterDatasource("oci_data_safe_security_assessments", DataSafeSecurityAssessmentsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"security_assessments": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DataSafeSecurityAssessmentResource()),
			},
		},
	}
}
...
func (s *DataSafeSecurityAssessmentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataSafeSecurityAssessmentsDataSource-", DataSafeSecurityAssessmentsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			securityAssessment["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataSafeSecurityAssessmentsDataSource().Schema["security_assessments"].Elem.(*schema.Resource).Schema)
	}
