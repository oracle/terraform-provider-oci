...
func init() {
	RegisterDatasource("oci_database_cloud_vm_clusters", DatabaseCloudVmClustersDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"cloud_vm_clusters": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseCloudVmClusterResource()),
			},
		},
	}
}
...
func (s *DatabaseCloudVmClustersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseCloudVmClustersDataSource-", DatabaseCloudVmClustersDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			cloudVmCluster["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseCloudVmClustersDataSource().Schema["cloud_vm_clusters"].Elem.(*schema.Resource).Schema)
	}
