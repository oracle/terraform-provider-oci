...
import (
	...
	oci_certificates_management "github.com/oracle/oci-go-sdk/v55/certificatesmanagement"
...
var (
	...
	caBundleRepresentationRequired = map[string]interface{}{
		...
		"compartment_id": Representation{RepType: Required, Create: `${var.compartment_id}`},
		"name":           Representation{RepType: Required, Create: caBundleRequiredResourceName},
		"defined_tags":   Representation{RepType: Optional, Create: `${map("${oci_identity_tag_namespace.tag-namespace1.name}.${oci_identity_tag.tag1.name}", "value")}`, Update: `${map("${oci_identity_tag_namespace.tag-namespace1.name}.${oci_identity_tag.tag1.name}", "updatedValue")}`},
		"description":    Representation{RepType: Optional, Create: `description`, Update: `description2`},
		"freeform_tags":  Representation{RepType: Optional, Create: map[string]string{"Department": "Finance"}, Update: map[string]string{"Department": "Accounting"}},
		"lifecycle":      RepresentationGroup{Required, certificatesManagementIgnoreChangesRepresentation},
...
func TestCertificatesManagementCaBundleResource_basic(t *testing.T) {
	...
	SaveConfigContent(config+compartmentIdVariableStr+CaBundleResourceDependencies+
		GenerateResourceFromRepresentationMap("oci_certificates_management_ca_bundle", "test_ca_bundle", Optional, Create, caBundleRepresentation), "certificatesmanagement", "caBundle", t)
		...
		Steps: []resource.TestStep{
			// verify create
			{
				Config: config + compartmentIdVariableStr +
					GenerateResourceFromRepresentationMap("oci_certificates_management_ca_bundle", "test_ca_bundle", Required, Create,
						RepresentationCopyWithNewProperties(caBundleRepresentation, map[string]interface{}{
							"name": Representation{RepType: Required, Create: caBundleRequiredResourceName},
						})),
			...
			{
				Config: config + compartmentIdVariableStr + CaBundleResourceDependencies +
					GenerateResourceFromRepresentationMap("oci_certificates_management_ca_bundle", "test_ca_bundle", Optional, Create, caBundleRepresentation),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet(resourceName, "ca_bundle_pem"),
					resource.TestCheckResourceAttr(resourceName, "compartment_id", compartmentId),
			...
			// verify update to the compartment (the compartment will be switched back in the next step)
			{
				Config: config + compartmentIdVariableStr + compartmentIdUVariableStr + CaBundleResourceDependencies +
					GenerateResourceFromRepresentationMap("oci_certificates_management_ca_bundle", "test_ca_bundle", Optional, Create,
						RepresentationCopyWithNewProperties(caBundleRepresentation, map[string]interface{}{
			...
			{
				Config: config +
					GenerateDataSourceFromRepresentationMap("oci_certificates_management_ca_bundles", "test_ca_bundles", Optional, Create, caBundleDataSourceRepresentation) +
			...
			{
				Config: config +
					GenerateDataSourceFromRepresentationMap("oci_certificates_management_ca_bundle", "test_ca_bundle", Required, Create, caBundleSingularDataSourceRepresentation) +
...
func testAccCheckCertificatesManagementCaBundleDestroy(s *terraform.State) error {
	...
	client := testAccProvider.Meta().(*OracleClients).certificatesManagementClient()
	for _, rs := range s.RootModule().Resources {
		if rs.Type == "oci_certificates_management_ca_bundle" {
			...
			request.RequestMetadata.RetryPolicy = GetRetryPolicy(true, "certificates_management")
...
func sweepCertificatesManagementCaBundleResource(compartment string) error {
	certificatesManagementClient := GetTestClients(&schema.ResourceData{}).certificatesManagementClient()
	caBundleIds, err := getCaBundleIds(compartment)
	if err != nil {
		return err
	}
	for _, caBundleId := range caBundleIds {
		if ok := SweeperDefaultResourceId[caBundleId]; !ok {
			deleteCaBundleRequest := oci_certificates_management.DeleteCaBundleRequest{}

			deleteCaBundleRequest.CaBundleId = &caBundleId

			deleteCaBundleRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(true, "certificates_management")
			...
			WaitTillCondition(testAccProvider, &caBundleId, caBundleSweepWaitCondition, time.Duration(3*time.Minute),
...
func getCaBundleIds(compartment string) ([]string, error) {
	ids := GetResourceIdsToSweep(compartment, "CaBundleId")
	if ids != nil {
		return ids, nil
	}
	...
	for _, caBundle := range listCaBundlesResponse.Items {
		...
		AddResourceIdToSweeperResourceIdMap(compartmentId, "CaBundleId", id)
	}
...
func caBundleSweepResponseFetchOperation(client *OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
