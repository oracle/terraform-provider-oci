...
func init() {
	RegisterResource("oci_dataflow_private_endpoint", DataflowPrivateEndpointResource())
}
...
func privateEndpointWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	...
	return func(response oci_common.OCIOperationResponse) bool {
		...
		if shouldRetry(response, false, "dataflow", startTime) {
			return true
		}
...
func privateEndpointWaitForWorkRequest(wId *string, entityType string, action oci_dataflow.WorkRequestResourceActionTypeEnum,
	...
	if identifier == nil || response.Status == oci_dataflow.WorkRequestStatusFailed || response.Status == oci_dataflow.WorkRequestStatusCancelled {
		return nil, getErrorFromPrivateEndpointWorkRequest(client, wId, retryPolicy, entityType, action)
	}

	return identifier, nil
}

func getErrorFromPrivateEndpointWorkRequest(client *oci_dataflow.DataFlowClient, wId *string, retryPolicy *oci_common.RetryPolicy, entityType string, action oci_dataflow.WorkRequestResourceActionTypeEnum) error {
