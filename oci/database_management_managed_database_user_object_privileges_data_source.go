...
func init() {
	RegisterDatasource("oci_database_management_managed_database_user_object_privileges", DatabaseManagementManagedDatabaseUserObjectPrivilegesDataSource())
}
...
func (s *DatabaseManagementManagedDatabaseUserObjectPrivilegesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseUserObjectPrivilegesDataSource-", DatabaseManagementManagedDatabaseUserObjectPrivilegesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseUserObjectPrivilegesDataSource().Schema["object_privilege_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseUserObjectPrivilege["items"] = items
	}
