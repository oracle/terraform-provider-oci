...
import (
	...
	oci_cloud_guard "github.com/oracle/oci-go-sdk/v30/cloudguard"
)

func init() {
	RegisterDatasource("oci_cloud_guard_detector_recipes", CloudGuardDetectorRecipesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(CloudGuardDetectorRecipeResource()),
						},
					},
				},
			},
...
func (s *CloudGuardDetectorRecipesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, CloudGuardDetectorRecipesDataSource().Schema["detector_recipe_collection"].Elem.(*schema.Resource).Schema)
		detectorRecipe["items"] = items
	}
