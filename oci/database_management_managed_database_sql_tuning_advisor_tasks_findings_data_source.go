...
func init() {
	RegisterDatasource("oci_database_management_managed_database_sql_tuning_advisor_tasks_findings", DatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindingsDataSource())
}
...
func readDatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindings(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).sqlTuningClient()
...
func (s *DatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindingsDataSource-", DatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindingsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseSqlTuningAdvisorTasksFindingsDataSource().Schema["sql_tuning_advisor_task_finding_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseSqlTuningAdvisorTasksFinding["items"] = items
	}
