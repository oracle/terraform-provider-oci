...
import (
	...
	oci_usage_proxy "github.com/oracle/oci-go-sdk/v51/usage"
)

func init() {
	RegisterDatasource("oci_usage_proxy_subscription_products", UsageProxySubscriptionProductsDataSource())
}
...
func (s *UsageProxySubscriptionProductsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("UsageProxySubscriptionProductsDataSource-", UsageProxySubscriptionProductsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		...
		items = ApplyFiltersInCollection(f.(*schema.Set), items, UsageProxySubscriptionProductsDataSource().Schema["product_collection"].Elem.(*schema.Resource).Schema)
		subscriptionProduct["items"] = items
	}
