...
func init() {
	RegisterDatasource("oci_devops_build_pipelines", DevopsBuildPipelinesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsBuildPipelineResource()),
						},
					},
				},
			},
...
func (s *DevopsBuildPipelinesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsBuildPipelinesDataSource-", DevopsBuildPipelinesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsBuildPipelinesDataSource().Schema["build_pipeline_collection"].Elem.(*schema.Resource).Schema)
		buildPipeline["items"] = items
	}
