...
func init() {
	RegisterDatasource("oci_database_management_managed_database_user_proxied_for_users", DatabaseManagementManagedDatabaseUserProxiedForUsersDataSource())
}
...
func (s *DatabaseManagementManagedDatabaseUserProxiedForUsersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseUserProxiedForUsersDataSource-", DatabaseManagementManagedDatabaseUserProxiedForUsersDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseUserProxiedForUsersDataSource().Schema["proxied_for_user_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseUserProxiedForUser["items"] = items
	}
