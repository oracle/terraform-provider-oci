...
func init() {
	RegisterDatasource("oci_devops_deployments", DevopsDeploymentsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsDeploymentResource()),
						},
					},
				},
			},
...
func (s *DevopsDeploymentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsDeploymentsDataSource-", DevopsDeploymentsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsDeploymentsDataSource().Schema["deployment_collection"].Elem.(*schema.Resource).Schema)
		deployment["items"] = items
	}
