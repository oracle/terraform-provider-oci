...
func init() {
	RegisterDatasource("oci_database_database_software_images", DatabaseDatabaseSoftwareImagesDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"database_software_images": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseDatabaseSoftwareImageResource()),
			},
		},
	}
}
...
func (s *DatabaseDatabaseSoftwareImagesDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			databaseSoftwareImage["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseDatabaseSoftwareImagesDataSource().Schema["database_software_images"].Elem.(*schema.Resource).Schema)
	}
