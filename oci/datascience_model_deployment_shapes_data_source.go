...
func init() {
	RegisterDatasource("oci_datascience_model_deployment_shapes", DatascienceModelDeploymentShapesDataSource())
}
...
func (s *DatascienceModelDeploymentShapesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatascienceModelDeploymentShapesDataSource-", DatascienceModelDeploymentShapesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceModelDeploymentShapesDataSource().Schema["model_deployment_shapes"].Elem.(*schema.Resource).Schema)
	}
