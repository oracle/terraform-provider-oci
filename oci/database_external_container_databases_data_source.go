...
func init() {
	RegisterDatasource("oci_database_external_container_databases", DatabaseExternalContainerDatabasesDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"external_container_databases": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseExternalContainerDatabaseResource()),
			},
		},
	}
}
...
func (s *DatabaseExternalContainerDatabasesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseExternalContainerDatabasesDataSource-", DatabaseExternalContainerDatabasesDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			externalContainerDatabase["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseExternalContainerDatabasesDataSource().Schema["external_container_databases"].Elem.(*schema.Resource).Schema)
	}
