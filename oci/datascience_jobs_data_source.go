...
func init() {
	RegisterDatasource("oci_datascience_jobs", DatascienceJobsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"jobs": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatascienceJobResource()),
			},
		},
	}
}
...
func (s *DatascienceJobsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatascienceJobsDataSource-", DatascienceJobsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			job["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceJobsDataSource().Schema["jobs"].Elem.(*schema.Resource).Schema)
	}
