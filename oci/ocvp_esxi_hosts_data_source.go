...
func init() {
	RegisterDatasource("oci_ocvp_esxi_hosts", OcvpEsxiHostsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"esxi_host_collection": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(OcvpEsxiHostResource()),
			},
		},
	}
}
...
func (s *OcvpEsxiHostsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, OcvpEsxiHostsDataSource().Schema["esxi_host_collection"].Elem.(*schema.Resource).Schema)
	}
