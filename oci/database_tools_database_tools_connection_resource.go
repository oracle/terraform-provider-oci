...
func init() {
	RegisterResource("oci_database_tools_database_tools_connection", DatabaseToolsDatabaseToolsConnectionResource())
}
...
func createDatabaseToolsDatabaseToolsConnection(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).databaseToolsClient()
...
func (s *DatabaseToolsDatabaseToolsConnectionResourceCrud) Create() error {
	...
	request.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "database_tools")
	...
	return s.getDatabaseToolsConnectionFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "database_tools"), oci_database_tools.ActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}
...
func databaseToolsConnectionWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	...
	return func(response oci_common.OCIOperationResponse) bool {
		...
		if shouldRetry(response, false, "database_tools", startTime) {
			return true
		}
...
func databaseToolsConnectionWaitForWorkRequest(wId *string, entityType string, action oci_database_tools.ActionTypeEnum,
	...
	retryPolicy := GetRetryPolicy(disableFoundRetries, "database_tools")
...
func (s *DatabaseToolsDatabaseToolsConnectionResourceCrud) Update() error {
	...
	workId := response.OpcWorkRequestId
	return s.getDatabaseToolsConnectionFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "database_tools"), oci_database_tools.ActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
}
...
func (s *DatabaseToolsDatabaseToolsConnectionResourceCrud) populateTopLevelPolymorphicCreateDatabaseToolsConnectionRequest(request *oci_database_tools.CreateDatabaseToolsConnectionRequest) error {
	...
	case strings.ToLower("ORACLE_DATABASE"):
		...
		if advancedProperties, ok := s.D.GetOkExists("advanced_properties"); ok {
			details.AdvancedProperties = ObjectMapToStringMap(advancedProperties.(map[string]interface{}))
		}
...
func (s *DatabaseToolsDatabaseToolsConnectionResourceCrud) updateCompartment(compartment interface{}) error {
	...
	changeCompartmentRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(s.DisableNotFoundRetries, "database_tools")
