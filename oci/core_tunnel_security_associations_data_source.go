...
func init() {
	RegisterDatasource("oci_core_tunnel_security_associations", CoreTunnelSecurityAssociationsDataSource())
}
...
func (s *CoreTunnelSecurityAssociationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("CoreTunnelSecurityAssociationsDataSource-", CoreTunnelSecurityAssociationsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreTunnelSecurityAssociationsDataSource().Schema["tunnel_security_associations"].Elem.(*schema.Resource).Schema)
	}
