...
func init() {
	RegisterDatasource("oci_database_vm_cluster_patches", DatabaseVmClusterPatchesDataSource())
}
...
func (s *DatabaseVmClusterPatchesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseVmClusterPatchesDataSource().Schema["patches"].Elem.(*schema.Resource).Schema)
	}
