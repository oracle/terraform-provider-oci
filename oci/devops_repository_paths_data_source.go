...
func init() {
	RegisterDatasource("oci_devops_repository_paths", DevopsRepositoryPathsDataSource())
}
...
func (s *DevopsRepositoryPathsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoryPathsDataSource-", DevopsRepositoryPathsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoryPathsDataSource().Schema["repository_path_collection"].Elem.(*schema.Resource).Schema)
		repositoryPath["items"] = items
	}
