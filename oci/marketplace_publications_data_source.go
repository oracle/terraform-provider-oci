...
func init() {
	RegisterDatasource("oci_marketplace_publications", MarketplacePublicationsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"publications": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     GetDataSourceItemSchema(MarketplacePublicationResource()),
			},
		},
	}
}
...
func (s *MarketplacePublicationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("MarketplacePublicationsDataSource-", MarketplacePublicationsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, MarketplacePublicationsDataSource().Schema["publications"].Elem.(*schema.Resource).Schema)
	}
