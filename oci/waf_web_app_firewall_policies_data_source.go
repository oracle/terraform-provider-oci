...
func init() {
	RegisterDatasource("oci_waf_web_app_firewall_policies", WafWebAppFirewallPoliciesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(WafWebAppFirewallPolicyResource()),
						},
					},
				},
			},
...
func (s *WafWebAppFirewallPoliciesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("WafWebAppFirewallPoliciesDataSource-", WafWebAppFirewallPoliciesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, WafWebAppFirewallPoliciesDataSource().Schema["web_app_firewall_policy_collection"].Elem.(*schema.Resource).Schema)
		webAppFirewallPolicy["items"] = items
	}
