...
func init() {
	RegisterDatasource("oci_bds_bds_instance_api_keys", BdsBdsInstanceApiKeysDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"bds_api_keys": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(BdsBdsInstanceApiKeyResource()),
			},
		},
	}
}
...
func (s *BdsBdsInstanceApiKeysDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("BdsBdsInstanceApiKeysDataSource-", BdsBdsInstanceApiKeysDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, BdsBdsInstanceApiKeysDataSource().Schema["bds_api_keys"].Elem.(*schema.Resource).Schema)
	}
