...
func init() {
	RegisterDatasource("oci_database_external_pluggable_databases", DatabaseExternalPluggableDatabasesDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"external_pluggable_databases": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseExternalPluggableDatabaseResource()),
			},
		},
	}
}
...
func (s *DatabaseExternalPluggableDatabasesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseExternalPluggableDatabasesDataSource-", DatabaseExternalPluggableDatabasesDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			externalPluggableDatabase["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseExternalPluggableDatabasesDataSource().Schema["external_pluggable_databases"].Elem.(*schema.Resource).Schema)
	}
