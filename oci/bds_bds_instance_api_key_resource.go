...
func init() {
	RegisterResource("oci_bds_bds_instance_api_key", BdsBdsInstanceApiKeyResource())
}
...
func (s *BdsBdsInstanceApiKeyResourceCrud) Create() error {
	...
	return s.getBdsInstanceApiKeyFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "bds"), oci_bds.ActionTypesCreated, s.D.Timeout(schema.TimeoutCreate))
}
