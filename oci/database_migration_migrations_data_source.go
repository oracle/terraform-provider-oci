...
func init() {
	RegisterDatasource("oci_database_migration_migrations", DatabaseMigrationMigrationsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DatabaseMigrationMigrationResource()),
						},
					},
				},
			},
...
func (s *DatabaseMigrationMigrationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseMigrationMigrationsDataSource-", DatabaseMigrationMigrationsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseMigrationMigrationsDataSource().Schema["migration_collection"].Elem.(*schema.Resource).Schema)
		migration["items"] = items
	}
