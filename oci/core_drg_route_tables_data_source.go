...
func init() {
	RegisterDatasource("oci_core_drg_route_tables", CoreDrgRouteTablesDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"drg_route_tables": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(CoreDrgRouteTableResource()),
			},
		},
	}
}
...
func (s *CoreDrgRouteTablesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("CoreDrgRouteTablesDataSource-", CoreDrgRouteTablesDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			drgRouteTable["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreDrgRouteTablesDataSource().Schema["drg_route_tables"].Elem.(*schema.Resource).Schema)
	}
