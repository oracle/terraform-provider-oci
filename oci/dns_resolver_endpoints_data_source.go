...
func init() {
	RegisterDatasource("oci_dns_resolver_endpoints", DnsResolverEndpointsDataSource())
}
...
func (s *DnsResolverEndpointsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DnsResolverEndpointsDataSource-", DnsResolverEndpointsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DnsResolverEndpointsDataSource().Schema["resolver_endpoints"].Elem.(*schema.Resource).Schema)
	}
