...
func init() {
	RegisterResource("oci_devops_repository_mirror", DevopsRepositoryMirrorResource())
}
...
func (s *DevopsRepositoryMirrorResourceCrud) Create() error {
	...
	return s.getRepositoryMirrorFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "devops"), oci_devops.ActionTypeUpdated, s.D.Timeout(schema.TimeoutCreate))
}
