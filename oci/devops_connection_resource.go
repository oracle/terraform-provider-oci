...
func init() {
	RegisterResource("oci_devops_connection", DevopsConnectionResource())
}
...
func (s *DevopsConnectionResourceCrud) Create() error {
	...
	return s.getConnectionFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "devops"), oci_devops.ActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}
...
func devopsConnectionWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	...
	return func(response oci_common.OCIOperationResponse) bool {
		...
		if shouldRetry(response, false, "devops", startTime) {
			return true
		}
...
func (s *DevopsConnectionResourceCrud) Update() error {
	...
	workId := response.OpcWorkRequestId
	return s.getConnectionFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "devops"), oci_devops.ActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
}
