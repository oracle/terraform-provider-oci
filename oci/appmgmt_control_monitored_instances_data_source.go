...
func init() {
	RegisterDatasource("oci_appmgmt_control_monitored_instances", AppmgmtControlMonitoredInstancesDataSource())
}
...
func (s *AppmgmtControlMonitoredInstancesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("AppmgmtControlMonitoredInstancesDataSource-", AppmgmtControlMonitoredInstancesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, AppmgmtControlMonitoredInstancesDataSource().Schema["monitored_instance_collection"].Elem.(*schema.Resource).Schema)
		monitoredInstance["items"] = items
	}
