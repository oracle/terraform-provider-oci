...
func init() {
	RegisterDatasource("oci_usage_proxy_subscription_redeemable_users", UsageProxySubscriptionRedeemableUsersDataSource())
}
...
func (s *UsageProxySubscriptionRedeemableUsersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("UsageProxySubscriptionRedeemableUsersDataSource-", UsageProxySubscriptionRedeemableUsersDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, UsageProxySubscriptionRedeemableUsersDataSource().Schema["redeemable_user_collection"].Elem.(*schema.Resource).Schema)
		subscriptionRedeemableUser["items"] = items
	}
