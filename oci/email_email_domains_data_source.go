...
func init() {
	RegisterDatasource("oci_email_email_domains", EmailEmailDomainsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(EmailEmailDomainResource()),
						},
					},
				},
			},
...
func (s *EmailEmailDomainsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("EmailEmailDomainsDataSource-", EmailEmailDomainsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, EmailEmailDomainsDataSource().Schema["email_domain_collection"].Elem.(*schema.Resource).Schema)
		emailDomain["items"] = items
	}
