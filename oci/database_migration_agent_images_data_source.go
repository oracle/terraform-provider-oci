...
func init() {
	RegisterDatasource("oci_database_migration_agent_images", DatabaseMigrationAgentImagesDataSource())
}
...
func (s *DatabaseMigrationAgentImagesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseMigrationAgentImagesDataSource-", DatabaseMigrationAgentImagesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseMigrationAgentImagesDataSource().Schema["agent_image_collection"].Elem.(*schema.Resource).Schema)
		agentImage["items"] = items
	}
