...
func init() {
	RegisterDatasource("oci_devops_repositories", DevopsRepositoriesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsRepositoryResource()),
						},
					},
				},
			},
...
func (s *DevopsRepositoriesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoriesDataSource-", DevopsRepositoriesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoriesDataSource().Schema["repository_collection"].Elem.(*schema.Resource).Schema)
		repository["items"] = items
	}
