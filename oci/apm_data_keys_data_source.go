...
func init() {
	RegisterDatasource("oci_apm_data_keys", ApmDataKeysDataSource())
}
...
func (s *ApmDataKeysDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ApmDataKeysDataSource-", ApmDataKeysDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, ApmDataKeysDataSource().Schema["data_keys"].Elem.(*schema.Resource).Schema)
	}
