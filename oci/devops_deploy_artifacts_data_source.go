...
func init() {
	RegisterDatasource("oci_devops_deploy_artifacts", DevopsDeployArtifactsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsDeployArtifactResource()),
						},
					},
				},
			},
...
func (s *DevopsDeployArtifactsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsDeployArtifactsDataSource-", DevopsDeployArtifactsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsDeployArtifactsDataSource().Schema["deploy_artifact_collection"].Elem.(*schema.Resource).Schema)
		deployArtifact["items"] = items
	}
