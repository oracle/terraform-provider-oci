...
import (
	...
	oci_database_management "github.com/oracle/oci-go-sdk/v35/databasemanagement"
)

func init() {
	RegisterDatasource("oci_database_management_managed_database_groups", DatabaseManagementManagedDatabaseGroupsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DatabaseManagementManagedDatabaseGroupResource()),
						},
					},
				},
			},
...
func (s *DatabaseManagementManagedDatabaseGroupsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseGroupsDataSource-", DatabaseManagementManagedDatabaseGroupsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseGroupsDataSource().Schema["managed_database_group_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseGroup["items"] = items
	}
