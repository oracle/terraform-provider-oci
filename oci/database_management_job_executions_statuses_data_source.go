...
func init() {
	RegisterDatasource("oci_database_management_job_executions_statuses", DatabaseManagementJobExecutionsStatusesDataSource())
}
...
func (s *DatabaseManagementJobExecutionsStatusesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementJobExecutionsStatusesDataSource-", DatabaseManagementJobExecutionsStatusesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementJobExecutionsStatusesDataSource().Schema["job_executions_status_summary_collection"].Elem.(*schema.Resource).Schema)
		jobExecutionsStatus["items"] = items
	}
