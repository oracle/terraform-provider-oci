...
func init() {
	RegisterDatasource("oci_ai_anomaly_detection_projects", AiAnomalyDetectionProjectsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(AiAnomalyDetectionProjectResource()),
						},
					},
				},
			},
...
func (s *AiAnomalyDetectionProjectsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("AiAnomalyDetectionProjectsDataSource-", AiAnomalyDetectionProjectsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, AiAnomalyDetectionProjectsDataSource().Schema["project_collection"].Elem.(*schema.Resource).Schema)
		project["items"] = items
	}
