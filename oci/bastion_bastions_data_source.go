...
func init() {
	RegisterDatasource("oci_bastion_bastions", BastionBastionsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"bastions": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(BastionBastionResource()),
			},
		},
	}
}
...
func (s *BastionBastionsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("BastionBastionsDataSource-", BastionBastionsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			bastion["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
		...
		if r.SystemTags != nil {
			bastion["system_tags"] = systemTagsToMap(r.SystemTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, BastionBastionsDataSource().Schema["bastions"].Elem.(*schema.Resource).Schema)
	}
