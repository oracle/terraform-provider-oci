...
func init() {
	RegisterDatasource("oci_service_catalog_private_application_packages", ServiceCatalogPrivateApplicationPackagesDataSource())
}
...
func (s *ServiceCatalogPrivateApplicationPackagesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ServiceCatalogPrivateApplicationPackagesDataSource-", ServiceCatalogPrivateApplicationPackagesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ServiceCatalogPrivateApplicationPackagesDataSource().Schema["private_application_package_collection"].Elem.(*schema.Resource).Schema)
		privateApplicationPackage["items"] = items
	}
