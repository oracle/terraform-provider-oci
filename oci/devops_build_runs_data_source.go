...
func init() {
	RegisterDatasource("oci_devops_build_runs", DevopsBuildRunsDataSource())
}
...
func (s *DevopsBuildRunsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsBuildRunsDataSource-", DevopsBuildRunsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsBuildRunsDataSource().Schema["build_run_summary_collection"].Elem.(*schema.Resource).Schema)
		buildRun["items"] = items
	}
