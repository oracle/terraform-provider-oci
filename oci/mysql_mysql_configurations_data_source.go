...
import (
	...
	oci_mysql "github.com/oracle/oci-go-sdk/v32/mysql"
)

func init() {
	RegisterDatasource("oci_mysql_mysql_configurations", MysqlMysqlConfigurationsDataSource())
}
...
func (s *MysqlMysqlConfigurationsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			mysqlConfiguration["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, MysqlMysqlConfigurationsDataSource().Schema["configurations"].Elem.(*schema.Resource).Schema)
	}
