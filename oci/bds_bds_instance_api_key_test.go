...
var (
	BdsInstanceApiKeyRequiredOnlyResource = BdsInstanceApiKeyResourceDependencies +
		GenerateResourceFromRepresentationMap("oci_bds_bds_instance_api_key", "test_bds_instance_api_key", Required, Create, bdsInstanceApiKeyRepresentation)

	BdsInstanceApiKeyResourceConfig = BdsInstanceApiKeyResourceDependencies +
		GenerateResourceFromRepresentationMap("oci_bds_bds_instance_api_key", "test_bds_instance_api_key", Optional, Update, bdsInstanceApiKeyRepresentation)

	bdsInstanceApiKeySingularDataSourceRepresentation = map[string]interface{}{
		"api_key_id":      Representation{RepType: Required, Create: `${oci_bds_bds_instance_api_key.test_bds_instance_api_key.id}`},
	...
	bdsInstanceApiKeyDataSourceRepresentation = map[string]interface{}{
		...
		"filter": RepresentationGroup{Required, bdsInstanceApiKeyDataSourceFilterRepresentation}}
	bdsInstanceApiKeyDataSourceFilterRepresentation = map[string]interface{}{
		"name":   Representation{RepType: Required, Create: `id`},
		"values": Representation{RepType: Required, Create: []string{`${oci_bds_bds_instance_api_key.test_bds_instance_api_key.id}`}},
	}

	bdsInstanceApiKeyRepresentation = map[string]interface{}{
		"bds_instance_id": Representation{RepType: Required, Create: `${oci_bds_bds_instance.test_bds_instance.id}`},
		"key_alias":       Representation{RepType: Required, Create: `keyAlias`},
		"passphrase":      Representation{RepType: Required, Create: `V2VsY29tZTE=`},
		"user_id":         Representation{RepType: Required, Create: `${oci_identity_user.test_user.id}`},
		"default_region":  Representation{RepType: Optional, Create: `us-ashburn-1`},
	}

	BdsInstanceApiKeyResourceDependencies = GenerateResourceFromRepresentationMap("oci_bds_bds_instance", "test_bds_instance", Required, Create, bdsInstanceOdhRepresentation) +
...
func TestBdsBdsInstanceApiKeyResource_basic(t *testing.T) {
	...
	SaveConfigContent(config+compartmentIdVariableStr+BdsInstanceApiKeyResourceDependencies+
		GenerateResourceFromRepresentationMap("oci_bds_bds_instance_api_key", "test_bds_instance_api_key", Optional, Create, bdsInstanceApiKeyRepresentation), "bds", "bdsInstanceApiKey", t)

	ResourceTest(t, testAccCheckBdsBdsInstanceApiKeyDestroy, []resource.TestStep{
		// verify Create
		{
		...
		{
			Config: config + compartmentIdVariableStr + BdsInstanceApiKeyResourceDependencies +
				GenerateResourceFromRepresentationMap("oci_bds_bds_instance_api_key", "test_bds_instance_api_key", Optional, Create, bdsInstanceApiKeyRepresentation),
			Check: ComposeAggregateTestCheckFuncWrapper(
				resource.TestCheckResourceAttrSet(resourceName, "bds_instance_id"),
		...
		{
			Config: config +
				GenerateDataSourceFromRepresentationMap("oci_bds_bds_instance_api_keys", "test_bds_instance_api_keys", Optional, Update, bdsInstanceApiKeyDataSourceRepresentation) +
		...
		{
			Config: config +
				GenerateDataSourceFromRepresentationMap("oci_bds_bds_instance_api_key", "test_bds_instance_api_key", Required, Create, bdsInstanceApiKeySingularDataSourceRepresentation) +
		...
		{
			Config: config + compartmentIdVariableStr + BdsInstanceApiKeyResourceConfig,
		},
...
func init() {
	...
	if !InSweeperExcludeList("BdsBdsInstanceApiKey") {
		resource.AddTestSweepers("BdsBdsInstanceApiKey", &resource.Sweeper{
			Name:         "BdsBdsInstanceApiKey",
			Dependencies: DependencyGraph["bdsInstanceApiKey"],
...
func sweepBdsBdsInstanceApiKeyResource(compartment string) error {
	...
	for _, bdsInstanceApiKeyId := range bdsInstanceApiKeyIds {
		if ok := SweeperDefaultResourceId[bdsInstanceApiKeyId]; !ok {
			deleteBdsApiKeyRequest := oci_bds.DeleteBdsApiKeyRequest{}

			deleteBdsApiKeyRequest.RequestMetadata.RetryPolicy = GetRetryPolicy(true, "bds")
			...
			WaitTillCondition(testAccProvider, &bdsInstanceApiKeyId, bdsInstanceApiKeySweepWaitCondition, time.Duration(3*time.Minute),
...
func getBdsInstanceApiKeyIds(compartment string) ([]string, error) {
	ids := GetResourceIdsToSweep(compartment, "BdsInstanceApiKeyId")
	if ids != nil {
		return ids, nil
	}
	...
	for _, bdsInstanceId := range bdsInstanceIds {
		...
		for _, bdsInstanceApiKey := range listBdsApiKeysResponse.Items {
			...
			AddResourceIdToSweeperResourceIdMap(compartmentId, "BdsInstanceApiKeyId", id)
			SweeperDefaultResourceId[*bdsInstanceApiKey.DefaultRegion] = true
...
func bdsInstanceApiKeySweepResponseFetchOperation(client *OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
