...
func init() {
	RegisterDatasource("oci_apm_synthetics_public_vantage_points", ApmSyntheticsPublicVantagePointsDataSource())
}
...
func (s *ApmSyntheticsPublicVantagePointsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ApmSyntheticsPublicVantagePointsDataSource-", ApmSyntheticsPublicVantagePointsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ApmSyntheticsPublicVantagePointsDataSource().Schema["public_vantage_point_collection"].Elem.(*schema.Resource).Schema)
		publicVantagePoint["items"] = items
	}
