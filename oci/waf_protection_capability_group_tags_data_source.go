...
func init() {
	RegisterDatasource("oci_waf_protection_capability_group_tags", WafProtectionCapabilityGroupTagsDataSource())
}
...
func (s *WafProtectionCapabilityGroupTagsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("WafProtectionCapabilityGroupTagsDataSource-", WafProtectionCapabilityGroupTagsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, WafProtectionCapabilityGroupTagsDataSource().Schema["protection_capability_group_tag_collection"].Elem.(*schema.Resource).Schema)
		protectionCapabilityGroupTag["items"] = items
	}
