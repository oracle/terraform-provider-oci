...
func init() {
	RegisterDatasource("oci_core_vlans", CoreVlansDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vlans": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(CoreVlanResource()),
			},
		},
	}
}
...
func (s *CoreVlansDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("CoreVlansDataSource-", CoreVlansDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			vlan["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreVlansDataSource().Schema["vlans"].Elem.(*schema.Resource).Schema)
	}
