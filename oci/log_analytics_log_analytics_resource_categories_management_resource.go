...
func init() {
	RegisterResource("oci_log_analytics_log_analytics_resource_categories_management", LogAnalyticsLogAnalyticsResourceCategoriesManagementResource())
}
...
func (s *LogAnalyticsLogAnalyticsResourceCategoriesManagementResourceCrud) SetData() error {
	if resourceCategories, ok := s.D.GetOkExists("resource_categories"); ok {
		...
		s.D.Set("resource_categories", schema.NewSet(LiteralTypeHashCodeForSets, checkItems))
	}
