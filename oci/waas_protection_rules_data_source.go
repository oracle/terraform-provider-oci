...
func init() {
	RegisterDatasource("oci_waas_protection_rules", WaasProtectionRulesDataSource())
}
...
func (s *WaasProtectionRulesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, WaasProtectionRulesDataSource().Schema["protection_rules"].Elem.(*schema.Resource).Schema)
	}
