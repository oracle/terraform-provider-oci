...
var (
	EmHostInsightRequiredOnlyResource = HostInsightResourceDependencies +
		GenerateResourceFromRepresentationMap("oci_opsi_host_insight", "test_host_insight", Required, Create, emHostInsightRepresentation)
	...
	emHostInsightDataSourceRepresentation = map[string]interface{}{
		...
		"filter":                       RepresentationGroup{Required, emHostInsightDataSourceFilterRepresentation},
	}
	...
	emHostInsightRepresentation = map[string]interface{}{
		...
		"lifecycle":                            RepresentationGroup{Required, ignoreChangesEmHostInsightRepresentation},
	}
...
func TestOpsiEmHostInsightResource_basic(t *testing.T) {
	...
	SaveConfigContent(config+compartmentIdVariableStr+emBridgeIdVariableStr+enterpriseManagerIdVariableStr+enterpriseManagerEntityIdVariableStr+EmHostInsightResourceDependencies+
		GenerateResourceFromRepresentationMap("oci_opsi_host_insight", "test_host_insight", Optional, Create, emHostInsightRepresentation), "opsi", "hostInsight", t)

	ResourceTest(t, testAccCheckOpsiEmHostInsightDestroy, []resource.TestStep{
		...
		//verify Update to the compartment (the compartment will be switched back in the next step)
		{
			Config: config + compartmentIdVariableStr + emBridgeIdVariableStr + enterpriseManagerIdVariableStr + enterpriseManagerEntityIdVariableStr + compartmentIdUVariableStr + EmHostInsightResourceDependencies +
				GenerateResourceFromRepresentationMap("oci_opsi_host_insight", "test_host_insight", Optional, Create,
					RepresentationCopyWithNewProperties(emHostInsightRepresentation, map[string]interface{}{
		...
		{
			Config: config +
				GenerateDataSourceFromRepresentationMap("oci_opsi_host_insights", "test_host_insights", Optional, Update, emHostInsightDataSourceRepresentation) +
		...
		{
			Config: config +
				GenerateDataSourceFromRepresentationMap("oci_opsi_host_insight", "test_host_insight", Required, Create, emHostInsightSingularDataSourceRepresentation) +
...
func init() {
	...
	if !InSweeperExcludeList("OpsiEmHostInsight") {
...
func sweepOpsiEmHostInsightResource(compartment string) error {
	...
	for _, hostInsightId := range hostInsightIds {
		if ok := SweeperDefaultResourceId[hostInsightId]; !ok {
			...
			WaitTillCondition(testAccProvider, &hostInsightId, emHostInsightDisableSweepWaitCondition, time.Duration(3*time.Minute),
			...
			WaitTillCondition(testAccProvider, &hostInsightId, emHostInsightSweepWaitCondition, time.Duration(3*time.Minute),
				emHostInsightSweepResponseFetchOperation, "opsi", true)
		}
	}
	return nil
...
func emHostInsightSweepResponseFetchOperation(client *OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
