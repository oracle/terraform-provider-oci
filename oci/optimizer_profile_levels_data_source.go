...
func init() {
	RegisterDatasource("oci_optimizer_profile_levels", OptimizerProfileLevelsDataSource())
}
...
func (s *OptimizerProfileLevelsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OptimizerProfileLevelsDataSource-", OptimizerProfileLevelsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OptimizerProfileLevelsDataSource().Schema["profile_level_collection"].Elem.(*schema.Resource).Schema)
		profileLevel["items"] = items
	}
