...
func init() {
	RegisterDatasource("oci_optimizer_enrollment_statuses", OptimizerEnrollmentStatusesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(OptimizerEnrollmentStatusResource()),
						},
					},
				},
			},
...
func (s *OptimizerEnrollmentStatusesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OptimizerEnrollmentStatusesDataSource-", OptimizerEnrollmentStatusesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OptimizerEnrollmentStatusesDataSource().Schema["enrollment_status_collection"].Elem.(*schema.Resource).Schema)
		enrollmentStatus["items"] = items
	}
