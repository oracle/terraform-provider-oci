...
func init() {
	RegisterDatasource("oci_cloud_guard_managed_lists", CloudGuardManagedListsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(CloudGuardManagedListResource()),
						},
					},
				},
			},
...
func (s *CloudGuardManagedListsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, CloudGuardManagedListsDataSource().Schema["managed_list_collection"].Elem.(*schema.Resource).Schema)
		managedList["items"] = items
	}
