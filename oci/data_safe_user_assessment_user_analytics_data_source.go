...
func init() {
	RegisterDatasource("oci_data_safe_user_assessment_user_analytics", DataSafeUserAssessmentUserAnalyticsDataSource())
}
...
func (s *DataSafeUserAssessmentUserAnalyticsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataSafeUserAssessmentUserAnalyticsDataSource-", DataSafeUserAssessmentUserAnalyticsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataSafeUserAssessmentUserAnalyticsDataSource().Schema["user_aggregations"].Elem.(*schema.Resource).Schema)
	}
