...
func init() {
	RegisterDatasource("oci_core_compute_global_image_capability_schemas", CoreComputeGlobalImageCapabilitySchemasDataSource())
}
...
func (s *CoreComputeGlobalImageCapabilitySchemasDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			computeGlobalImageCapabilitySchema["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreComputeGlobalImageCapabilitySchemasDataSource().Schema["compute_global_image_capability_schemas"].Elem.(*schema.Resource).Schema)
	}
