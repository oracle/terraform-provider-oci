...
func init() {
	RegisterDatasource("oci_database_management_managed_database_users", DatabaseManagementManagedDatabaseUsersDataSource())
}
...
func (s *DatabaseManagementManagedDatabaseUsersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseUsersDataSource-", DatabaseManagementManagedDatabaseUsersDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseUsersDataSource().Schema["user_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseUser["items"] = items
	}
