...
func init() {
	RegisterDatasource("oci_database_cloud_autonomous_vm_clusters", DatabaseCloudAutonomousVmClustersDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"cloud_autonomous_vm_clusters": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseCloudAutonomousVmClusterResource()),
			},
		},
	}
}
...
func (s *DatabaseCloudAutonomousVmClustersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseCloudAutonomousVmClustersDataSource-", DatabaseCloudAutonomousVmClustersDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			cloudAutonomousVmCluster["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseCloudAutonomousVmClustersDataSource().Schema["cloud_autonomous_vm_clusters"].Elem.(*schema.Resource).Schema)
	}
