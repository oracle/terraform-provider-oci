...
import (
	...
	oci_jms "github.com/oracle/oci-go-sdk/v52/jms"
)

func init() {
	RegisterDatasource("oci_jms_fleet", JmsFleetDataSource())
}
...
func JmsFleetDataSource() *schema.Resource {
	...
	return GetSingularDataSourceItemSchema(JmsFleetResource(), fieldMap, readSingularJmsFleet)
}

func readSingularJmsFleet(d *schema.ResourceData, m interface{}) error {
	sync := &JmsFleetDataSourceCrud{}
	sync.D = d
	sync.Client = m.(*OracleClients).javaManagementServiceClient()
