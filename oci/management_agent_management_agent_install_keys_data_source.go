...
func init() {
	RegisterDatasource("oci_management_agent_management_agent_install_keys", ManagementAgentManagementAgentInstallKeysDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"management_agent_install_keys": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(ManagementAgentManagementAgentInstallKeyResource()),
			},
		},
	}
}
...
func (s *ManagementAgentManagementAgentInstallKeysDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, ManagementAgentManagementAgentInstallKeysDataSource().Schema["management_agent_install_keys"].Elem.(*schema.Resource).Schema)
	}
