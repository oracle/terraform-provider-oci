...
func init() {
	RegisterDatasource("oci_dns_resolvers", DnsResolversDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"resolvers": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DnsResolverResource()),
			},
		},
	}
}
...
func (s *DnsResolversDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DnsResolversDataSource-", DnsResolversDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			resolver["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DnsResolversDataSource().Schema["resolvers"].Elem.(*schema.Resource).Schema)
	}
