...
func init() {
	RegisterDatasource("oci_log_analytics_log_analytics_object_collection_rules", LogAnalyticsLogAnalyticsObjectCollectionRulesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(LogAnalyticsLogAnalyticsObjectCollectionRuleResource()),
						},
					},
				},
			},
...
func (s *LogAnalyticsLogAnalyticsObjectCollectionRulesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("LogAnalyticsLogAnalyticsObjectCollectionRulesDataSource-", LogAnalyticsLogAnalyticsObjectCollectionRulesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, LogAnalyticsLogAnalyticsObjectCollectionRulesDataSource().Schema["log_analytics_object_collection_rule_collection"].Elem.(*schema.Resource).Schema)
		logAnalyticsObjectCollectionRule["items"] = items
	}
