...
func init() {
	RegisterResource("oci_opsi_awr_hub", OpsiAwrHubResource())
}
...
func (s *OpsiAwrHubResourceCrud) Create() error {
	...
	return s.getAwrHubFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "opsi"), oci_opsi.ActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}
...
func (s *OpsiAwrHubResourceCrud) Update() error {
	...
	workId := response.OpcWorkRequestId
	return s.getAwrHubFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "opsi"), oci_opsi.ActionTypeUpdated, s.D.Timeout(schema.TimeoutUpdate))
}
