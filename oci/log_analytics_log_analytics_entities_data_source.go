...
import (
	...
	oci_log_analytics "github.com/oracle/oci-go-sdk/v39/loganalytics"
)

func init() {
	RegisterDatasource("oci_log_analytics_log_analytics_entities", LogAnalyticsLogAnalyticsEntitiesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(LogAnalyticsLogAnalyticsEntityResource()),
						},
					},
				},
			},
...
func (s *LogAnalyticsLogAnalyticsEntitiesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("LogAnalyticsLogAnalyticsEntitiesDataSource-", LogAnalyticsLogAnalyticsEntitiesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, LogAnalyticsLogAnalyticsEntitiesDataSource().Schema["log_analytics_entity_collection"].Elem.(*schema.Resource).Schema)
		logAnalyticsEntity["items"] = items
	}
