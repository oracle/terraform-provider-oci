...
import (
	...
	oci_artifacts "github.com/oracle/oci-go-sdk/v37/artifacts"
)

func init() {
	RegisterDatasource("oci_artifacts_container_images", ArtifactsContainerImagesDataSource())
}
...
func (s *ArtifactsContainerImagesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ArtifactsContainerImagesDataSource-", ArtifactsContainerImagesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ArtifactsContainerImagesDataSource().Schema["container_image_collection"].Elem.(*schema.Resource).Schema)
		containerImage["items"] = items
	}
