...
func init() {
	RegisterDatasource("oci_osmanagement_managed_instance_event_report", OsmanagementManagedInstanceEventReportDataSource())
}
...
func readSingularOsmanagementManagedInstanceEventReport(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).eventClient()
...
func (s *OsmanagementManagedInstanceEventReportDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OsmanagementManagedInstanceEventReportDataSource-", OsmanagementManagedInstanceEventReportDataSource(), s.D))
