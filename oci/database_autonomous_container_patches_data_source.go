...
func init() {
	RegisterDatasource("oci_database_autonomous_container_patches", DatabaseAutonomousContainerPatchesDataSource())
}
...
func (s *DatabaseAutonomousContainerPatchesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseAutonomousContainerPatchesDataSource-", DatabaseAutonomousContainerPatchesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseAutonomousContainerPatchesDataSource().Schema["autonomous_patches"].Elem.(*schema.Resource).Schema)
	}
