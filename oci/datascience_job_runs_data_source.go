...
func init() {
	RegisterDatasource("oci_datascience_job_runs", DatascienceJobRunsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"job_runs": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatascienceJobRunResource()),
			},
		},
	}
}
...
func (s *DatascienceJobRunsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatascienceJobRunsDataSource-", DatascienceJobRunsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			jobRun["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceJobRunsDataSource().Schema["job_runs"].Elem.(*schema.Resource).Schema)
	}
