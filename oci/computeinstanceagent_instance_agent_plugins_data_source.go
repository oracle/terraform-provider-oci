...
import (
	...
	oci_computeinstanceagent "github.com/oracle/oci-go-sdk/v38/computeinstanceagent"
)

func init() {
	RegisterDatasource("oci_computeinstanceagent_instance_agent_plugins", ComputeinstanceagentInstanceAgentPluginsDataSource())
}
...
func readComputeinstanceagentInstanceAgentPlugins(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).pluginClient()
...
func (s *ComputeinstanceagentInstanceAgentPluginsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ComputeinstanceagentInstanceAgentPluginsDataSource-", ComputeinstanceagentInstanceAgentPluginsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, ComputeinstanceagentInstanceAgentPluginsDataSource().Schema["instance_agent_plugins"].Elem.(*schema.Resource).Schema)
	}
