...
func init() {
	RegisterDatasource("oci_waf_network_address_lists", WafNetworkAddressListsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(WafNetworkAddressListResource()),
						},
					},
				},
			},
...
func (s *WafNetworkAddressListsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("WafNetworkAddressListsDataSource-", WafNetworkAddressListsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, WafNetworkAddressListsDataSource().Schema["network_address_list_collection"].Elem.(*schema.Resource).Schema)
		networkAddressList["items"] = items
	}
