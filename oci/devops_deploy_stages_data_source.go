...
func init() {
	RegisterDatasource("oci_devops_deploy_stages", DevopsDeployStagesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DevopsDeployStageResource()),
						},
					},
				},
			},
...
func (s *DevopsDeployStagesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsDeployStagesDataSource-", DevopsDeployStagesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsDeployStagesDataSource().Schema["deploy_stage_collection"].Elem.(*schema.Resource).Schema)
		deployStage["items"] = items
	}
