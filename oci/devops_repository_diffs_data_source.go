...
func init() {
	RegisterDatasource("oci_devops_repository_diffs", DevopsRepositoryDiffsDataSource())
}
...
func (s *DevopsRepositoryDiffsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoryDiffsDataSource-", DevopsRepositoryDiffsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoryDiffsDataSource().Schema["diff_collection"].Elem.(*schema.Resource).Schema)
		repositoryDiff["items"] = items
	}
