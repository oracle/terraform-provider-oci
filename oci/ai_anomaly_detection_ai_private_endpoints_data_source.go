...
func init() {
	RegisterDatasource("oci_ai_anomaly_detection_ai_private_endpoints", AiAnomalyDetectionAiPrivateEndpointsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(AiAnomalyDetectionAiPrivateEndpointResource()),
						},
					},
				},
			},
...
func (s *AiAnomalyDetectionAiPrivateEndpointsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("AiAnomalyDetectionAiPrivateEndpointsDataSource-", AiAnomalyDetectionAiPrivateEndpointsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, AiAnomalyDetectionAiPrivateEndpointsDataSource().Schema["ai_private_endpoint_collection"].Elem.(*schema.Resource).Schema)
		aiPrivateEndpoint["items"] = items
	}
