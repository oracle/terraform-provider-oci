...
func (s *DatascienceNotebookSessionResourceCrud) Create() error {
	...
	return s.getNotebookSessionFromWorkRequest(workId, getRetryPolicy(s.DisableNotFoundRetries, "datascience"), oci_datascience.WorkRequestResourceActionTypeCreated, s.D.Timeout(schema.TimeoutCreate))
}

func (s *DatascienceNotebookSessionResourceCrud) getNotebookSessionFromWorkRequest(workId *string, retryPolicy *oci_common.RetryPolicy,
	actionTypeEnum oci_datascience.WorkRequestResourceActionTypeEnum, timeout time.Duration) error {

	// Wait until it finishes
	notebookSessionId, err := notebookSessionWaitForWorkRequest(workId, "datascience",
		actionTypeEnum, timeout, s.DisableNotFoundRetries, s.Client)

	if err != nil {
		// Try to cancel the work request
		log.Printf("[DEBUG] creation failed, attempting to cancel the workrequest: %v for identifier: %v\n", workId, notebookSessionId)
	...
	s.D.SetId(*notebookSessionId)

	return s.Get()
}

func notebookSessionWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	startTime := time.Now()
...
func notebookSessionWaitForWorkRequest(wId *string, entityType string, action oci_datascience.WorkRequestResourceActionTypeEnum,
	...
	retryPolicy.ShouldRetryOperation = notebookSessionWorkRequestShouldRetryFunc(timeout)
	...
	if identifier == nil || response.Status == oci_datascience.WorkRequestStatusFailed || response.Status == oci_datascience.WorkRequestStatusCanceled {
		return nil, getErrorFromDatascienceNotebookSessionWorkRequest(client, wId, retryPolicy, entityType, action)
	}

	return identifier, nil
}

func getErrorFromDatascienceNotebookSessionWorkRequest(client *oci_datascience.DataScienceClient, workId *string, retryPolicy *oci_common.RetryPolicy, entityType string, action oci_datascience.WorkRequestResourceActionTypeEnum) error {
...
func (s *DatascienceNotebookSessionResourceCrud) Delete() error {
	...
	response, err := s.Client.DeleteNotebookSession(context.Background(), request)
	if err != nil {
		return err
	}
	...
	workId := response.OpcWorkRequestId
	// Wait until it finishes
	_, delWorkRequestErr := notebookSessionWaitForWorkRequest(workId, "datascience",
