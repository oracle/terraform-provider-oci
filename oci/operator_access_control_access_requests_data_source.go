...
import (
	...
	oci_operator_access_control "github.com/oracle/oci-go-sdk/v52/operatoraccesscontrol"
)

func init() {
	RegisterDatasource("oci_operator_access_control_access_requests", OperatorAccessControlAccessRequestsDataSource())
}
...
func readOperatorAccessControlAccessRequests(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).accessRequestsClient()
...
func (s *OperatorAccessControlAccessRequestsDataSourceCrud) Get() error {
	...
	request.RequestMetadata.RetryPolicy = GetRetryPolicy(false, "operator_access_control")
...
func (s *OperatorAccessControlAccessRequestsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OperatorAccessControlAccessRequestsDataSource-", OperatorAccessControlAccessRequestsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OperatorAccessControlAccessRequestsDataSource().Schema["access_request_collection"].Elem.(*schema.Resource).Schema)
		accessRequest["items"] = items
	}
