...
func init() {
	RegisterDatasource("oci_network_load_balancer_backends", NetworkLoadBalancerBackendsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(NetworkLoadBalancerBackendResource()),
						},
					},
				},
			},
...
func (s *NetworkLoadBalancerBackendsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("NetworkLoadBalancerBackendsDataSource-", NetworkLoadBalancerBackendsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, NetworkLoadBalancerBackendsDataSource().Schema["backend_collection"].Elem.(*schema.Resource).Schema)
		backend["items"] = items
	}
