...
import (
	...
	oci_golden_gate "github.com/oracle/oci-go-sdk/v37/goldengate"
)

func init() {
	RegisterDatasource("oci_golden_gate_deployments", GoldenGateDeploymentsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(GoldenGateDeploymentResource()),
						},
					},
				},
			},
...
func (s *GoldenGateDeploymentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("GoldenGateDeploymentsDataSource-", GoldenGateDeploymentsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, GoldenGateDeploymentsDataSource().Schema["deployment_collection"].Elem.(*schema.Resource).Schema)
		deployment["items"] = items
	}
