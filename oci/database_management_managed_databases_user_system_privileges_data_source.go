...
func init() {
	RegisterDatasource("oci_database_management_managed_databases_user_system_privileges", DatabaseManagementManagedDatabasesUserSystemPrivilegesDataSource())
}
...
func (s *DatabaseManagementManagedDatabasesUserSystemPrivilegesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabasesUserSystemPrivilegesDataSource-", DatabaseManagementManagedDatabasesUserSystemPrivilegesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabasesUserSystemPrivilegesDataSource().Schema["system_privilege_collection"].Elem.(*schema.Resource).Schema)
		managedDatabasesUserSystemPrivilege["items"] = items
	}
