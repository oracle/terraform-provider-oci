...
import (
	...
	oci_work_requests "github.com/oracle/oci-go-sdk/v51/workrequests"
)

func init() {
	RegisterResource("oci_database_cloud_database_management", DatabaseCloudDatabaseManagementResource())
}
...
func (s *DatabaseCloudDatabaseManagementResourceCrud) ID() string {
	return GenerateDataSourceHashID("DatabaseCloudDatabaseManagementResource-", DatabaseCloudDatabaseManagementResource(), s.D)
}
...
func (s *DatabaseCloudDatabaseManagementResourceCrud) Create() error {
	...
	if operation {
		...
		if workId != nil {
			_, err = WaitForWorkRequestWithErrorHandling(s.WorkRequestClient, workId, "database", oci_work_requests.WorkRequestResourceActionTypeUpdated, s.D.Timeout(schema.TimeoutCreate), s.DisableNotFoundRetries)
			if err != nil {
				return err
			}
		}
...
func (s *DatabaseCloudDatabaseManagementResourceCrud) getDatabaseFromWorkRequest(workId *string, actionTypeEnum oci_work_requests.WorkRequestResourceActionTypeEnum, timeout time.Duration) error {
	databaseId, err := WaitForWorkRequest(s.WorkRequestClient, workId, "database", actionTypeEnum, timeout, s.DisableNotFoundRetries, true)
