...
func init() {
	RegisterDatasource("oci_database_flex_components", DatabaseFlexComponentsDataSource())
}
...
func (s *DatabaseFlexComponentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseFlexComponentsDataSource-", DatabaseFlexComponentsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseFlexComponentsDataSource().Schema["flex_component_collection"].Elem.(*schema.Resource).Schema)
		flexComponent["items"] = items
	}
