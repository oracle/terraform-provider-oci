...
func init() {
	RegisterDatasource("oci_database_external_non_container_databases", DatabaseExternalNonContainerDatabasesDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"external_non_container_databases": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseExternalNonContainerDatabaseResource()),
			},
		},
	}
}
...
func (s *DatabaseExternalNonContainerDatabasesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseExternalNonContainerDatabasesDataSource-", DatabaseExternalNonContainerDatabasesDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			externalNonContainerDatabase["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseExternalNonContainerDatabasesDataSource().Schema["external_non_container_databases"].Elem.(*schema.Resource).Schema)
	}
