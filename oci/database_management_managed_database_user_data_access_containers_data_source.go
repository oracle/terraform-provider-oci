...
func init() {
	RegisterDatasource("oci_database_management_managed_database_user_data_access_containers", DatabaseManagementManagedDatabaseUserDataAccessContainersDataSource())
}
...
func (s *DatabaseManagementManagedDatabaseUserDataAccessContainersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseManagementManagedDatabaseUserDataAccessContainersDataSource-", DatabaseManagementManagedDatabaseUserDataAccessContainersDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseManagementManagedDatabaseUserDataAccessContainersDataSource().Schema["data_access_container_collection"].Elem.(*schema.Resource).Schema)
		managedDatabaseUserDataAccessContainer["items"] = items
	}
