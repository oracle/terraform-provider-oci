...
import (
	...
	oci_network_load_balancer "github.com/oracle/oci-go-sdk/v37/networkloadbalancer"
)

func init() {
	RegisterDatasource("oci_network_load_balancer_network_load_balancers", NetworkLoadBalancerNetworkLoadBalancersDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(NetworkLoadBalancerNetworkLoadBalancerResource()),
						},
					},
				},
			},
...
func (s *NetworkLoadBalancerNetworkLoadBalancersDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("NetworkLoadBalancerNetworkLoadBalancersDataSource-", NetworkLoadBalancerNetworkLoadBalancersDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, NetworkLoadBalancerNetworkLoadBalancersDataSource().Schema["network_load_balancer_collection"].Elem.(*schema.Resource).Schema)
		networkLoadBalancer["items"] = items
	}
