...
func init() {
	RegisterDatasource("oci_bastion_sessions", BastionSessionsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"sessions": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(BastionSessionResource()),
			},
		},
	}
}
...
func readBastionSessions(d *schema.ResourceData, m interface{}) error {
	...
	sync.D = d
	sync.Client = m.(*OracleClients).bastionClient()
...
func (s *BastionSessionsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("BastionSessionsDataSource-", BastionSessionsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, BastionSessionsDataSource().Schema["sessions"].Elem.(*schema.Resource).Schema)
	}
