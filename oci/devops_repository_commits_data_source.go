...
func init() {
	RegisterDatasource("oci_devops_repository_commits", DevopsRepositoryCommitsDataSource())
}
...
func (s *DevopsRepositoryCommitsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoryCommitsDataSource-", DevopsRepositoryCommitsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoryCommitsDataSource().Schema["repository_commit_collection"].Elem.(*schema.Resource).Schema)
		repositoryCommit["items"] = items
	}
