...
func init() {
	RegisterDatasource("oci_devops_repository_mirror_records", DevopsRepositoryMirrorRecordsDataSource())
}
...
func (s *DevopsRepositoryMirrorRecordsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DevopsRepositoryMirrorRecordsDataSource-", DevopsRepositoryMirrorRecordsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DevopsRepositoryMirrorRecordsDataSource().Schema["repository_mirror_record_collection"].Elem.(*schema.Resource).Schema)
		repositoryMirrorRecord["items"] = items
	}
