...
func init() {
	RegisterDatasource("oci_artifacts_container_image_signatures", ArtifactsContainerImageSignaturesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(ArtifactsContainerImageSignatureResource()),
						},
...
func (s *ArtifactsContainerImageSignaturesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ArtifactsContainerImageSignaturesDataSource-", ArtifactsContainerImageSignaturesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ArtifactsContainerImageSignaturesDataSource().Schema["container_image_signature_collection"].Elem.(*schema.Resource).Schema)
		containerImageSignature["items"] = items
	}
