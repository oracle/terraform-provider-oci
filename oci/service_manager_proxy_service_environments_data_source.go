...
func init() {
	RegisterDatasource("oci_service_manager_proxy_service_environments", ServiceManagerProxyServiceEnvironmentsDataSource())
}
...
func (s *ServiceManagerProxyServiceEnvironmentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ServiceManagerProxyServiceEnvironmentsDataSource-", ServiceManagerProxyServiceEnvironmentsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ServiceManagerProxyServiceEnvironmentsDataSource().Schema["service_environment_collection"].Elem.(*schema.Resource).Schema)
		serviceEnvironment["items"] = items
	}
