...
func init() {
	RegisterDatasource("oci_optimizer_recommendations", OptimizerRecommendationsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(OptimizerRecommendationResource()),
						},
					},
				},
			},
...
func (s *OptimizerRecommendationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OptimizerRecommendationsDataSource-", OptimizerRecommendationsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OptimizerRecommendationsDataSource().Schema["recommendation_collection"].Elem.(*schema.Resource).Schema)
		recommendation["items"] = items
	}
