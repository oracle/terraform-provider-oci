...
func init() {
	RegisterDatasource("oci_core_compute_capacity_reservations", CoreComputeCapacityReservationsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"compute_capacity_reservations": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(CoreComputeCapacityReservationResource()),
			},
		},
	}
}
...
func (s *CoreComputeCapacityReservationsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			computeCapacityReservation["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreComputeCapacityReservationsDataSource().Schema["compute_capacity_reservations"].Elem.(*schema.Resource).Schema)
	}
