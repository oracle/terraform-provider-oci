...
func init() {
	RegisterDatasource("oci_usage_proxy_subscription_rewards", UsageProxySubscriptionRewardsDataSource())
}
...
func (s *UsageProxySubscriptionRewardsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("UsageProxySubscriptionRewardsDataSource-", UsageProxySubscriptionRewardsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, UsageProxySubscriptionRewardsDataSource().Schema["reward_collection"].Elem.(*schema.Resource).Schema)
		subscriptionReward["items"] = items
	}
