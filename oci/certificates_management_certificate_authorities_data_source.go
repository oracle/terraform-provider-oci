...
func init() {
	RegisterDatasource("oci_certificates_management_certificate_authorities", CertificatesManagementCertificateAuthoritiesDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(CertificatesManagementCertificateAuthorityResource()),
						},
					},
				},
			},
...
func (s *CertificatesManagementCertificateAuthoritiesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("CertificatesManagementCertificateAuthoritiesDataSource-", CertificatesManagementCertificateAuthoritiesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, CertificatesManagementCertificateAuthoritiesDataSource().Schema["certificate_authority_collection"].Elem.(*schema.Resource).Schema)
		certificateAuthority["items"] = items
	}
