...
func init() {
	RegisterDatasource("oci_apm_synthetics_monitors", ApmSyntheticsMonitorsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(ApmSyntheticsMonitorResource()),
						},
					},
				},
			},
...
func (s *ApmSyntheticsMonitorsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ApmSyntheticsMonitorsDataSource-", ApmSyntheticsMonitorsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ApmSyntheticsMonitorsDataSource().Schema["monitor_collection"].Elem.(*schema.Resource).Schema)
		monitor["items"] = items
	}
