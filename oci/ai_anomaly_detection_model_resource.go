...
func init() {
	RegisterResource("oci_ai_anomaly_detection_model", AiAnomalyDetectionModelResource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"target_fap": {
							...
							Computed:         true,
							ForceNew:         true,
							DiffSuppressFunc: adDiffSuppress,
						},
						"training_fraction": {
							Type:             schema.TypeFloat,
							Optional:         true,
...
func modelWorkRequestShouldRetryFunc(timeout time.Duration) func(response oci_common.OCIOperationResponse) bool {
	...
	return func(response oci_common.OCIOperationResponse) bool {
		...
		if shouldRetry(response, false, "ai_anomaly_detection", startTime) {
			return true
		}
