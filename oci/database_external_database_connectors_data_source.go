...
func init() {
	RegisterDatasource("oci_database_external_database_connectors", DatabaseExternalDatabaseConnectorsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"external_database_connectors": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseExternalDatabaseConnectorResource()),
			},
		},
	}
}
...
func (s *DatabaseExternalDatabaseConnectorsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseExternalDatabaseConnectorsDataSource-", DatabaseExternalDatabaseConnectorsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseExternalDatabaseConnectorsDataSource().Schema["external_database_connectors"].Elem.(*schema.Resource).Schema)
	}
