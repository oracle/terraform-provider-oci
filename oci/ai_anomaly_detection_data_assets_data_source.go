...
func init() {
	RegisterDatasource("oci_ai_anomaly_detection_data_assets", AiAnomalyDetectionDataAssetsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(AiAnomalyDetectionDataAssetResource()),
						},
					},
				},
			},
...
func (s *AiAnomalyDetectionDataAssetsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("AiAnomalyDetectionDataAssetsDataSource-", AiAnomalyDetectionDataAssetsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, AiAnomalyDetectionDataAssetsDataSource().Schema["data_asset_collection"].Elem.(*schema.Resource).Schema)
		dataAsset["items"] = items
	}
