...
func init() {
	RegisterDatasource("oci_logging_unified_agent_configurations", LoggingUnifiedAgentConfigurationsDataSource())
}
...
func (s *LoggingUnifiedAgentConfigurationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("LoggingUnifiedAgentConfigurationsDataSource-", LoggingUnifiedAgentConfigurationsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, LoggingUnifiedAgentConfigurationsDataSource().Schema["unified_agent_configuration_collection"].Elem.(*schema.Resource).Schema)
		unifiedAgentConfiguration["items"] = items
	}
