...
func init() {
	RegisterDatasource("oci_dns_views", DnsViewsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"views": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DnsViewResource()),
			},
		},
	}
}
...
func (s *DnsViewsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DnsViewsDataSource-", DnsViewsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			view["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DnsViewsDataSource().Schema["views"].Elem.(*schema.Resource).Schema)
	}
