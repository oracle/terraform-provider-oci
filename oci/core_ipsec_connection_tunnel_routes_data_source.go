...
func init() {
	RegisterDatasource("oci_core_ipsec_connection_tunnel_routes", CoreIpsecConnectionTunnelRoutesDataSource())
}
...
func (s *CoreIpsecConnectionTunnelRoutesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("CoreIpsecConnectionTunnelRoutesDataSource-", CoreIpsecConnectionTunnelRoutesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreIpsecConnectionTunnelRoutesDataSource().Schema["tunnel_routes"].Elem.(*schema.Resource).Schema)
	}
