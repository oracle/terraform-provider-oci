...
import (
	...
	oci_apm_config "github.com/oracle/oci-go-sdk/v49/apmconfig"
)

func init() {
	RegisterDatasource("oci_apm_config_config", ApmConfigConfigDataSource())
}
...
func ApmConfigConfigDataSource() *schema.Resource {
	...
	return GetSingularDataSourceItemSchema(ApmConfigConfigResource(), fieldMap, readSingularApmConfigConfig)
}

func readSingularApmConfigConfig(d *schema.ResourceData, m interface{}) error {
	sync := &ApmConfigConfigDataSourceCrud{}
	sync.D = d
	sync.Client = m.(*OracleClients).configClient()
...
func (s *ApmConfigConfigDataSourceCrud) Get() error {
	...
	request.RequestMetadata.RetryPolicy = GetRetryPolicy(false, "apm_config")
