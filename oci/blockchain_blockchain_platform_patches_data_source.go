...
func init() {
	RegisterDatasource("oci_blockchain_blockchain_platform_patches", BlockchainBlockchainPlatformPatchesDataSource())
}
...
func (s *BlockchainBlockchainPlatformPatchesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("BlockchainBlockchainPlatformPatchesDataSource-", BlockchainBlockchainPlatformPatchesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, BlockchainBlockchainPlatformPatchesDataSource().Schema["blockchain_platform_patch_collection"].Elem.(*schema.Resource).Schema)
		blockchainPlatformPatch["items"] = items
	}
