...
func init() {
	RegisterDatasource("oci_apm_config_configs", ApmConfigConfigsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(ApmConfigConfigResource()),
						},
					},
				},
			},
...
func (s *ApmConfigConfigsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ApmConfigConfigsDataSource-", ApmConfigConfigsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ApmConfigConfigsDataSource().Schema["config_collection"].Elem.(*schema.Resource).Schema)
		config["items"] = items
	}
