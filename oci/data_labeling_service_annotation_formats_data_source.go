...
func init() {
	RegisterDatasource("oci_data_labeling_service_annotation_formats", DataLabelingServiceAnnotationFormatsDataSource())
}
...
func (s *DataLabelingServiceAnnotationFormatsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataLabelingServiceAnnotationFormatsDataSource-", DataLabelingServiceAnnotationFormatsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DataLabelingServiceAnnotationFormatsDataSource().Schema["annotation_format_collection"].Elem.(*schema.Resource).Schema)
		annotationFormat["items"] = items
	}
