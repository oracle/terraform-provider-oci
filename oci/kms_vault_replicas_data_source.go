...
func init() {
	RegisterDatasource("oci_kms_vault_replicas", KmsVaultReplicasDataSource())
}
...
func (s *KmsVaultReplicasDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("KmsVaultReplicasDataSource-", KmsVaultReplicasDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, KmsVaultReplicasDataSource().Schema["vault_replicas"].Elem.(*schema.Resource).Schema)
	}
