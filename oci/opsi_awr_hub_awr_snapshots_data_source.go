...
func init() {
	RegisterDatasource("oci_opsi_awr_hub_awr_snapshots", OpsiAwrHubAwrSnapshotsDataSource())
}
...
func (s *OpsiAwrHubAwrSnapshotsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OpsiAwrHubAwrSnapshotsDataSource-", OpsiAwrHubAwrSnapshotsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OpsiAwrHubAwrSnapshotsDataSource().Schema["awr_snapshot_collection"].Elem.(*schema.Resource).Schema)
		awrHubAwrSnapshot["items"] = items
	}
