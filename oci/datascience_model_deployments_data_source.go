...
func init() {
	RegisterDatasource("oci_datascience_model_deployments", DatascienceModelDeploymentsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"model_deployments": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatascienceModelDeploymentResource()),
			},
		},
	}
}
...
func (s *DatascienceModelDeploymentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatascienceModelDeploymentsDataSource-", DatascienceModelDeploymentsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			modelDeployment["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceModelDeploymentsDataSource().Schema["model_deployments"].Elem.(*schema.Resource).Schema)
	}
