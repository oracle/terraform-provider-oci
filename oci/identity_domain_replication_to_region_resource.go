...
func init() {
	RegisterResource("oci_identity_domain_replication_to_region", IdentityDomainReplicationToRegionResource())
}
...
func (s *IdentityDomainReplicationToRegionResourceCrud) Create() error {
	...
	return s.getDomainReplicationToRegionFromWorkRequest(workId, GetRetryPolicy(s.DisableNotFoundRetries, "domain"), oci_identity.IamWorkRequestResourceActionTypeUpdated, s.D.Timeout(schema.TimeoutCreate))
}
