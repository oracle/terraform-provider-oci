...
func init() {
	RegisterDatasource("oci_data_safe_list_user_grants", DataSafeListUserGrantsDataSource())
}
...
func (s *DataSafeListUserGrantsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataSafeListUserGrantsDataSource-", DataSafeListUserGrantsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataSafeListUserGrantsDataSource().Schema["grants"].Elem.(*schema.Resource).Schema)
	}
