...
func init() {
	RegisterDatasource("oci_artifacts_generic_artifacts", ArtifactsGenericArtifactsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(ArtifactsGenericArtifactResource()),
						},
					},
				},
			},
...
func (s *ArtifactsGenericArtifactsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("ArtifactsGenericArtifactsDataSource-", ArtifactsGenericArtifactsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, ArtifactsGenericArtifactsDataSource().Schema["generic_artifact_collection"].Elem.(*schema.Resource).Schema)
		genericArtifact["items"] = items
	}
