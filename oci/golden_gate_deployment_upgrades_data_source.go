...
func init() {
	RegisterDatasource("oci_golden_gate_deployment_upgrades", GoldenGateDeploymentUpgradesDataSource())
}
...
func (s *GoldenGateDeploymentUpgradesDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("GoldenGateDeploymentUpgradesDataSource-", GoldenGateDeploymentUpgradesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, GoldenGateDeploymentUpgradesDataSource().Schema["deployment_upgrade_collection"].Elem.(*schema.Resource).Schema)
		deploymentUpgrade["items"] = items
	}
