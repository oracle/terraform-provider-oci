...
func init() {
	RegisterDatasource("oci_identity_iam_work_requests", IdentityIamWorkRequestsDataSource())
}
...
func (s *IdentityIamWorkRequestsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("IdentityIamWorkRequestsDataSource-", IdentityIamWorkRequestsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		iamWorkRequestItems = ApplyFilters(f.(*schema.Set), iamWorkRequestItems, IdentityIamWorkRequestsDataSource().Schema["iam_work_requests"].Elem.(*schema.Resource).Schema)
	}
