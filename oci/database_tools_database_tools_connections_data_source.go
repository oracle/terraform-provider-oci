...
func init() {
	RegisterDatasource("oci_database_tools_database_tools_connections", DatabaseToolsDatabaseToolsConnectionsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DatabaseToolsDatabaseToolsConnectionResource()),
						},
					},
				},
			},
...
func (s *DatabaseToolsDatabaseToolsConnectionsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DatabaseToolsDatabaseToolsConnectionsDataSource-", DatabaseToolsDatabaseToolsConnectionsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseToolsDatabaseToolsConnectionsDataSource().Schema["database_tools_connection_collection"].Elem.(*schema.Resource).Schema)
		databaseToolsConnection["items"] = items
	}
