...
func init() {
	RegisterDatasource("oci_data_labeling_service_datasets", DataLabelingServiceDatasetsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DataLabelingServiceDatasetResource()),
						},
					},
				},
			},
...
func (s *DataLabelingServiceDatasetsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("DataLabelingServiceDatasetsDataSource-", DataLabelingServiceDatasetsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DataLabelingServiceDatasetsDataSource().Schema["dataset_collection"].Elem.(*schema.Resource).Schema)
		dataset["items"] = items
	}
