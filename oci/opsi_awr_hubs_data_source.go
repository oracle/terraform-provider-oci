...
func init() {
	RegisterDatasource("oci_opsi_awr_hubs", OpsiAwrHubsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(OpsiAwrHubResource()),
						},
					},
				},
			},
...
func (s *OpsiAwrHubsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("OpsiAwrHubsDataSource-", OpsiAwrHubsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, OpsiAwrHubsDataSource().Schema["awr_hub_summary_collection"].Elem.(*schema.Resource).Schema)
		awrHub["items"] = items
	}
