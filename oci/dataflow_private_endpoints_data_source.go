...
func init() {
	RegisterDatasource("oci_dataflow_private_endpoints", DataflowPrivateEndpointsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(DataflowPrivateEndpointResource()),
						},
					},
				},
			},
...
func (s *DataflowPrivateEndpointsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, DataflowPrivateEndpointsDataSource().Schema["private_endpoint_collection"].Elem.(*schema.Resource).Schema)
		privateEndpoint["items"] = items
	}
