...
func init() {
	RegisterDatasource("oci_waf_web_app_firewalls", WafWebAppFirewallsDataSource())
}
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     GetDataSourceItemSchema(WafWebAppFirewallResource()),
						},
					},
				},
			},
...
func (s *WafWebAppFirewallsDataSourceCrud) SetData() error {
	...
	s.D.SetId(GenerateDataSourceHashID("WafWebAppFirewallsDataSource-", WafWebAppFirewallsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = ApplyFiltersInCollection(f.(*schema.Set), items, WafWebAppFirewallsDataSource().Schema["web_app_firewall_collection"].Elem.(*schema.Resource).Schema)
		webAppFirewall["items"] = items
	}
