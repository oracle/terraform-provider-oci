...
var (
	...
	CoreVnicAttachmentIpv6AutoAssignedRepresentation = acctest.GetRepresentationCopyWithMultipleRemovedProperties(
		[]string{
			"create_vnic_details.ipv6address_ipv6subnet_cidr_pair_details",
		},
		CoreVnicAttachmentIpv6Representation)
...
// issue-routing-tag: core/computeSharedOwnershipVmAndBm
func TestCoreVnicAttachmentResource_AssignIpv6_AutoAssignedPairDetails(t *testing.T) {
	httpreplay.SetScenario("TestCoreVnicAttachmentResource_AssignIpv6_AutoAssignedPairDetails")
	defer httpreplay.SaveScenario()
	...
	acctest.SaveConfigContent(config+compartmentIdVariableStr+CoreVnicAttachmentResourceDependenciesIPV6+
		acctest.GenerateResourceFromRepresentationMap("oci_core_vnic_attachment", "test_vnic_attachment", acctest.Optional, acctest.Create, CoreVnicAttachmentIpv6AutoAssignedRepresentation), "core", "vnicAttachment", t)

	acctest.ResourceTest(t, testAccCheckCoreVnicAttachmentDestroy, []resource.TestStep{
		// verify Create with optionals
		{
			Config: config + compartmentIdVariableStr + CoreVnicAttachmentResourceDependenciesIPV6 +
				acctest.GenerateResourceFromRepresentationMap("oci_core_vnic_attachment", "test_vnic_attachment", acctest.Optional, acctest.Create, CoreVnicAttachmentIpv6AutoAssignedRepresentation) +
				acctest.GenerateDataSourceFromRepresentationMap("oci_core_vnic", "test_vnic", acctest.Required, acctest.Create, CoreSecondaryVnicRepresentation),
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
				resource.TestCheckResourceAttrSet(resourceName, "availability_domain"),
				resource.TestCheckResourceAttrSet(resourceName, "compartment_id"),
