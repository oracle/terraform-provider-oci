...
func (s *ResourceCoreSecurityListTestSuite) TestAccResourceCoreSecurityList_resourceDiscovery_crossCompartment() {
	...
	newConfig := s.Config + `
    	...
    	resource "oci_core_security_list" "sl1" {
            compartment_id = "${var.compartment_id}"
            vcn_id = "${oci_core_virtual_network.vcn1.id}"
        }

    	resource "oci_core_security_list" "sl2" {
            compartment_id = "${var.compartment_id}"
            vcn_id = "${oci_core_virtual_network.vcn2.id}"
        }
        ` + compartmentIdUVariableStr

	resourceName := "oci_core_security_list.sl1"
	resourceName2 := "oci_core_security_list.sl2"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: acctest.ComposeAggregateTestCheckFuncWrapper(
					func(s *terraform.State) (err error) {
						resId, err = acctest.FromInstanceState(s, "oci_core_security_list.sl1", "id")
						resId2, err = acctest.FromInstanceState(s, "oci_core_security_list.sl2", "id")
