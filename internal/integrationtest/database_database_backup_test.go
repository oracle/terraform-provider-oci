...
var (
	databaseBackupUniqueName = fmt.Sprintf("myTestDb_%d", time.Now().Unix()%100000000)
	...
	databaseDatabaseBackupRepresentationCopy = map[string]interface{}{
		...
		"db_unique_name":   acctest.Representation{RepType: acctest.Optional, Create: databaseBackupUniqueName},
	...
	databaseBackupSourceDatabaseRepresentation = acctest.RepresentationCopyWithNewProperties(
		acctest.RepresentationCopyWithRemovedProperties(DatabaseDatabaseRepresentation, []string{"key_store_id", "kms_key_id"}),
		map[string]interface{}{
			"database": acctest.RepresentationGroup{
				RepType: acctest.Required,
				Group:   acctest.RepresentationCopyWithRemovedProperties(databaseDatabaseRepresentation, []string{"db_backup_config"}),
			},
		})
	...
	DatabaseBackupResourceDependencies = DatabaseDatabaseResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_database_database", "db", acctest.Optional, acctest.Create, databaseBackupSourceDatabaseRepresentation) +
	...
	DatabaseBackupResourceDbHomeDependencies = ExaBaseDependencies + DefinedTagsDependencies + AvailabilityDomainConfig + KeyResourceDependencyConfig2 +
		...
		acctest.GenerateResourceFromRepresentationMap("oci_database_database", "db", acctest.Optional, acctest.Create,
			acctest.RepresentationCopyWithRemovedProperties(DatabaseDatabaseBackupRepresentation, []string{"kms_key_id"})) +
		acctest.GenerateResourceFromRepresentationMap("oci_database_backup", "test_backup", acctest.Required, acctest.Create, backupDatabaseRepresentation)
)
		...
		Steps: []resource.TestStep{
			...
			// verify Create with optionals
			{
				...
				Check: acctest.ComposeAggregateTestCheckFuncWrapper(
					...
					resource.TestCheckResourceAttr(resourceName, "db_backup_config.#", "0"),
			...
			// verify datasource
			{
				...
				Check: acctest.ComposeAggregateTestCheckFuncWrapper(
					...
					resource.TestCheckResourceAttr(datasourceName, "databases.0.db_backup_config.#", "0"),
			...
			// verify singular datasource
			{
				...
				Check: acctest.ComposeAggregateTestCheckFuncWrapper(
					...
					resource.TestCheckResourceAttr(singularDatasourceName, "db_backup_config.#", "0"),
