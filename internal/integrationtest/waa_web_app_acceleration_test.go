...
var (
	WebAppAccelerationRequiredOnlyResource = WebAppAccelerationResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Required, acctest.Create, webAppAccelerationRepresentation)

	WebAppAccelerationResourceConfig = WebAppAccelerationResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Optional, acctest.Update, webAppAccelerationRepresentation)

	webAppAccelerationSingularDataSourceRepresentation = map[string]interface{}{
	...
	webAppAccelerationDataSourceRepresentation = map[string]interface{}{
		...
		"filter":                         acctest.RepresentationGroup{RepType: acctest.Required, Group: webAppAccelerationDataSourceFilterRepresentation}}
	webAppAccelerationDataSourceFilterRepresentation = map[string]interface{}{
	...
	webAppAccelerationRepresentation = map[string]interface{}{
	...
	WebAppAccelerationResourceDependencies = acctest.GenerateResourceFromRepresentationMap("oci_load_balancer_load_balancer", "test_load_balancer", acctest.Required, acctest.Create, loadBalancerRepresentation) +
...
func TestWaaWebAppAccelerationResource_basic(t *testing.T) {
	...
	acctest.SaveConfigContent(config+compartmentIdVariableStr+WebAppAccelerationResourceDependencies+
		acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Optional, acctest.Create, webAppAccelerationRepresentation), "waa", "webAppAcceleration", t)

	acctest.ResourceTest(t, testAccCheckWaaWebAppAccelerationDestroy, []resource.TestStep{
		// verify Create
		{
			Config: config + compartmentIdVariableStr + WebAppAccelerationResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Required, acctest.Create, webAppAccelerationRepresentation),
		...
		{
			Config: config + compartmentIdVariableStr + WebAppAccelerationResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Optional, acctest.Create, webAppAccelerationRepresentation),
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
				resource.TestCheckResourceAttr(resourceName, "backend_type", "LOAD_BALANCER"),
				resource.TestCheckResourceAttr(resourceName, "compartment_id", compartmentId),
		...
		{
			Config: config + compartmentIdVariableStr + compartmentIdUVariableStr + WebAppAccelerationResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Optional, acctest.Create,
					acctest.RepresentationCopyWithNewProperties(webAppAccelerationRepresentation, map[string]interface{}{
		...
		{
			Config: config +
				acctest.GenerateDataSourceFromRepresentationMap("oci_waa_web_app_accelerations", "test_web_app_accelerations", acctest.Optional, acctest.Update, webAppAccelerationDataSourceRepresentation) +
				compartmentIdVariableStr + WebAppAccelerationResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Optional, acctest.Update, webAppAccelerationRepresentation),
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
		...
		{
			Config: config +
				acctest.GenerateDataSourceFromRepresentationMap("oci_waa_web_app_acceleration", "test_web_app_acceleration", acctest.Required, acctest.Create, webAppAccelerationSingularDataSourceRepresentation) +
				compartmentIdVariableStr + WebAppAccelerationResourceConfig,
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
		...
		{
			Config:                  config + WebAppAccelerationRequiredOnlyResource,
			ImportState:             true,
...
func sweepWaaWebAppAccelerationResource(compartment string) error {
	...
	webAppAccelerationIds, err := getWebAppAccelerationIds(compartment)
	if err != nil {
		return err
	}
	for _, webAppAccelerationId := range webAppAccelerationIds {
		if ok := acctest.SweeperDefaultResourceId[webAppAccelerationId]; !ok {
			...
			acctest.WaitTillCondition(acctest.TestAccProvider, &webAppAccelerationId, webAppAccelerationSweepWaitCondition, time.Duration(3*time.Minute),
				webAppAccelerationSweepResponseFetchOperation, "waa", true)
		}
	}
	return nil
}

func getWebAppAccelerationIds(compartment string) ([]string, error) {
...
func webAppAccelerationSweepWaitCondition(response common.OCIOperationResponse) bool {
...
func webAppAccelerationSweepResponseFetchOperation(client *tf_client.OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
