...
var (
	peComanagedExadataInsightRequiredOnlyResource = peComanagedExadataInsightResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Required, acctest.Create, peComanagedExadataInsightRepresentation)

	peComanagedExadataInsightResourceConfig = peComanagedExadataInsightResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Update, peComanagedExadataInsightRepresentation)

	peComanagedOpsiExadataInsightSingularDataSourceRepresentation = map[string]interface{}{
	...
	peComanagedOpsiExadataInsightDataSourceRepresentation = map[string]interface{}{
		...
		"filter":                    acctest.RepresentationGroup{RepType: acctest.Required, Group: peComanagedExadataInsightDataSourceFilterRepresentation}}

	peComanagedExadataInsightDataSourceFilterRepresentation = map[string]interface{}{
	...
	peComanagedExadataInsightRepresentation = map[string]interface{}{
		...
		"lifecycle":                 acctest.RepresentationGroup{RepType: acctest.Required, Group: ignoreChangesPeComanagedExadataInsightRepresentation},
		"member_vm_cluster_details": acctest.RepresentationGroup{RepType: acctest.Optional, Group: peComanagedExadataInsightMemberVmClusterDetailsRepresentation},
	}

	peComanagedExadataInsightMemberVmClusterDetailsRepresentation = map[string]interface{}{
		"compartment_id":           acctest.Representation{RepType: acctest.Optional, Create: `${var.compartment_id}`},
		"member_database_details":  acctest.RepresentationGroup{RepType: acctest.Optional, Group: peComanagedExadataInsightMemberVmClusterDetailsMemberDatabaseDetailsRepresentation},
	...
	peComanagedExadataInsightMemberVmClusterDetailsMemberDatabaseDetailsRepresentation = map[string]interface{}{
		"compartment_id":           acctest.Representation{RepType: acctest.Optional, Create: `${var.compartment_id}`},
		"credential_details":       acctest.RepresentationGroup{RepType: acctest.Required, Group: peComanagedExadataInsightCredentialDetailsRepresentation},
	...
	peComanagedExadataInsightCredentialDetailsRepresentation = map[string]interface{}{
	...
	ignoreChangesPeComanagedExadataInsightRepresentation = map[string]interface{}{
		"ignore_changes": acctest.Representation{RepType: acctest.Required, Create: []string{`defined_tags`}},
	}

	peComanagedExadataInsightResourceDependencies = DefinedTagsDependencies
)
...
func TestPeComanagedExadataInsightResource_basic(t *testing.T) {
	...
	envVarsString := compartmentIdVariableStr + compartmentIdUVariableStr + opsiPrivateEndpointIdVariableStr + exadataInfraIdVariableStr + vmClusterIdVariableStr + secretIdVariableStr + userNameVariableStr + dbsystemDatabaseIdVariableStr + serviceNameVariableStr
	...
	acctest.SaveConfigContent(config+envVarsString+peComanagedExadataInsightResourceDependencies+
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create, peComanagedExadataInsightRepresentation), "opsi", "exadataInsight", t)

	acctest.ResourceTest(t, testAccCheckPeComanagedExadataInsightDestroy, []resource.TestStep{
		// verify Create with optionals - Step 0
		{
			Config: config + envVarsString + peComanagedExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create, peComanagedExadataInsightRepresentation),
		...
		// verify update to the compartment (the compartment will be switched back in the next step) - Step 1
		{
			Config: config + envVarsString + peComanagedExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create,
					acctest.RepresentationCopyWithNewProperties(peComanagedExadataInsightRepresentation, map[string]interface{}{
		...
		// verify datasource - Step 4
		{
			Config: config +
				...
				envVarsString + peComanagedExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Update, peComanagedExadataInsightRepresentation),
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
		...
		//verify singular datasource - Step 5
		{
			Config: config +
				...
				envVarsString + peComanagedExadataInsightResourceConfig,
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
		...
		{
			Config:            config + peComanagedExadataInsightRequiredOnlyResource,
			ImportState:       true,
...
func testAccCheckPeComanagedExadataInsightDestroy(s *terraform.State) error {
...
func init() {
	...
	if !acctest.InSweeperExcludeList("PeComanagedExadataInsight") {
		resource.AddTestSweepers("PeComanagedExadataInsight", &resource.Sweeper{
			Name:         "PeComanagedExadataInsight",
			Dependencies: acctest.DependencyGraph["exadataInsight"],
			F:            sweepPeComanagedExadataInsightResource,
		})
	}
}
...
func sweepPeComanagedExadataInsightResource(compartment string) error {
	...
	exadataInsightIds, err := getPeComanagedExadataInsightIds(compartment)
	if err != nil {
		return err
	}
	for _, exadataInsightId := range exadataInsightIds {
		if ok := acctest.SweeperDefaultResourceId[exadataInsightId]; !ok {
			...
			acctest.WaitTillCondition(acctest.TestAccProvider, &exadataInsightId, PeComanagedExadataInsightSweepWaitCondition, time.Duration(3*time.Minute),
				PeComanagedExadataInsightSweepResponseFetchOperation, "opsi", true)
		}
	}
	return nil
}

func getPeComanagedExadataInsightIds(compartment string) ([]string, error) {
...
func PeComanagedExadataInsightSweepWaitCondition(response common.OCIOperationResponse) bool {
...
func PeComanagedExadataInsightSweepResponseFetchOperation(client *tf_client.OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
