...
var (
	FleetAppsManagementInstalledPatchDataSourceRepresentation = map[string]interface{}{
...
func TestFleetAppsManagementInstalledPatchResource_basic(t *testing.T) {
	httpreplay.SetScenario("TestFleetAppsManagementInstalledPatchResource_basic")
	...
	datasourceName := "data.oci_fleet_apps_management_installed_patches.test_installed_patches"
	...
	acctest.ResourceTest(t, nil, []resource.TestStep{
		...
		{
			Config: config +
				acctest.GenerateDataSourceFromRepresentationMap("oci_fleet_apps_management_installed_patches", "test_installed_patches", acctest.Required, acctest.Create, FleetAppsManagementInstalledPatchDataSourceRepresentation) +
				compartmentIdVariableStr,
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
				resource.TestCheckResourceAttrSet(datasourceName, "installed_patch_collection.#"),
				resource.TestCheckResourceAttr(datasourceName, "installed_patch_collection.0.items.#", "0"),
			),
		},
	})
}
