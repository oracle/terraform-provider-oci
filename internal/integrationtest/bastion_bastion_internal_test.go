...
var (
	...
	InternalBastionResourceDependencies = acctest.GenerateResourceFromRepresentationMap("oci_core_subnet", "test_subnet", acctest.Required, acctest.Create, CoreSubnetRepresentation) +
...
func TestBastionBastionResource_internal(t *testing.T) {
	...
	acctest.ResourceTest(t, testAccCheckBastionBastionDestroy, []resource.TestStep{
		...
		{
			Config:                  config + InternalBastionRequiredOnlyResource,
			ImportState:             true,
