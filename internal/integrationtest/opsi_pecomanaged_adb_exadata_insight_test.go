...
var (
	peComanagedAdbExadataInsightRequiredOnlyResource = peComanagedAdbExadataInsightResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Required, acctest.Create, peComanagedAdbExadataInsightRepresentation)

	peComanagedAdbExadataInsightResourceConfig = peComanagedAdbExadataInsightResourceDependencies +
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Update, peComanagedAdbExadataInsightRepresentation)

	peComanagedAdbExadataInsightSingularDataSourceRepresentation = map[string]interface{}{
	...
	peComanagedAdbExadataInsightDataSourceRepresentation = map[string]interface{}{
		...
		"filter":                    acctest.RepresentationGroup{RepType: acctest.Required, Group: peComanagedAdbExadataInsightDataSourceFilterRepresentation}}

	peComanagedAdbExadataInsightDataSourceFilterRepresentation = map[string]interface{}{
	...
	peComanagedAdbExadataInsightRepresentation = map[string]interface{}{
		...
		"lifecycle":                 acctest.RepresentationGroup{RepType: acctest.Required, Group: ignoreChangespeComanagedAdbExadataInsightRepresentation},
		"member_vm_cluster_details": acctest.RepresentationGroup{RepType: acctest.Optional, Group: peComanagedAdbExadataInsightMemberVmClusterDetailsRepresentation},
	}

	peComanagedAdbExadataInsightMemberVmClusterDetailsRepresentation = map[string]interface{}{
		"compartment_id":            acctest.Representation{RepType: acctest.Optional, Create: `${var.compartment_id}`},
		"vm_cluster_type":           acctest.Representation{RepType: acctest.Optional, Create: `autonomousVmCluster`},
		"member_autonomous_details": acctest.RepresentationGroup{RepType: acctest.Optional, Group: peComanagedAdbExadataInsightMemberVmClusterDetailsMemberDatabaseDetailsRepresentation},
	...
	peComanagedAdbExadataInsightMemberVmClusterDetailsMemberDatabaseDetailsRepresentation = map[string]interface{}{
		"compartment_id":               acctest.Representation{RepType: acctest.Optional, Create: `${var.adb_compartment_id}`},
		"credential_details":           acctest.RepresentationGroup{RepType: acctest.Required, Group: peComanagedAdbExadataInsightCredentialDetailsRepresentation},
		"connection_details":           acctest.RepresentationGroup{RepType: acctest.Required, Group: peComanagedAdbExadataInsightConnectionDetailsRepresentation},
		"database_id":                  acctest.Representation{RepType: acctest.Optional, Create: `${var.adb_id}`},
		"database_resource_type":       acctest.Representation{RepType: acctest.Optional, Create: `autonomousdatabase`},
		"entity_source":                acctest.Representation{RepType: acctest.Optional, Create: `AUTONOMOUS_DATABASE`},
	...
	peComanagedAdbExadataInsightCredentialDetailsRepresentation = map[string]interface{}{
	...
	peComanagedAdbExadataInsightConnectionDetailsRepresentation = map[string]interface{}{
	...
	ignoreChangespeComanagedAdbExadataInsightRepresentation = map[string]interface{}{
		"ignore_changes": acctest.Representation{RepType: acctest.Required, Create: []string{`defined_tags`}},
	}

	peComanagedAdbExadataInsightResourceDependencies = DefinedTagsDependencies
)

// issue-routing-tag: opsi/controlPlane
func TestPeComanagedAdbExadataInsightResource_basic(t *testing.T) {
	httpreplay.SetScenario("TestPeComanagedAdbExadataInsightResource_basic")
	...
	adbCompartmentId := utils.GetEnvSettingWithBlankDefault("adb_compartment_ocid")
	adbCompartmentIdVariableStr := fmt.Sprintf("variable \"adb_compartment_id\" { default = \"%s\" }\n", adbCompartmentId)
	...
	adbId := utils.GetEnvSettingWithBlankDefault("adb_id")
	...
	envVarsString := compartmentIdVariableStr + compartmentIdUVariableStr + adbCompartmentIdVariableStr + opsiPrivateEndpointIdVariableStr + exadataInfraIdVariableStr + vmClusterIdVariableStr +
		secretIdVariableStr + userNameVariableStr + adbIdVariableStr + serviceNameVariableStr + adbHostNameVariableStr + adbPortVariableStr
	...
	acctest.SaveConfigContent(config+envVarsString+ //peComanagedAdbExadataInsightResourceDependencies+
		acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create, peComanagedAdbExadataInsightRepresentation), "opsi", "exadataInsight", t)

	acctest.ResourceTest(t, testAccCheckpeComanagedAdbExadataInsightDestroy, []resource.TestStep{
		// verify Create with optionals - Step 0
		{
			Config: config + envVarsString + peComanagedAdbExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create, peComanagedAdbExadataInsightRepresentation),
		...
		// verify update to the compartment (the compartment will be switched back in the next step) - Step 1
		{
			Config: config + envVarsString + peComanagedAdbExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Create,
					acctest.RepresentationCopyWithNewProperties(peComanagedAdbExadataInsightRepresentation, map[string]interface{}{
		...
		{
			Config: config +
				acctest.GenerateDataSourceFromRepresentationMap("oci_opsi_exadata_insights", "test_exadata_insights", acctest.Optional, acctest.Update, peComanagedOpsiExadataInsightDataSourceRepresentation) +
				envVarsString + peComanagedAdbExadataInsightResourceDependencies +
				acctest.GenerateResourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Optional, acctest.Update, peComanagedAdbExadataInsightRepresentation),
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
		...
		{
			Config: config +
				acctest.GenerateDataSourceFromRepresentationMap("oci_opsi_exadata_insight", "test_exadata_insight", acctest.Required, acctest.Create, peComanagedOpsiExadataInsightSingularDataSourceRepresentation) +
		...
		{
			Config:            config + peComanagedAdbExadataInsightRequiredOnlyResource,
			ImportState:       true,
...
func testAccCheckpeComanagedAdbExadataInsightDestroy(s *terraform.State) error {
...
func init() {
	...
	if !acctest.InSweeperExcludeList("peComanagedAdbExadataInsight") {
		resource.AddTestSweepers("peComanagedAdbExadataInsight", &resource.Sweeper{
			Name:         "peComanagedAdbExadataInsight",
			Dependencies: acctest.DependencyGraph["exadataInsight"],
			F:            sweeppeComanagedAdbExadataInsightResource,
		})
	}
}

func sweeppeComanagedAdbExadataInsightResource(compartment string) error {
	operationsInsightsClient := acctest.GetTestClients(&schema.ResourceData{}).OperationsInsightsClient()
	exadataInsightIds, err := getpeComanagedAdbExadataInsightIds(compartment)
	if err != nil {
		return err
	}
	for _, exadataInsightId := range exadataInsightIds {
		if ok := acctest.SweeperDefaultResourceId[exadataInsightId]; !ok {
			...
			acctest.WaitTillCondition(acctest.TestAccProvider, &exadataInsightId, peComanagedAdbExadataInsightSweepWaitCondition, time.Duration(3*time.Minute),
				peComanagedAdbExadataInsightSweepResponseFetchOperation, "opsi", true)
		}
	}
	return nil
}

func getpeComanagedAdbExadataInsightIds(compartment string) ([]string, error) {
...
func peComanagedAdbExadataInsightSweepWaitCondition(response common.OCIOperationResponse) bool {
...
func peComanagedAdbExadataInsightSweepResponseFetchOperation(client *tf_client.OracleClients, resourceId *string, retryPolicy *common.RetryPolicy) error {
