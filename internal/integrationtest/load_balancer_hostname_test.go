...
var (
	HostnameRequiredOnlyResource = acctest.GenerateResourceFromRepresentationMap("oci_load_balancer_hostname", "test_hostname", acctest.Required, acctest.Create, hostnameRepresentation)
...
func TestLoadBalancerHostnameResource_basic(t *testing.T) {
	...
	acctest.ResourceTest(t, testAccCheckLoadBalancerHostnameDestroy, []resource.TestStep{
		...
		{
			Config:            config + HostnameRequiredOnlyResource,
			ImportState:       true,
