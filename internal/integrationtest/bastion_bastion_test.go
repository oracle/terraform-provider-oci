...
var (
	...
	BastionbastionRepresentation = map[string]interface{}{
		...
		"name":                         acctest.Representation{RepType: acctest.Optional, Create: bastionName},
		"security_attributes":          acctest.Representation{RepType: acctest.Optional, Create: map[string]string{"oracle-zpr.sensitivity.value": "42", "oracle-zpr.sensitivity.mode": "enforce"}, Update: map[string]string{"oracle-zpr.sensitivity.value": "updatedValue", "oracle-zpr.sensitivity.mode": "enforce"}},
	}
	...
	acctest.ResourceTest(t, testAccCheckBastionBastionDestroy, []resource.TestStep{
		...
		// verify updates to updatable parameters
		{
			...
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
				...
				resource.TestCheckResourceAttr(resourceName, "security_attributes.%", "2"),
				resource.TestCheckResourceAttr(resourceName, "security_attributes.oracle-zpr.sensitivity.mode", "enforce"),
				resource.TestCheckResourceAttr(resourceName, "security_attributes.oracle-zpr.sensitivity.value", "updatedValue"),
				resource.TestCheckResourceAttrSet(resourceName, "state"),
				resource.TestCheckResourceAttrSet(resourceName, "target_subnet_id"),
				resource.TestCheckResourceAttrSet(resourceName, "target_vcn_id"),
				resource.TestCheckResourceAttrSet(resourceName, "time_created"),
		...
		// verify datasource
		{
			...
			Check: acctest.ComposeAggregateTestCheckFuncWrapper(
				...
				resource.TestCheckResourceAttr(datasourceName, "bastions.0.security_attributes.%", "2"),
