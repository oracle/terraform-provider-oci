...
type ociPluginProvider struct {
	...
	realmSpecificServiceEndpointTemplateEnabled string
	dualStackEndpointEnabled                    string
...
func (p *ociPluginProvider) SetDefaults(config *ociProviderModel) {
	...
	p.realmSpecificServiceEndpointTemplateEnabled = getStringFromFwBool(config.RealmSpecificServiceEndpointTemplateEnabled)
	p.dualStackEndpointEnabled = getStringFromFwBool(config.DualStackEndpointEnabled)
...
func (p *ociPluginProvider) _GetSdkConfigProvider(clients *tf_client.OracleClients) (oci_common.ConfigurationProvider, error) {
	...
	if p.inProcess && auth != strings.ToLower(globalvar.AuthSecurityToken) {
...
func (p *ociPluginProvider) _getConfigProviders() ([]oci_common.ConfigurationProvider, error) {
	...
	case strings.ToLower(globalvar.AuthSecurityToken):
		...
		var err error
		if p.inProcess {
			securityTokenBasedAuthConfigProvider, err = loadFileCredentialSnapshot(defaultPath, profileString, privateKeyPasswordString)
		} else {
