...
func (s *FleetAppsManagementCompliancePolicyRuleResourceCrud) Create() error {
	...
	if patchTypeId, ok := s.D.GetOkExists("patch_type_id"); ok {
		interfaces := patchTypeId.([]interface{})
		...
		if len(tmp) != 0 || s.D.HasChange("patch_type_id") {
	...
	if severity, ok := s.D.GetOkExists("severity"); ok {
		...
		for i := range interfaces {
			if interfaces[i] != nil {
				...
				tmp[i] = oci_fleet_apps_management.ComplianceRuleSeverityEnum(interfaces[i].(string))
			}
		}
...
func CompliancePolicyRuleSummaryToMap(obj oci_fleet_apps_management.CompliancePolicyRuleSummary) map[string]interface{} {
	...
	result["patch_type_id"] = obj.PatchTypeId
