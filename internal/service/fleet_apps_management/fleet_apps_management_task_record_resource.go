...
func ContentDetailsToMap(obj *oci_fleet_apps_management.ContentDetails) map[string]interface{} {
	result := map[string]interface{}{}
	switch v := (*obj).(type) {
	case oci_fleet_apps_management.CatalogContentDetails:
		result["source_type"] = "CATALOG"

		if v.CatalogId != nil {
			result["catalog_id"] = string(*v.CatalogId)
		}
...
func (s *FleetAppsManagementTaskRecordResourceCrud) mapToDetails(fieldKeyFormat string) (oci_fleet_apps_management.Details, error) {
	...
	if operation, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "operation")); ok {
		tmp := operation.(string)
		result.Operation = &tmp
	}
...
func ExecutionDetailsToMap(obj *oci_fleet_apps_management.ExecutionDetails) map[string]interface{} {
	...
	case oci_fleet_apps_management.ScriptBasedExecutionDetails:
		...
		if v.IsExecutableContent != nil {
			result["is_executable_content"] = bool(*v.IsExecutableContent)
		}
	...
	case oci_fleet_apps_management.TerraformBasedExecutionDetails:
		result["execution_type"] = "TERRAFORM"
		...
		if v.ConfigFile != nil {
			result["config_file"] = string(*v.ConfigFile)
		}
...
func (s *FleetAppsManagementTaskRecordResourceCrud) mapToInputArgument(fieldKeyFormat string) (oci_fleet_apps_management.InputArgument, error) {
	...
	// var name_ string
	// if v, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "name")); ok {
	// 	name_ = v.(string)
	// }
	// fmt.Println("Name:", name_)

	// var description string
	// if v, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "description")); ok {
...
func InputArgumentToMap(obj oci_fleet_apps_management.InputArgument) map[string]interface{} {
	...
	case oci_fleet_apps_management.FileInputArgument:
		result["type"] = "FILE"
		if typedObj.GetName() != nil {
			result["name"] = *(typedObj.GetName())
		} else {
			result["name"] = ""
		}
		if typedObj.GetDescription() != nil {
			result["description"] = *(typedObj.GetDescription())
		} else {
			result["description"] = ""
		}
...
// This function has no change with commented verison
...
// This functipn has no change with commented versiom
...
func (s *FleetAppsManagementTaskRecordResourceCrud) updateCompartment(compartment interface{}) error {
	...
	idTmp := s.D.Id()
	changeCompartmentRequest.TaskRecordId = &idTmp
...
//		return result
//	}
// func (s *FleetAppsManagementTaskRecordResourceCrud) mapToInputArgument2(fieldKeyFormat string) (oci_fleet_apps_management.InputArgument, error) {
// 	var baseObject oci_fleet_apps_management.InputArgument
// 	//discriminator
// 	typeRaw, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "type"))
// 	var type_ string
