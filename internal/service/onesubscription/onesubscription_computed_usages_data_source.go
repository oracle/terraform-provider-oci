...
func OnesubscriptionComputedUsagesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readOnesubscriptionComputedUsages,
...
type OnesubscriptionComputedUsagesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_onesubscription.ComputedUsageClient
	Res    *oci_onesubscription.ListComputedUsagesResponse
}
...
func (s *OnesubscriptionComputedUsagesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("OnesubscriptionComputedUsagesDataSource-", OnesubscriptionComputedUsagesDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.ParentProduct != nil {
			computedUsage["parent_product"] = []interface{}{ComputedUsageProductToMap(r.ParentProduct)}
		} else {
		...
		if r.Product != nil {
			computedUsage["product"] = []interface{}{ComputedUsageProductToMap(r.Product)}
		} else {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = tfresource.ApplyFilters(f.(*schema.Set), resources, OnesubscriptionComputedUsagesDataSource().Schema["computed_usages"].Elem.(*schema.Resource).Schema)
	}
...
func ComputedUsageProductToMap(obj *oci_onesubscription.ComputedUsageProduct) map[string]interface{} {
