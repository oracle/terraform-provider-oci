...
func DataintegrationWorkspaceApplicationsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readDataintegrationWorkspaceApplications,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(DataintegrationWorkspaceApplicationResource()),
						},
					},
				},
			},
...
type DataintegrationWorkspaceApplicationsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_dataintegration.DataIntegrationClient
	Res    *oci_dataintegration.ListApplicationsResponse
}
...
func (s *DataintegrationWorkspaceApplicationsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("DataintegrationWorkspaceApplicationsDataSource-", DataintegrationWorkspaceApplicationsDataSource(), s.D))
	resources := []map[string]interface{}{}
	workspaceApplication := map[string]interface{}{}

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, ApplicationSummaryToMap(item))
	}
	workspaceApplication["items"] = items

	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, DataintegrationWorkspaceApplicationsDataSource().Schema["application_summary_collection"].Elem.(*schema.Resource).Schema)
		workspaceApplication["items"] = items
	}

	resources = append(resources, workspaceApplication)
