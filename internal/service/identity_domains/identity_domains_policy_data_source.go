...
func IdentityDomainsPolicyDataSource() *schema.Resource {
	...
	fieldMap["policy_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
	...
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsPolicyResource(), fieldMap, readSingularIdentityDomainsPolicy)
}
...
type IdentityDomainsPolicyDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetPolicyResponse
}
...
func (s *IdentityDomainsPolicyDataSourceCrud) SetData() error {
	...
	if s.Res.PolicyGroovy != nil {
		s.D.Set("policy_groovy", *s.Res.PolicyGroovy)
	}

	if s.Res.PolicyType != nil {
		s.D.Set("policy_type", []interface{}{PolicyPolicyTypeToMap(s.Res.PolicyType)})
	} else {
	...
	for _, item := range s.Res.Rules {
		rules = append(rules, PolicyRulesToMap(item))
	}
	s.D.Set("rules", rules)
