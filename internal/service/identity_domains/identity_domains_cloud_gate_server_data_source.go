...
func IdentityDomainsCloudGateServerDataSource() *schema.Resource {
	...
	fieldMap["cloud_gate_server_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
	...
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsCloudGateServerResource(), fieldMap, readSingularIdentityDomainsCloudGateServer)
}
...
type IdentityDomainsCloudGateServerDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetCloudGateServerResponse
}
...
func (s *IdentityDomainsCloudGateServerDataSourceCrud) Get() error {
	...
	if cloudGateServerId, ok := s.D.GetOkExists("cloud_gate_server_id"); ok {
		tmp := cloudGateServerId.(string)
