...
func IdentityDomainsSecurityQuestionSettingsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsSecurityQuestionSettings,
		Schema: map[string]*schema.Schema{
			...
			"security_question_settings": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsSecurityQuestionSettingResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsSecurityQuestionSettingsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListSecurityQuestionSettingsResponse
}
...
func (s *IdentityDomainsSecurityQuestionSettingsDataSourceCrud) Get() error {
	...
	response, err := s.Client.ListSecurityQuestionSettings(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsSecurityQuestionSettingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsSecurityQuestionSettingsDataSource-", IdentityDomainsSecurityQuestionSettingsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, SecurityQuestionSettingToMap(item))
	}
	s.D.Set("security_question_settings", resources)
