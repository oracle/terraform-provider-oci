...
func IdentityDomainsIdentityProvidersDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsIdentityProviders,
		Schema: map[string]*schema.Schema{
			...
			"identity_providers": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsIdentityProviderResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsIdentityProvidersDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListIdentityProvidersResponse
}
...
func (s *IdentityDomainsIdentityProvidersDataSourceCrud) Get() error {
	...
	if identityProviderCount, ok := s.D.GetOkExists("identity_provider_count"); ok {
		tmp := identityProviderCount.(int)
		request.Count = &tmp
	}

	if identityProviderFilter, ok := s.D.GetOkExists("identity_provider_filter"); ok {
		tmp := identityProviderFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListIdentityProvidersSortOrderEnum(sortOrder.(string))
...
func (s *IdentityDomainsIdentityProvidersDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsIdentityProvidersDataSource-", IdentityDomainsIdentityProvidersDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, IdentityProviderToMap(item))
	}
	s.D.Set("identity_providers", resources)
