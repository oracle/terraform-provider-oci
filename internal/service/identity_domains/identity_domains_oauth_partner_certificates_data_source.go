...
func IdentityDomainsOAuthPartnerCertificatesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsOAuthPartnerCertificates,
		Schema: map[string]*schema.Schema{
			...
			"oauth_partner_certificates": {
				...
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsOAuthPartnerCertificateResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsOAuthPartnerCertificatesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListOAuthPartnerCertificatesResponse
}
...
func (s *IdentityDomainsOAuthPartnerCertificatesDataSourceCrud) Get() error {
	...
	if oAuthPartnerCertificateCount, ok := s.D.GetOkExists("oauth_partner_certificate_count"); ok {
		tmp := oAuthPartnerCertificateCount.(int)
		request.Count = &tmp
	}

	if oAuthPartnerCertificateFilter, ok := s.D.GetOkExists("oauth_partner_certificate_filter"); ok {
		tmp := oAuthPartnerCertificateFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListOAuthPartnerCertificatesSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListOAuthPartnerCertificates(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListOAuthPartnerCertificates(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsOAuthPartnerCertificatesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsOAuthPartnerCertificatesDataSource-", IdentityDomainsOAuthPartnerCertificatesDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, OAuthPartnerCertificateToMap(item))
	}
	s.D.Set("oauth_partner_certificates", resources)
