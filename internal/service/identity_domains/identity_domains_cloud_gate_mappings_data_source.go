...
func IdentityDomainsCloudGateMappingsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsCloudGateMappings,
		Schema: map[string]*schema.Schema{
			...
			"cloud_gate_mappings": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsCloudGateMappingResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsCloudGateMappingsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListCloudGateMappingsResponse
}
...
func (s *IdentityDomainsCloudGateMappingsDataSourceCrud) Get() error {
	...
	if cloudGateMappingCount, ok := s.D.GetOkExists("cloud_gate_mapping_count"); ok {
		tmp := cloudGateMappingCount.(int)
		request.Count = &tmp
	}

	if cloudGateMappingFilter, ok := s.D.GetOkExists("cloud_gate_mapping_filter"); ok {
		tmp := cloudGateMappingFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListCloudGateMappingsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListCloudGateMappings(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListCloudGateMappings(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsCloudGateMappingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsCloudGateMappingsDataSource-", IdentityDomainsCloudGateMappingsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, CloudGateMappingToMap(item))
	}
	s.D.Set("cloud_gate_mappings", resources)
