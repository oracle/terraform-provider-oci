...
func IdentityDomainsApprovalWorkflowAssignmentResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsApprovalWorkflowAssignment,
		Read:     readIdentityDomainsApprovalWorkflowAssignment,
		Delete:   deleteIdentityDomainsApprovalWorkflowAssignment,
...
func readIdentityDomainsApprovalWorkflowAssignment(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsApprovalWorkflowAssignmentResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "approvalWorkflowAssignments")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsApprovalWorkflowAssignmentResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.ApprovalWorkflowAssignment
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) Create() error {
	...
	if approvalWorkflow, ok := s.D.GetOkExists("approval_workflow"); ok {
		if tmpList := approvalWorkflow.([]interface{}); len(tmpList) > 0 {
			fieldKeyFormat := fmt.Sprintf("%s.%d.%%s", "approval_workflow", 0)
			tmp, err := s.mapToApprovalWorkflowAssignmentApprovalWorkflow(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.ApprovalWorkflow = &tmp
		}
	}

	if assignedTo, ok := s.D.GetOkExists("assigned_to"); ok {
		if tmpList := assignedTo.([]interface{}); len(tmpList) > 0 {
			fieldKeyFormat := fmt.Sprintf("%s.%d.%%s", "assigned_to", 0)
			tmp, err := s.mapToApprovalWorkflowAssignmentAssignedTo(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.AssignedTo = &tmp
		}
	}

	if assignmentType, ok := s.D.GetOkExists("assignment_type"); ok {
		request.AssignmentType = oci_identity_domains.ApprovalWorkflowAssignmentAssignmentTypeEnum(assignmentType.(string))
	}
	...
	response, err := s.Client.CreateApprovalWorkflowAssignment(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.ApprovalWorkflowAssignment
	return nil
}
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) Get() error {
	...
	approvalWorkflowAssignmentId, err := parseApprovalWorkflowAssignmentCompositeId(s.D.Id())
	if err == nil {
		request.ApprovalWorkflowAssignmentId = &approvalWorkflowAssignmentId
	} else {
	...
	response, err := s.Client.GetApprovalWorkflowAssignment(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteApprovalWorkflowAssignment(context.Background(), request)
	return err
}
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) SetData() error {
	...
	approvalWorkflowAssignmentId, err := parseApprovalWorkflowAssignmentCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(approvalWorkflowAssignmentId)
	} else {
		log.Printf("[WARN] SetData() unable to parse current ID: %s", s.D.Id())
	}

	if s.Res.ApprovalWorkflow != nil {
		s.D.Set("approval_workflow", []interface{}{ApprovalWorkflowAssignmentApprovalWorkflowToMap(s.Res.ApprovalWorkflow, s)})
	} else {
		s.D.Set("approval_workflow", nil)
	}

	if s.Res.AssignedTo != nil {
		s.D.Set("assigned_to", []interface{}{ApprovalWorkflowAssignmentAssignedToToMap(s.Res.AssignedTo)})
	} else {
		s.D.Set("assigned_to", nil)
	}

	s.D.Set("assignment_type", s.Res.AssignmentType)
...
func parseApprovalWorkflowAssignmentCompositeId(compositeId string) (approvalWorkflowAssignmentId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/approvalWorkflowAssignments/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}

	approvalWorkflowAssignmentId, _ = url.PathUnescape(parts[3])

	return
}

func ApprovalWorkflowAssignmentToMap(obj oci_identity_domains.ApprovalWorkflowAssignment) map[string]interface{} {
	result := map[string]interface{}{}

	if obj.ApprovalWorkflow != nil {
		result["approval_workflow"] = []interface{}{ApprovalWorkflowAssignmentApprovalWorkflowToMap(obj.ApprovalWorkflow, nil)}
	}

	if obj.AssignedTo != nil {
		result["assigned_to"] = []interface{}{ApprovalWorkflowAssignmentAssignedToToMap(obj.AssignedTo)}
	}

	result["assignment_type"] = string(obj.AssignmentType)
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) mapToApprovalWorkflowAssignmentApprovalWorkflow(fieldKeyFormat string) (oci_identity_domains.ApprovalWorkflowAssignmentApprovalWorkflow, error) {
	...
	if type_, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "type")); ok {
		result.Type = oci_identity_domains.ApprovalWorkflowAssignmentApprovalWorkflowTypeEnum(type_.(string))
	}
...
func ApprovalWorkflowAssignmentApprovalWorkflowToMap(obj *oci_identity_domains.ApprovalWorkflowAssignmentApprovalWorkflow, s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) map[string]interface{} {
	...
	// `type` is not returned from the API, set it to the value in the configuration
	if s != nil {
		fieldKeyFormat := fmt.Sprintf("%s.%d.%%s", "approval_workflow", 0)
		if type_, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "type")); ok {
			result["type"] = oci_identity_domains.ApprovalWorkflowAssignmentApprovalWorkflowTypeEnum(type_.(string))
...
func (s *IdentityDomainsApprovalWorkflowAssignmentResourceCrud) mapToApprovalWorkflowAssignmentAssignedTo(fieldKeyFormat string) (oci_identity_domains.ApprovalWorkflowAssignmentAssignedTo, error) {
	...
	if type_, ok := s.D.GetOkExists(fmt.Sprintf(fieldKeyFormat, "type")); ok {
		result.Type = oci_identity_domains.ApprovalWorkflowAssignmentAssignedToTypeEnum(type_.(string))
	}
...
func ApprovalWorkflowAssignmentAssignedToToMap(obj *oci_identity_domains.ApprovalWorkflowAssignmentAssignedTo) map[string]interface{} {
	result := map[string]interface{}{}
