...
func IdentityDomainsConditionsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsConditions,
		Schema: map[string]*schema.Schema{
			...
			"conditions": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsConditionResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsConditionsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListConditionsResponse
}
...
func (s *IdentityDomainsConditionsDataSourceCrud) Get() error {
	...
	if conditionCount, ok := s.D.GetOkExists("condition_count"); ok {
		tmp := conditionCount.(int)
		request.Count = &tmp
	}

	if conditionFilter, ok := s.D.GetOkExists("condition_filter"); ok {
		tmp := conditionFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListConditionsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListConditions(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListConditions(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsConditionsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsConditionsDataSource-", IdentityDomainsConditionsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, ConditionToMap(item))
	}
	s.D.Set("conditions", resources)
