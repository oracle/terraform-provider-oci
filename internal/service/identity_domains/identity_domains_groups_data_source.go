...
func IdentityDomainsGroupsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsGroups,
		Schema: map[string]*schema.Schema{
			...
			"groups": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsGroupResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsGroupsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListGroupsResponse
}
...
func (s *IdentityDomainsGroupsDataSourceCrud) Get() error {
	...
	if groupCount, ok := s.D.GetOkExists("group_count"); ok {
		tmp := groupCount.(int)
		request.Count = &tmp
	}

	if groupFilter, ok := s.D.GetOkExists("group_filter"); ok {
		tmp := groupFilter.(string)
		request.Filter = &tmp
	}
...
func (s *IdentityDomainsGroupsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsGroupsDataSource-", IdentityDomainsGroupsDataSource(), s.D))
