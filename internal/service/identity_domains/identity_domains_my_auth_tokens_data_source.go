...
func IdentityDomainsMyAuthTokensDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsMyAuthTokens,
		Schema: map[string]*schema.Schema{
			...
			"my_auth_tokens": {
				...
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsMyAuthTokenResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsMyAuthTokensDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListMyAuthTokensResponse
}
...
func (s *IdentityDomainsMyAuthTokensDataSourceCrud) Get() error {
	...
	if myAuthTokenCount, ok := s.D.GetOkExists("my_auth_token_count"); ok {
		tmp := myAuthTokenCount.(int)
		request.Count = &tmp
	}

	if myAuthTokenFilter, ok := s.D.GetOkExists("my_auth_token_filter"); ok {
		tmp := myAuthTokenFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListMyAuthTokensSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListMyAuthTokens(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListMyAuthTokens(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsMyAuthTokensDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsMyAuthTokensDataSource-", IdentityDomainsMyAuthTokensDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, MyAuthTokenToMap(item))
	}
	s.D.Set("my_auth_tokens", resources)
