...
func IdentityDomainsOAuthClientCertificateResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsOAuthClientCertificate,
		Read:     readIdentityDomainsOAuthClientCertificate,
		Delete:   deleteIdentityDomainsOAuthClientCertificate,
...
func readIdentityDomainsOAuthClientCertificate(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsOAuthClientCertificateResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "oauthClientCertificates")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsOAuthClientCertificateResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.OAuthClientCertificate
...
func (s *IdentityDomainsOAuthClientCertificateResourceCrud) Create() error {
	...
	response, err := s.Client.CreateOAuthClientCertificate(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.OAuthClientCertificate
	return nil
}
...
func (s *IdentityDomainsOAuthClientCertificateResourceCrud) Get() error {
	...
	oAuthClientCertificateId, err := parseOAuthClientCertificateCompositeId(s.D.Id())
	if err == nil {
	...
	response, err := s.Client.GetOAuthClientCertificate(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsOAuthClientCertificateResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteOAuthClientCertificate(context.Background(), request)
	return err
}
...
func (s *IdentityDomainsOAuthClientCertificateResourceCrud) SetData() error {
	...
	oAuthClientCertificateId, err := parseOAuthClientCertificateCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(oAuthClientCertificateId)
	} else {
...
func parseOAuthClientCertificateCompositeId(compositeId string) (oAuthClientCertificateId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/oAuthClientCertificates/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	oAuthClientCertificateId, _ = url.PathUnescape(parts[3])

	return
}

func OAuthClientCertificateToMap(obj oci_identity_domains.OAuthClientCertificate) map[string]interface{} {
