...
func IdentityDomainsSmtpCredentialsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsSmtpCredentials,
		Schema: map[string]*schema.Schema{
			...
			"smtp_credentials": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsSmtpCredentialResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsSmtpCredentialsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListSmtpCredentialsResponse
}
...
func (s *IdentityDomainsSmtpCredentialsDataSourceCrud) Get() error {
	...
	if smtpCredentialCount, ok := s.D.GetOkExists("smtp_credential_count"); ok {
		tmp := smtpCredentialCount.(int)
		request.Count = &tmp
	}

	if smtpCredentialFilter, ok := s.D.GetOkExists("smtp_credential_filter"); ok {
		tmp := smtpCredentialFilter.(string)
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListSmtpCredentialsSortOrderEnum(sortOrder.(string))
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListSmtpCredentials(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsSmtpCredentialsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsSmtpCredentialsDataSource-", IdentityDomainsSmtpCredentialsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, SmtpCredentialToMap(item))
	}
	s.D.Set("smtp_credentials", resources)
