...
func IdentityDomainsUserDbCredentialDataSource() *schema.Resource {
	...
	fieldMap["user_db_credential_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsUserDbCredentialResource(), fieldMap, readSingularIdentityDomainsUserDbCredential)
}
...
type IdentityDomainsUserDbCredentialDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetUserDbCredentialResponse
}
...
func (s *IdentityDomainsUserDbCredentialDataSourceCrud) Get() error {
	...
	if userDbCredentialId, ok := s.D.GetOkExists("user_db_credential_id"); ok {
		tmp := userDbCredentialId.(string)
