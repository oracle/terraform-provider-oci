...
func IdentityDomainsPoliciesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsPolicies,
		Schema: map[string]*schema.Schema{
			...
			"policies": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsPolicyResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsPoliciesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListPoliciesResponse
}
...
func (s *IdentityDomainsPoliciesDataSourceCrud) Get() error {
	...
	if policyCount, ok := s.D.GetOkExists("policy_count"); ok {
		tmp := policyCount.(int)
		request.Count = &tmp
	}

	if policyFilter, ok := s.D.GetOkExists("policy_filter"); ok {
		tmp := policyFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListPoliciesSortOrderEnum(sortOrder.(string))
...
func (s *IdentityDomainsPoliciesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsPoliciesDataSource-", IdentityDomainsPoliciesDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, PolicyToMap(item))
	}
