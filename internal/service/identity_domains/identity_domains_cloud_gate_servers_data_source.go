...
func IdentityDomainsCloudGateServersDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsCloudGateServers,
		Schema: map[string]*schema.Schema{
			...
			"cloud_gate_servers": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsCloudGateServerResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsCloudGateServersDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListCloudGateServersResponse
}
...
func (s *IdentityDomainsCloudGateServersDataSourceCrud) Get() error {
	...
	if cloudGateServerCount, ok := s.D.GetOkExists("cloud_gate_server_count"); ok {
		tmp := cloudGateServerCount.(int)
		request.Count = &tmp
	}

	if cloudGateServerFilter, ok := s.D.GetOkExists("cloud_gate_server_filter"); ok {
		tmp := cloudGateServerFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListCloudGateServersSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListCloudGateServers(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListCloudGateServers(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsCloudGateServersDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsCloudGateServersDataSource-", IdentityDomainsCloudGateServersDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, CloudGateServerToMap(item))
	}
	s.D.Set("cloud_gate_servers", resources)
