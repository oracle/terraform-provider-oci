...
func IdentityDomainsMyCompletedApprovalDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readSingularIdentityDomainsMyCompletedApproval,
...
type IdentityDomainsMyCompletedApprovalDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetMyCompletedApprovalResponse
}
...
func (s *IdentityDomainsMyCompletedApprovalDataSourceCrud) Get() error {
	...
	if myCompletedApprovalId, ok := s.D.GetOkExists("my_completed_approval_id"); ok {
		tmp := myCompletedApprovalId.(string)
		request.MyCompletedApprovalId = &tmp
	}
	...
	response, err := s.Client.GetMyCompletedApproval(context.Background(), request)
	if err != nil {
		return err
	}
