...
func IdentityDomainsSecurityQuestionDataSource() *schema.Resource {
	...
	fieldMap["security_question_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Optional: true,
	}
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsSecurityQuestionResource(), fieldMap, readSingularIdentityDomainsSecurityQuestion)
}
...
type IdentityDomainsSecurityQuestionDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetSecurityQuestionResponse
}
...
func (s *IdentityDomainsSecurityQuestionDataSourceCrud) Get() error {
	...
	if securityQuestionId, ok := s.D.GetOkExists("security_question_id"); ok {
		tmp := securityQuestionId.(string)
