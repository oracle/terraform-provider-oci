...
func IdentityDomainsMyApiKeyResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsMyApiKey,
		Read:     readIdentityDomainsMyApiKey,
		Delete:   deleteIdentityDomainsMyApiKey,
...
func readIdentityDomainsMyApiKey(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsMyApiKeyResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "myApiKeys")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsMyApiKeyResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.MyApiKey
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsMyApiKeyResourceCrud) ID() string {
	return *s.Res.Id
	//return GetMyApiKeyCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsMyApiKeyResourceCrud) Create() error {
	...
	if user, ok := s.D.GetOkExists("user"); ok {
		if tmpList := user.([]interface{}); len(tmpList) > 0 {
			...
			tmp, err := s.mapToMyApiKeyUser(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.User = &tmp
	...
	response, err := s.Client.CreateMyApiKey(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.MyApiKey
	return nil
}
...
func (s *IdentityDomainsMyApiKeyResourceCrud) Get() error {
	...
	myApiKeyId, err := parseMyApiKeyCompositeId(s.D.Id())
	if err == nil {
...
func (s *IdentityDomainsMyApiKeyResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteMyApiKey(context.Background(), request)
	return err
}
...
//func GetMyApiKeyCompositeId(myApiKeyId string) string {
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	myApiKeyId = url.PathEscape(myApiKeyId)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/myApiKeys/" + myApiKeyId
//	return compositeId
//}

func parseMyApiKeyCompositeId(compositeId string) (myApiKeyId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/myApiKeys/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	//idcsEndpoint, _ = url.PathUnescape(parts[1])
	myApiKeyId, _ = url.PathUnescape(parts[3])

	return
}

func MyApiKeyToMap(obj oci_identity_domains.MyApiKey) map[string]interface{} {
	...
	if obj.User != nil {
		result["user"] = []interface{}{MyApiKeyUserToMap(obj.User)}
	}
...
func MyApiKeyUserToMap(obj *oci_identity_domains.MyApiKeyUser) map[string]interface{} {
	result := map[string]interface{}{}
