...
func IdentityDomainsMySmtpCredentialResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsMySmtpCredential,
		Read:     readIdentityDomainsMySmtpCredential,
		Delete:   deleteIdentityDomainsMySmtpCredential,
...
func readIdentityDomainsMySmtpCredential(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsMySmtpCredentialResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "mySmtpCredentials")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsMySmtpCredentialResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.MySmtpCredential
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsMySmtpCredentialResourceCrud) ID() string {
	return *s.Res.Id
	//return GetMySmtpCredentialCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsMySmtpCredentialResourceCrud) Create() error {
	...
	if status, ok := s.D.GetOkExists("status"); ok {
		request.Status = oci_identity_domains.MySmtpCredentialStatusEnum(status.(string))
	}
	...
	if user, ok := s.D.GetOkExists("user"); ok {
		if tmpList := user.([]interface{}); len(tmpList) > 0 {
			...
			tmp, err := s.mapToMySmtpCredentialUser(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.User = &tmp
	...
	response, err := s.Client.CreateMySmtpCredential(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.MySmtpCredential
	return nil
}
...
func (s *IdentityDomainsMySmtpCredentialResourceCrud) Get() error {
	...
	mySmtpCredentialId, err := parseMySmtpCredentialCompositeId(s.D.Id())
	if err == nil {
	...
	response, err := s.Client.GetMySmtpCredential(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsMySmtpCredentialResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteMySmtpCredential(context.Background(), request)
	return err
}
...
func (s *IdentityDomainsMySmtpCredentialResourceCrud) SetData() error {
	...
	if s.Res.User != nil {
		s.D.Set("user", []interface{}{MySmtpCredentialUserToMap(s.Res.User)})
	} else {
		s.D.Set("user", nil)
	}
...
//func GetMySmtpCredentialCompositeId(mySmtpCredentialId string) string {
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	mySmtpCredentialId = url.PathEscape(mySmtpCredentialId)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/mySmtpCredentials/" + mySmtpCredentialId
//	return compositeId
//}

func parseMySmtpCredentialCompositeId(compositeId string) (mySmtpCredentialId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/mySmtpCredentials/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	//idcsEndpoint, _ = url.PathUnescape(parts[1])
	mySmtpCredentialId, _ = url.PathUnescape(parts[3])

	return
}

func MySmtpCredentialToMap(obj oci_identity_domains.MySmtpCredential) map[string]interface{} {
	...
	if obj.User != nil {
		result["user"] = []interface{}{MySmtpCredentialUserToMap(obj.User)}
	}
...
func MySmtpCredentialUserToMap(obj *oci_identity_domains.MySmtpCredentialUser) map[string]interface{} {
	result := map[string]interface{}{}
