...
func IdentityDomainsMyUserDbCredentialsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsMyUserDbCredentials,
		Schema: map[string]*schema.Schema{
			...
			"my_user_db_credentials": {
				...
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsMyUserDbCredentialResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsMyUserDbCredentialsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListMyUserDbCredentialsResponse
}
...
func (s *IdentityDomainsMyUserDbCredentialsDataSourceCrud) Get() error {
	...
	if myUserDbCredentialCount, ok := s.D.GetOkExists("my_user_db_credential_count"); ok {
		tmp := myUserDbCredentialCount.(int)
		request.Count = &tmp
	}

	if myUserDbCredentialFilter, ok := s.D.GetOkExists("my_user_db_credential_filter"); ok {
		tmp := myUserDbCredentialFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListMyUserDbCredentialsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListMyUserDbCredentials(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListMyUserDbCredentials(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsMyUserDbCredentialsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsMyUserDbCredentialsDataSource-", IdentityDomainsMyUserDbCredentialsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, MyUserDbCredentialToMap(item))
	}
	s.D.Set("my_user_db_credentials", resources)
