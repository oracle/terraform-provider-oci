...
func IdentityDomainsUserDbCredentialsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsUserDbCredentials,
		Schema: map[string]*schema.Schema{
			...
			"user_db_credentials": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsUserDbCredentialResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsUserDbCredentialsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListUserDbCredentialsResponse
}
...
func (s *IdentityDomainsUserDbCredentialsDataSourceCrud) Get() error {
	...
	if userDbCredentialCount, ok := s.D.GetOkExists("user_db_credential_count"); ok {
		tmp := userDbCredentialCount.(int)
		request.Count = &tmp
	}

	if userDbCredentialFilter, ok := s.D.GetOkExists("user_db_credential_filter"); ok {
		tmp := userDbCredentialFilter.(string)
		tmp = tryMatchUserSubResFilter(tmp)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListUserDbCredentialsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListUserDbCredentials(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListUserDbCredentials(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsUserDbCredentialsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsUserDbCredentialsDataSource-", IdentityDomainsUserDbCredentialsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, UserDbCredentialToMap(item))
	}
	s.D.Set("user_db_credentials", resources)
