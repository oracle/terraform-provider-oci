...
func IdentityDomainsApiKeysDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsApiKeys,
		Schema: map[string]*schema.Schema{
			...
			"api_keys": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsApiKeyResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsApiKeysDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListApiKeysResponse
}
...
func (s *IdentityDomainsApiKeysDataSourceCrud) Get() error {
	...
	if apiKeyCount, ok := s.D.GetOkExists("api_key_count"); ok {
		tmp := apiKeyCount.(int)
		request.Count = &tmp
	}

	if apiKeyFilter, ok := s.D.GetOkExists("api_key_filter"); ok {
		tmp := apiKeyFilter.(string)
		// additional step to try match user sub-resources' pattern
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListApiKeysSortOrderEnum(sortOrder.(string))
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListApiKeys(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsApiKeysDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsApiKeysDataSource-", IdentityDomainsApiKeysDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, ApiKeyToMap(item))
	}
