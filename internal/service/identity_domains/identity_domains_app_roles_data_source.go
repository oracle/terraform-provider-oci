...
func IdentityDomainsAppRolesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsAppRoles,
		Schema: map[string]*schema.Schema{
			...
			"app_roles": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsAppRoleResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsAppRolesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListAppRolesResponse
}
...
func (s *IdentityDomainsAppRolesDataSourceCrud) Get() error {
	...
	if appRoleCount, ok := s.D.GetOkExists("app_role_count"); ok {
		tmp := appRoleCount.(int)
		request.Count = &tmp
	}

	if appRoleFilter, ok := s.D.GetOkExists("app_role_filter"); ok {
		tmp := appRoleFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListAppRolesSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListAppRoles(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListAppRoles(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsAppRolesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsAppRolesDataSource-", IdentityDomainsAppRolesDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, AppRoleToMap(item))
	}
	s.D.Set("app_roles", resources)
