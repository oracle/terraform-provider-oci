...
func IdentityDomainsNetworkPerimeterDataSource() *schema.Resource {
	...
	fieldMap["network_perimeter_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
	...
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsNetworkPerimeterResource(), fieldMap, readSingularIdentityDomainsNetworkPerimeter)
}
...
type IdentityDomainsNetworkPerimeterDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetNetworkPerimeterResponse
}
...
func (s *IdentityDomainsNetworkPerimeterDataSourceCrud) Get() error {
	...
	if networkPerimeterId, ok := s.D.GetOkExists("network_perimeter_id"); ok {
		tmp := networkPerimeterId.(string)
