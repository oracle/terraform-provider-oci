...
func IdentityDomainsSecurityQuestionSettingResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsSecurityQuestionSetting,
		Read:     readIdentityDomainsSecurityQuestionSetting,
		Update:   updateIdentityDomainsSecurityQuestionSetting,
		Delete:   deleteIdentityDomainsSecurityQuestionSetting,
...
func readIdentityDomainsSecurityQuestionSetting(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsSecurityQuestionSettingResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "securityQuestionSettings")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsSecurityQuestionSettingResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.SecurityQuestionSetting
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsSecurityQuestionSettingResourceCrud) ID() string {
	return *s.Res.Id
	//return GetSecurityQuestionSettingCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsSecurityQuestionSettingResourceCrud) Create() error {
	...
	if maxFieldLength, ok := s.D.GetOkExists("max_field_length"); ok {
		tmp := maxFieldLength.(int)
		request.MaxFieldLength = &tmp
	}

	if minAnswerLength, ok := s.D.GetOkExists("min_answer_length"); ok {
		tmp := minAnswerLength.(int)
		request.MinAnswerLength = &tmp
	}

	if numQuestionsToAns, ok := s.D.GetOkExists("num_questions_to_ans"); ok {
		tmp := numQuestionsToAns.(int)
		request.NumQuestionsToAns = &tmp
	}

	if numQuestionsToSetup, ok := s.D.GetOkExists("num_questions_to_setup"); ok {
		tmp := numQuestionsToSetup.(int)
		request.NumQuestionsToSetup = &tmp
	}
	...
	response, err := s.Client.PutSecurityQuestionSetting(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.SecurityQuestionSetting
	return nil
}
...
func (s *IdentityDomainsSecurityQuestionSettingResourceCrud) Get() error {
	...
	securityQuestionSettingId, err := parseSecurityQuestionSettingCompositeId(s.D.Id())
	if err == nil {
...
func (s *IdentityDomainsSecurityQuestionSettingResourceCrud) SetData() error {
	...
	securityQuestionSettingId, err := parseSecurityQuestionSettingCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(securityQuestionSettingId)
	} else {
...
//func GetSecurityQuestionSettingCompositeId(securityQuestionSettingId string) string {
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	securityQuestionSettingId = url.PathEscape(securityQuestionSettingId)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/securityQuestionSettings/" + securityQuestionSettingId
//	return compositeId
//}

func parseSecurityQuestionSettingCompositeId(compositeId string) (securityQuestionSettingId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/securityQuestionSettings/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}

	securityQuestionSettingId, _ = url.PathUnescape(parts[3])

	return
}

func SecurityQuestionSettingToMap(obj oci_identity_domains.SecurityQuestionSetting) map[string]interface{} {
	...
	if obj.MaxFieldLength != nil {
		result["max_field_length"] = int(*obj.MaxFieldLength)
	}
	...
	if obj.MinAnswerLength != nil {
		result["min_answer_length"] = int(*obj.MinAnswerLength)
	}

	if obj.NumQuestionsToAns != nil {
		result["num_questions_to_ans"] = int(*obj.NumQuestionsToAns)
	}

	if obj.NumQuestionsToSetup != nil {
		result["num_questions_to_setup"] = int(*obj.NumQuestionsToSetup)
	}
