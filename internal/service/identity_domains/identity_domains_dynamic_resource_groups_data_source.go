...
func IdentityDomainsDynamicResourceGroupsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsDynamicResourceGroups,
		Schema: map[string]*schema.Schema{
			...
			"dynamic_resource_groups": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsDynamicResourceGroupResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsDynamicResourceGroupsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListDynamicResourceGroupsResponse
}
...
func (s *IdentityDomainsDynamicResourceGroupsDataSourceCrud) Get() error {
	...
	if dynamicResourceGroupCount, ok := s.D.GetOkExists("dynamic_resource_group_count"); ok {
		tmp := dynamicResourceGroupCount.(int)
		request.Count = &tmp
	}

	if dynamicResourceGroupFilter, ok := s.D.GetOkExists("dynamic_resource_group_filter"); ok {
		tmp := dynamicResourceGroupFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListDynamicResourceGroupsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListDynamicResourceGroups(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListDynamicResourceGroups(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsDynamicResourceGroupsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsDynamicResourceGroupsDataSource-", IdentityDomainsDynamicResourceGroupsDataSource(), s.D))
