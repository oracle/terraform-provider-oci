...
func IdentityDomainsAccountRecoverySettingResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsAccountRecoverySetting,
		Read:     readIdentityDomainsAccountRecoverySetting,
		Update:   updateIdentityDomainsAccountRecoverySetting,
		Delete:   deleteIdentityDomainsAccountRecoverySetting,
...
func readIdentityDomainsAccountRecoverySetting(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsAccountRecoverySettingResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "accountRecoverySettings")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsAccountRecoverySettingResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.AccountRecoverySetting
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsAccountRecoverySettingResourceCrud) ID() string {
	return *s.Res.Id
	//return GetAccountRecoverySettingCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsAccountRecoverySettingResourceCrud) Create() error {
	...
	if accountRecoverySettingId, ok := s.D.GetOkExists("account_recovery_setting_id"); ok {
		tmp := accountRecoverySettingId.(string)
		request.AccountRecoverySettingId = &tmp
	}
	...
	if factors, ok := s.D.GetOkExists("factors"); ok {
		interfaces := factors.([]interface{})
		tmp := make([]oci_identity_domains.AccountRecoverySettingFactorsEnum, len(interfaces))
		for i := range interfaces {
			if interfaces[i] != nil {
				tmp[i] = oci_identity_domains.AccountRecoverySettingFactorsEnum(interfaces[i].(string))
			}
		}
		if len(tmp) != 0 || s.D.HasChange("factors") {
			request.Factors = tmp
		}
	}
	...
	response, err := s.Client.PutAccountRecoverySetting(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.AccountRecoverySetting
	return nil
}
...
func (s *IdentityDomainsAccountRecoverySettingResourceCrud) Get() error {
	...
	accountRecoverySettingId, err := parseAccountRecoverySettingCompositeId(s.D.Id())
	if err == nil {
	...
	response, err := s.Client.GetAccountRecoverySetting(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsAccountRecoverySettingResourceCrud) SetData() error {
	...
	accountRecoverySettingId, err := parseAccountRecoverySettingCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(accountRecoverySettingId)
	} else {
...
//func GetAccountRecoverySettingCompositeId(accountRecoverySettingId string) string {
//	accountRecoverySettingId = url.PathEscape(accountRecoverySettingId)
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/accountRecoverySettings/" + accountRecoverySettingId
//	return compositeId
//}

func parseAccountRecoverySettingCompositeId(compositeId string) (accountRecoverySettingId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/accountRecoverySettings/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	//idcsEndpoint, _ = url.PathUnescape(parts[1])
	accountRecoverySettingId, _ = url.PathUnescape(parts[3])

	return
}

func AccountRecoverySettingToMap(obj oci_identity_domains.AccountRecoverySetting) map[string]interface{} {
