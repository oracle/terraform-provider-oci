...
func IdentityDomainsSettingsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsSettings,
		Schema: map[string]*schema.Schema{
			...
			"settings": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsSettingResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsSettingsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListSettingsResponse
}
...
func (s *IdentityDomainsSettingsDataSourceCrud) Get() error {
	...
	response, err := s.Client.ListSettings(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsSettingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsSettingsDataSource-", IdentityDomainsSettingsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, SettingToMap(item))
	}
	s.D.Set("settings", resources)
