...
func IdentityDomainsMyApiKeyDataSource() *schema.Resource {
	...
	fieldMap["my_api_key_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
	...
	return tfresource.GetSingularDataSourceItemSchema(IdentityDomainsMyApiKeyResource(), fieldMap, readSingularIdentityDomainsMyApiKey)
}
...
type IdentityDomainsMyApiKeyDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetMyApiKeyResponse
}
...
func (s *IdentityDomainsMyApiKeyDataSourceCrud) Get() error {
	...
	if myApiKeyId, ok := s.D.GetOkExists("my_api_key_id"); ok {
		tmp := myApiKeyId.(string)
	...
	response, err := s.Client.GetMyApiKey(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsMyApiKeyDataSourceCrud) SetData() error {
	...
	if s.Res.User != nil {
		s.D.Set("user", []interface{}{MyApiKeyUserToMap(s.Res.User)})
	} else {
		s.D.Set("user", nil)
	}
