...
func IdentityDomainsCloudGatesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsCloudGates,
		Schema: map[string]*schema.Schema{
			...
			"cloud_gates": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsCloudGateResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsCloudGatesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListCloudGatesResponse
}
...
func (s *IdentityDomainsCloudGatesDataSourceCrud) Get() error {
	...
	if cloudGateCount, ok := s.D.GetOkExists("cloud_gate_count"); ok {
		tmp := cloudGateCount.(int)
		request.Count = &tmp
	}

	if cloudGateFilter, ok := s.D.GetOkExists("cloud_gate_filter"); ok {
		tmp := cloudGateFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListCloudGatesSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListCloudGates(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListCloudGates(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsCloudGatesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsCloudGatesDataSource-", IdentityDomainsCloudGatesDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, CloudGateToMap(item))
	}
	s.D.Set("cloud_gates", resources)
