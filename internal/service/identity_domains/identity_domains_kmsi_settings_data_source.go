...
func IdentityDomainsKmsiSettingsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsKmsiSettings,
		Schema: map[string]*schema.Schema{
			...
			"kmsi_settings": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsKmsiSettingResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsKmsiSettingsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListKmsiSettingsResponse
}
...
func (s *IdentityDomainsKmsiSettingsDataSourceCrud) Get() error {
	...
	response, err := s.Client.ListKmsiSettings(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsKmsiSettingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsKmsiSettingsDataSource-", IdentityDomainsKmsiSettingsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, KmsiSettingToMap(item))
	}
	s.D.Set("kmsi_settings", resources)
