...
func IdentityDomainsCustomerSecretKeysDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsCustomerSecretKeys,
		Schema: map[string]*schema.Schema{
			...
			"customer_secret_keys": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsCustomerSecretKeyResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsCustomerSecretKeysDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListCustomerSecretKeysResponse
}
...
func (s *IdentityDomainsCustomerSecretKeysDataSourceCrud) Get() error {
	...
	if customerSecretKeyCount, ok := s.D.GetOkExists("customer_secret_key_count"); ok {
		tmp := customerSecretKeyCount.(int)
		request.Count = &tmp
	}

	if customerSecretKeyFilter, ok := s.D.GetOkExists("customer_secret_key_filter"); ok {
		tmp := customerSecretKeyFilter.(string)
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListCustomerSecretKeysSortOrderEnum(sortOrder.(string))
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListCustomerSecretKeys(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsCustomerSecretKeysDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsCustomerSecretKeysDataSource-", IdentityDomainsCustomerSecretKeysDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, CustomerSecretKeyToMap(item))
	}
	s.D.Set("customer_secret_keys", resources)
