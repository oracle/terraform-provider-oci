...
func IdentityDomainsMyCustomerSecretKeyResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsMyCustomerSecretKey,
		Read:     readIdentityDomainsMyCustomerSecretKey,
		Delete:   deleteIdentityDomainsMyCustomerSecretKey,
...
func readIdentityDomainsMyCustomerSecretKey(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsMyCustomerSecretKeyResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "myCustomerSecretKeys")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsMyCustomerSecretKeyResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.MyCustomerSecretKey
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsMyCustomerSecretKeyResourceCrud) ID() string {
	return *s.Res.Id
	//return GetMyCustomerSecretKeyCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsMyCustomerSecretKeyResourceCrud) Create() error {
	...
	if status, ok := s.D.GetOkExists("status"); ok {
		request.Status = oci_identity_domains.MyCustomerSecretKeyStatusEnum(status.(string))
	}
	...
	if user, ok := s.D.GetOkExists("user"); ok {
		if tmpList := user.([]interface{}); len(tmpList) > 0 {
			...
			tmp, err := s.mapToMyCustomerSecretKeyUser(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.User = &tmp
	...
	response, err := s.Client.CreateMyCustomerSecretKey(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.MyCustomerSecretKey
	return nil
}
...
func (s *IdentityDomainsMyCustomerSecretKeyResourceCrud) Get() error {
	...
	myCustomerSecretKeyId, err := parseMyCustomerSecretKeyCompositeId(s.D.Id())
	if err == nil {
	...
	response, err := s.Client.GetMyCustomerSecretKey(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsMyCustomerSecretKeyResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteMyCustomerSecretKey(context.Background(), request)
	return err
}
...
func (s *IdentityDomainsMyCustomerSecretKeyResourceCrud) SetData() error {
	...
	myCustomerSecretKeyId, err := parseMyCustomerSecretKeyCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(myCustomerSecretKeyId)
	} else {
	...
	if s.Res.User != nil {
		s.D.Set("user", []interface{}{MyCustomerSecretKeyUserToMap(s.Res.User)})
	} else {
		s.D.Set("user", nil)
	}
...
//func GetMyCustomerSecretKeyCompositeId(myCustomerSecretKeyId string) string {
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	myCustomerSecretKeyId = url.PathEscape(myCustomerSecretKeyId)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/myCustomerSecretKeys/" + myCustomerSecretKeyId
//	return compositeId
//}

func parseMyCustomerSecretKeyCompositeId(compositeId string) (myCustomerSecretKeyId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/myCustomerSecretKeys/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	//idcsEndpoint, _ = url.PathUnescape(parts[1])
	myCustomerSecretKeyId, _ = url.PathUnescape(parts[3])

	return
}

func MyCustomerSecretKeyToMap(obj oci_identity_domains.MyCustomerSecretKey) map[string]interface{} {
	...
	if obj.User != nil {
		result["user"] = []interface{}{MyCustomerSecretKeyUserToMap(obj.User)}
	}
...
func MyCustomerSecretKeyUserToMap(obj *oci_identity_domains.MyCustomerSecretKeyUser) map[string]interface{} {
	result := map[string]interface{}{}
