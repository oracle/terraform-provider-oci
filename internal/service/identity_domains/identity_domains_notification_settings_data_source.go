...
func IdentityDomainsNotificationSettingsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsNotificationSettings,
		Schema: map[string]*schema.Schema{
			...
			"notification_settings": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsNotificationSettingResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsNotificationSettingsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListNotificationSettingsResponse
}
...
func (s *IdentityDomainsNotificationSettingsDataSourceCrud) Get() error {
	...
	response, err := s.Client.ListNotificationSettings(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response
	// this endpoint doesn't support pagination. startIndex/count is not supported param in request.
	return nil
}
...
func (s *IdentityDomainsNotificationSettingsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsNotificationSettingsDataSource-", IdentityDomainsNotificationSettingsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, NotificationSettingToMap(item))
	}
	s.D.Set("notification_settings", resources)
