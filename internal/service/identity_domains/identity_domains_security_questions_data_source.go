...
func IdentityDomainsSecurityQuestionsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsSecurityQuestions,
		Schema: map[string]*schema.Schema{
			...
			"security_questions": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsSecurityQuestionResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsSecurityQuestionsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListSecurityQuestionsResponse
}
...
func (s *IdentityDomainsSecurityQuestionsDataSourceCrud) Get() error {
	...
	if securityQuestionCount, ok := s.D.GetOkExists("security_question_count"); ok {
		tmp := securityQuestionCount.(int)
		request.Count = &tmp
	}

	if securityQuestionFilter, ok := s.D.GetOkExists("security_question_filter"); ok {
		tmp := securityQuestionFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListSecurityQuestionsSortOrderEnum(sortOrder.(string))
	...
	response, err := s.Client.ListSecurityQuestions(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListSecurityQuestions(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsSecurityQuestionsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsSecurityQuestionsDataSource-", IdentityDomainsSecurityQuestionsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, SecurityQuestionToMap(item))
	}
	s.D.Set("security_questions", resources)
