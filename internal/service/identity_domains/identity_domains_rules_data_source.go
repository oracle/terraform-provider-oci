	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"rules": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsRuleResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsRulesDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListRulesResponse
}
...
func (s *IdentityDomainsRulesDataSourceCrud) Get() error {
	...
	if ruleCount, ok := s.D.GetOkExists("rule_count"); ok {
		tmp := ruleCount.(int)
		request.Count = &tmp
	}

	if ruleFilter, ok := s.D.GetOkExists("rule_filter"); ok {
		tmp := ruleFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListRulesSortOrderEnum(sortOrder.(string))
...
func (s *IdentityDomainsRulesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsRulesDataSource-", IdentityDomainsRulesDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, RuleToMap(item))
	}
	s.D.Set("rules", resources)
