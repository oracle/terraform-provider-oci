...
func IdentityDomainsAuthTokensDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsAuthTokens,
		Schema: map[string]*schema.Schema{
			...
			"auth_tokens": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsAuthTokenResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsAuthTokensDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListAuthTokensResponse
}
...
func (s *IdentityDomainsAuthTokensDataSourceCrud) Get() error {
	...
	if authTokenCount, ok := s.D.GetOkExists("auth_token_count"); ok {
		tmp := authTokenCount.(int)
		request.Count = &tmp
	}

	if authTokenFilter, ok := s.D.GetOkExists("auth_token_filter"); ok {
		tmp := authTokenFilter.(string)
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListAuthTokensSortOrderEnum(sortOrder.(string))
	...
	for startIndex+*response.ItemsPerPage <= *response.TotalResults {
		...
		listResponse, err := s.Client.ListAuthTokens(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *IdentityDomainsAuthTokensDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsAuthTokensDataSource-", IdentityDomainsAuthTokensDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, AuthTokenToMap(item))
	}
	s.D.Set("auth_tokens", resources)
