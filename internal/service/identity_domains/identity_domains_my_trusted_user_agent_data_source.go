...
func IdentityDomainsMyTrustedUserAgentDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readSingularIdentityDomainsMyTrustedUserAgent,
...
type IdentityDomainsMyTrustedUserAgentDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetMyTrustedUserAgentResponse
}
...
func (s *IdentityDomainsMyTrustedUserAgentDataSourceCrud) Get() error {
	...
	if myTrustedUserAgentId, ok := s.D.GetOkExists("my_trusted_user_agent_id"); ok {
		tmp := myTrustedUserAgentId.(string)
		request.MyTrustedUserAgentId = &tmp
	}
	...
	response, err := s.Client.GetMyTrustedUserAgent(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsMyTrustedUserAgentDataSourceCrud) SetData() error {
	...
	if s.Res.ExpiryTime != nil {
		s.D.Set("expiry_time", *s.Res.ExpiryTime)
	}
	...
	s.D.Set("token_type", s.Res.TokenType)

	if s.Res.TrustToken != nil {
		s.D.Set("trust_token", *s.Res.TrustToken)
	}

	trustedFactors := []interface{}{}
	for _, item := range s.Res.TrustedFactors {
		trustedFactors = append(trustedFactors, MyTrustedUserAgentTrustedFactorsToMap(item))
	}
	s.D.Set("trusted_factors", trustedFactors)

	if s.Res.User != nil {
		s.D.Set("user", []interface{}{MyTrustedUserAgentUserToMap(s.Res.User)})
	} else {
		s.D.Set("user", nil)
	}
