...
func IdentityDomainsCustomerSecretKeyResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsCustomerSecretKey,
		Read:     readIdentityDomainsCustomerSecretKey,
		Delete:   deleteIdentityDomainsCustomerSecretKey,
...
func readIdentityDomainsCustomerSecretKey(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsCustomerSecretKeyResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "customerSecretKeys")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsCustomerSecretKeyResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.CustomerSecretKey
...
func (s *IdentityDomainsCustomerSecretKeyResourceCrud) Create() error {
	...
	if status, ok := s.D.GetOkExists("status"); ok {
		request.Status = oci_identity_domains.CustomerSecretKeyStatusEnum(status.(string))
	}
	...
	if user, ok := s.D.GetOkExists("user"); ok {
		if tmpList := user.([]interface{}); len(tmpList) > 0 {
			...
			tmp, err := s.mapToCustomerSecretKeyUser(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.User = &tmp
...
func (s *IdentityDomainsCustomerSecretKeyResourceCrud) Get() error {
	...
	customerSecretKeyId, err := parseCustomerSecretKeyCompositeId(s.D.Id())
	if err == nil {
	...
	response, err := s.Client.GetCustomerSecretKey(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *IdentityDomainsCustomerSecretKeyResourceCrud) SetData() error {
	...
	if s.Res.AccessKey != nil {
		s.D.Set("access_key", *s.Res.AccessKey)
	}
	...
	if s.Res.User != nil {
		s.D.Set("user", []interface{}{CustomerSecretKeyUserToMap(s.Res.User)})
	} else {
		s.D.Set("user", nil)
	}
...
//func GetCustomerSecretKeyCompositeId(customerSecretKeyId string) string {
//	customerSecretKeyId = url.PathEscape(customerSecretKeyId)
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/customerSecretKeys/" + customerSecretKeyId
//	return compositeId
//}

func parseCustomerSecretKeyCompositeId(compositeId string) (customerSecretKeyId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/customerSecretKeys/.*", compositeId)
...
func CustomerSecretKeyToMap(obj oci_identity_domains.CustomerSecretKey) map[string]interface{} {
	result := map[string]interface{}{}

	if obj.AccessKey != nil {
		result["access_key"] = string(*obj.AccessKey)
	}
	...
	if obj.User != nil {
		result["user"] = []interface{}{CustomerSecretKeyUserToMap(obj.User)}
	}
...
func CustomerSecretKeyUserToMap(obj *oci_identity_domains.CustomerSecretKeyUser) map[string]interface{} {
	result := map[string]interface{}{}
