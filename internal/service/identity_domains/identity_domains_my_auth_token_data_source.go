...
func IdentityDomainsMyAuthTokenDataSource() *schema.Resource {
	...
	fieldMap["my_auth_token_id"] = &schema.Schema{
		Type:     schema.TypeString,
		Required: true,
	}
...
type IdentityDomainsMyAuthTokenDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.GetMyAuthTokenResponse
}
...
func (s *IdentityDomainsMyAuthTokenDataSourceCrud) Get() error {
	...
	if myAuthTokenId, ok := s.D.GetOkExists("my_auth_token_id"); ok {
		tmp := myAuthTokenId.(string)
