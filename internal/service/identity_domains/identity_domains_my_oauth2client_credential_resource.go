...
func IdentityDomainsMyOAuth2ClientCredentialResource() *schema.Resource {
	return &schema.Resource{
		...
		Create:   createIdentityDomainsMyOAuth2ClientCredential,
		Read:     readIdentityDomainsMyOAuth2ClientCredential,
		Delete:   deleteIdentityDomainsMyOAuth2ClientCredential,
...
func readIdentityDomainsMyOAuth2ClientCredential(d *schema.ResourceData, m interface{}) error {
	sync := &IdentityDomainsMyOAuth2ClientCredentialResourceCrud{}
	sync.D = d
	idcsEndpoint, err := getIdcsEndpointForRead(d, "myOAuth2ClientCredentials")
	if err != nil {
		return err
	}
	client, err := m.(*client.OracleClients).IdentityDomainsClientWithEndpoint(idcsEndpoint)
...
type IdentityDomainsMyOAuth2ClientCredentialResourceCrud struct {
	tfresource.BaseCrud
	Client                 *oci_identity_domains.IdentityDomainsClient
	Res                    *oci_identity_domains.MyOAuth2ClientCredential
	DisableNotFoundRetries bool
}

func (s *IdentityDomainsMyOAuth2ClientCredentialResourceCrud) ID() string {
	return *s.Res.Id
	//return GetMyOAuth2ClientCredentialCompositeId(s.D.Get("id").(string))
}
...
func (s *IdentityDomainsMyOAuth2ClientCredentialResourceCrud) Create() error {
	...
	if scopes, ok := s.D.GetOkExists("scopes"); ok {
		...
		tmp := make([]oci_identity_domains.MyOAuth2ClientCredentialScopes, len(interfaces))
		for i := range interfaces {
			stateDataIndex := i
			fieldKeyFormat := fmt.Sprintf("%s.%d.%%s", "scopes", stateDataIndex)
			converted, err := s.mapToMyOAuth2ClientCredentialScopes(fieldKeyFormat)
			if err != nil {
				return err
			}
			tmp[i] = converted
	...
	if status, ok := s.D.GetOkExists("status"); ok {
		request.Status = oci_identity_domains.MyOAuth2ClientCredentialStatusEnum(status.(string))
	}
	...
	if user, ok := s.D.GetOkExists("user"); ok {
		if tmpList := user.([]interface{}); len(tmpList) > 0 {
			...
			tmp, err := s.mapToMyOAuth2ClientCredentialUser(fieldKeyFormat)
			if err != nil {
				return err
			}
			request.User = &tmp
	...
	response, err := s.Client.CreateMyOAuth2ClientCredential(context.Background(), request)
	if err != nil {
		return err
	}

	s.Res = &response.MyOAuth2ClientCredential
	return nil
}
...
func (s *IdentityDomainsMyOAuth2ClientCredentialResourceCrud) Get() error {
	...
	myOAuth2ClientCredentialId, err := parseMyOAuth2ClientCredentialCompositeId(s.D.Id())
	if err == nil {
...
func (s *IdentityDomainsMyOAuth2ClientCredentialResourceCrud) Delete() error {
	...
	_, err := s.Client.DeleteMyOAuth2ClientCredential(context.Background(), request)
	return err
}
...
func (s *IdentityDomainsMyOAuth2ClientCredentialResourceCrud) SetData() error {
	...
	myOAuth2ClientCredentialId, err := parseMyOAuth2ClientCredentialCompositeId(s.D.Id())
	if err == nil {
		s.D.SetId(myOAuth2ClientCredentialId)
	} else {
...
//func GetMyOAuth2ClientCredentialCompositeId(myOAuth2ClientCredentialId string) string {
//	id = url.PathEscape(id)
//	idcsEndpoint = url.PathEscape(idcsEndpoint)
//	myOAuth2ClientCredentialId = url.PathEscape(myOAuth2ClientCredentialId)
//	compositeId := "idcsEndpoint/" + idcsEndpoint + "/myOAuth2ClientCredentials/" + myOAuth2ClientCredentialId
//	return compositeId
//}

func parseMyOAuth2ClientCredentialCompositeId(compositeId string) (myOAuth2ClientCredentialId string, err error) {
	parts := strings.Split(compositeId, "/")
	match, _ := regexp.MatchString("idcsEndpoint/.*/myOAuth2ClientCredentials/.*", compositeId)
	if !match || len(parts) != 4 {
		err = fmt.Errorf("illegal compositeId %s encountered", compositeId)
		return
	}
	//idcsEndpoint, _ = url.PathUnescape(parts[1])
	myOAuth2ClientCredentialId, _ = url.PathUnescape(parts[3])

	return
}

func MyOAuth2ClientCredentialToMap(obj oci_identity_domains.MyOAuth2ClientCredential) map[string]interface{} {
	...
	if obj.User != nil {
		result["user"] = []interface{}{MyOAuth2ClientCredentialUserToMap(obj.User)}
	}
...
func MyOAuth2ClientCredentialScopesToMap(obj oci_identity_domains.MyOAuth2ClientCredentialScopes) map[string]interface{} {
	result := map[string]interface{}{}
...
func MyOAuth2ClientCredentialUserToMap(obj *oci_identity_domains.MyOAuth2ClientCredentialUser) map[string]interface{} {
	result := map[string]interface{}{}
