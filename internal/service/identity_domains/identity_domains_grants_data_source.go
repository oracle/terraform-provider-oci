...
func IdentityDomainsGrantsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readIdentityDomainsGrants,
		Schema: map[string]*schema.Schema{
			...
			"grants": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(IdentityDomainsGrantResource()),
			},
			"items_per_page": {
				Type:     schema.TypeInt,
				Computed: true,
...
type IdentityDomainsGrantsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_identity_domains.IdentityDomainsClient
	Res    *oci_identity_domains.ListGrantsResponse
}
...
func (s *IdentityDomainsGrantsDataSourceCrud) Get() error {
	...
	if grantCount, ok := s.D.GetOkExists("grant_count"); ok {
		tmp := grantCount.(int)
		request.Count = &tmp
	}

	if grantFilter, ok := s.D.GetOkExists("grant_filter"); ok {
		tmp := grantFilter.(string)
		request.Filter = &tmp
	}
	...
	if sortOrder, ok := s.D.GetOkExists("sort_order"); ok {
		tmp := oci_identity_domains.ListGrantsSortOrderEnum(sortOrder.(string))
...
func (s *IdentityDomainsGrantsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("IdentityDomainsGrantsDataSource-", IdentityDomainsGrantsDataSource(), s.D))

	resources := []interface{}{}
	for _, item := range s.Res.Resources {
		resources = append(resources, GrantToMap(item))
	}
	s.D.Set("grants", resources)
