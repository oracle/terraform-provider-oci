...
func CapacityManagementInternalOccHandoverResourceBlockDetailsDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readCapacityManagementInternalOccHandoverResourceBlockDetails,
...
type CapacityManagementInternalOccHandoverResourceBlockDetailsDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_capacity_management.CapacityManagementClient
	Res    *oci_capacity_management.ListInternalOccHandoverResourceBlockDetailsResponse
}
...
func (s *CapacityManagementInternalOccHandoverResourceBlockDetailsDataSourceCrud) Get() error {
	...
	response, err := s.Client.ListInternalOccHandoverResourceBlockDetails(context.Background(), request)
	if err != nil {
		return err
	}
	...
	for request.Page != nil {
		listResponse, err := s.Client.ListInternalOccHandoverResourceBlockDetails(context.Background(), request)
		if err != nil {
			return err
		}
...
func (s *CapacityManagementInternalOccHandoverResourceBlockDetailsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("CapacityManagementInternalOccHandoverResourceBlockDetailsDataSource-", CapacityManagementInternalOccHandoverResourceBlockDetailsDataSource(), s.D))
	resources := []map[string]interface{}{}
	internalOccHandoverResourceBlockDetail := map[string]interface{}{}

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, OccHandoverResourceBlockDetailSummaryToMap(item))
	}
	internalOccHandoverResourceBlockDetail["items"] = items

	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, CapacityManagementInternalOccHandoverResourceBlockDetailsDataSource().Schema["occ_handover_resource_block_detail_collection"].Elem.(*schema.Resource).Schema)
		internalOccHandoverResourceBlockDetail["items"] = items
	}

	resources = append(resources, internalOccHandoverResourceBlockDetail)
