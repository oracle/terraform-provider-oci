...
func LicenseManagerProductLicenseConsumersDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readLicenseManagerProductLicenseConsumers,
...
type LicenseManagerProductLicenseConsumersDataSourceCrud struct {
...
func (s *LicenseManagerProductLicenseConsumersDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("LicenseManagerProductLicenseConsumersDataSource-", LicenseManagerProductLicenseConsumersDataSource(), s.D))

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, ProductLicenseConsumersSummaryToMap(item))
	}
	s.D.Set("items", items)
...
func ProductLicenseConsumersSummaryToMap(obj oci_license_manager.ProductLicenseConsumerSummary) map[string]interface{} {
	result := map[string]interface{}{}
