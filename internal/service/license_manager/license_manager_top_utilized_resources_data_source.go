...
func LicenseManagerTopUtilizedResourcesDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readLicenseManagerTopUtilizedResources,
...
type LicenseManagerTopUtilizedResourcesDataSourceCrud struct {
...
func (s *LicenseManagerTopUtilizedResourcesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("LicenseManagerTopUtilizedResourcesDataSource-", LicenseManagerTopUtilizedResourcesDataSource(), s.D))

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, TopUtilizedResourcesSummaryToMap(item))
	}
	s.D.Set("items", items)
