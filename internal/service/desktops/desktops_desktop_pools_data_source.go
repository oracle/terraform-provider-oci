				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(DesktopsDesktopPoolResource()),
						},
					},
				},
			},
...
type DesktopsDesktopPoolsDataSourceCrud struct {
	...
	Res    *oci_desktops.ListDesktopPoolsResponse
}
...
func (s *DesktopsDesktopPoolsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("DesktopsDesktopPoolsDataSource-", DesktopsDesktopPoolsDataSource(), s.D))
	resources := []map[string]interface{}{}
	desktopPool := map[string]interface{}{}

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, DesktopPoolSummaryToMap(item))
	}
	desktopPool["items"] = items

	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, DesktopsDesktopPoolsDataSource().Schema["desktop_pool_collection"].Elem.(*schema.Resource).Schema)
		desktopPool["items"] = items
	}

	resources = append(resources, desktopPool)
	if err := s.D.Set("desktop_pool_collection", resources); err != nil {
		return err
	}
