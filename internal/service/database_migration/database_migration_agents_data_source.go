				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(DatabaseMigrationAgentResource()),
						},
					},
				},
			},
...
func (s *DatabaseMigrationAgentsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("DatabaseMigrationAgentsDataSource-", DatabaseMigrationAgentsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, DatabaseMigrationAgentsDataSource().Schema["agent_collection"].Elem.(*schema.Resource).Schema)
		agent["items"] = items
	}
