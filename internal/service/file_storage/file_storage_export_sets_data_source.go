	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"export_sets": {
				...
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(FileStorageExportSetResource()),
			},
		},
	}
}
...
func (s *FileStorageExportSetsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("FileStorageExportSetsDataSource-", FileStorageExportSetsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = tfresource.ApplyFilters(f.(*schema.Set), resources, FileStorageExportSetsDataSource().Schema["export_sets"].Elem.(*schema.Resource).Schema)
	}
