				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(ArtifactsRepositoryResource()),
						},
					},
				},
			},
...
func (s *ArtifactsRepositoriesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("ArtifactsRepositoriesDataSource-", ArtifactsRepositoriesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, ArtifactsRepositoriesDataSource().Schema["repository_collection"].Elem.(*schema.Resource).Schema)
		repository["items"] = items
	}
