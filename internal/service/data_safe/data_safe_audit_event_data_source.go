...
func DataSafeAuditEventDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readSingularDataSafeAuditEvent,
...
type DataSafeAuditEventDataSourceCrud struct {
...
func (s *DataSafeAuditEventDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("DataSafeAuditEventDataSource-", DataSafeAuditEventDataSource(), s.D))

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, DataSafeAuditEventSummaryToMap(item))
	}
	s.D.Set("items", items)
...
func DataSafeAuditEventSummaryToMap(obj oci_data_safe.AuditEventSummary) map[string]interface{} {
