				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(LogAnalyticsLogAnalyticsLogGroupResource()),
						},
					},
				},
			},
...
func (s *LogAnalyticsLogAnalyticsLogGroupsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("LogAnalyticsLogAnalyticsLogGroupsDataSource-", LogAnalyticsLogAnalyticsLogGroupsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, LogAnalyticsLogAnalyticsLogGroupsDataSource().Schema["log_analytics_log_group_summary_collection"].Elem.(*schema.Resource).Schema)
		logAnalyticsLogGroup["items"] = items
	}
