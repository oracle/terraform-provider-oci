...
func WaaWebAppAccelerationPoliciesDataSource() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"web_app_acceleration_policy_collection": {
				Type:     schema.TypeList,
				Computed: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(WaaWebAppAccelerationPolicyResource()),
						},
					},
				},
			},
...
type WaaWebAppAccelerationPoliciesDataSourceCrud struct {
	...
	Res    *oci_waa.ListWebAppAccelerationPoliciesResponse
}
...
func (s *WaaWebAppAccelerationPoliciesDataSourceCrud) Get() error {
	...
	if state, ok := s.D.GetOkExists("state"); ok {
		...
		tmp := make([]oci_waa.WebAppAccelerationPolicyLifecycleStateEnum, len(interfaces))

		for i := range interfaces {
			if interfaces[i] != nil {
				tmp[i] = oci_waa.WebAppAccelerationPolicyLifecycleStateEnum(interfaces[i].(string))
			}
		}
...
func (s *WaaWebAppAccelerationPoliciesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("WaaWebAppAccelerationPoliciesDataSource-", WaaWebAppAccelerationPoliciesDataSource(), s.D))
	resources := []map[string]interface{}{}
	webAppAccelerationPolicy := map[string]interface{}{}

	items := []interface{}{}
	for _, item := range s.Res.Items {
		items = append(items, WebAppAccelerationPolicySummaryToMap(item))
	}
	webAppAccelerationPolicy["items"] = items

	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, WaaWebAppAccelerationPoliciesDataSource().Schema["web_app_acceleration_policy_collection"].Elem.(*schema.Resource).Schema)
		webAppAccelerationPolicy["items"] = items
	}

	resources = append(resources, webAppAccelerationPolicy)
	if err := s.D.Set("web_app_acceleration_policy_collection", resources); err != nil {
		return err
	}
