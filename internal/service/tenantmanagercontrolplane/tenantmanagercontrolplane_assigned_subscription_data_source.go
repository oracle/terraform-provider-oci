...
func TenantmanagercontrolplaneAssignedSubscriptionDataSource() *schema.Resource {
	return &schema.Resource{
		Read: readSingularTenantmanagercontrolplaneAssignedSubscription,
...
func readSingularTenantmanagercontrolplaneAssignedSubscription(d *schema.ResourceData, m interface{}) error {
	sync := &TenantmanagercontrolplaneAssignedSubscriptionDataSourceCrud{}
	sync.D = d
...
type TenantmanagercontrolplaneAssignedSubscriptionDataSourceCrud struct {
	D      *schema.ResourceData
	Client *oci_tenantmanagercontrolplane.SubscriptionClient
	Res    *oci_tenantmanagercontrolplane.GetAssignedSubscriptionResponse
}

func (s *TenantmanagercontrolplaneAssignedSubscriptionDataSourceCrud) VoidState() {
	s.D.SetId("")
}

func (s *TenantmanagercontrolplaneAssignedSubscriptionDataSourceCrud) Get() error {
	request := oci_tenantmanagercontrolplane.GetAssignedSubscriptionRequest{}
	...
	response, err := s.Client.GetAssignedSubscription(context.Background(), request)
	if err != nil {
		return err
	}
...
func (s *TenantmanagercontrolplaneAssignedSubscriptionDataSourceCrud) SetData() error {
	if s.Res == nil {
		return nil
	}

	s.D.SetId(*s.Res.GetId())
	switch v := (s.Res.AssignedSubscription).(type) {
	case oci_tenantmanagercontrolplane.ClassicAssignedSubscription:
		...
		s.D.Set("managed_by", v.ManagedBy)

		s.D.Set("order_ids", v.OrderIds)
		...
		for _, item := range v.Promotion {
			promotion = append(promotion, AssignedSubscriptionPromotionToMap(item))
		}
		...
		for _, item := range v.Skus {
			skus = append(skus, AssignedSubscriptionSkuToMap(item))
		}
		s.D.Set("skus", skus)
	...
	case oci_tenantmanagercontrolplane.CloudAssignedSubscription:
	...
	default:
		log.Printf("[WARN] Received 'entity_version' of unknown type %v", s.Res.AssignedSubscription)
		return nil
	}
