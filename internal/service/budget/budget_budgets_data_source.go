...
func (s *BudgetBudgetsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.EndDate != nil {
			budget["end_date"] = r.EndDate.Format(time.RFC3339Nano)
		}
		...
		if r.StartDate != nil {
			budget["start_date"] = r.StartDate.Format(time.RFC3339Nano)
		}

		budget["state"] = r.LifecycleState

		if r.SystemTags != nil {
			budget["system_tags"] = tfresource.SystemTagsToMap(r.SystemTags)
		}
