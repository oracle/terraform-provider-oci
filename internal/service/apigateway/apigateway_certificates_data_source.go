				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(ApigatewayCertificateResource()),
						},
					},
				},
			},
...
func (s *ApigatewayCertificatesDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("ApigatewayCertificatesDataSource-", ApigatewayCertificatesDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, ApigatewayCertificatesDataSource().Schema["certificate_collection"].Elem.(*schema.Resource).Schema)
		certificate["items"] = items
	}
