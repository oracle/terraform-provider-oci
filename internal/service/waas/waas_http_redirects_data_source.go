	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"http_redirects": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     tfresource.GetDataSourceItemSchema(WaasHttpRedirectResource()),
			},
		},
	}
}
...
func (s *WaasHttpRedirectsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("WaasHttpRedirectsDataSource-", WaasHttpRedirectsDataSource(), s.D))
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			httpRedirect["defined_tags"] = tfresource.DefinedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = tfresource.ApplyFilters(f.(*schema.Set), resources, WaasHttpRedirectsDataSource().Schema["http_redirects"].Elem.(*schema.Resource).Schema)
	}
