				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"items": {
							Type:     schema.TypeList,
							Computed: true,
							Elem:     tfresource.GetDataSourceItemSchema(AiDocumentModelResource()),
						},
					},
				},
			},
...
type AiDocumentModelsDataSourceCrud struct {
	...
	Res    *oci_ai_document.ListModelsResponse
}
...
func (s *AiDocumentModelsDataSourceCrud) Get() error {
	...
	if state, ok := s.D.GetOkExists("state"); ok {
		request.LifecycleState = oci_ai_document.ModelLifecycleStateEnum(state.(string))
	}
...
func (s *AiDocumentModelsDataSourceCrud) SetData() error {
	...
	s.D.SetId(tfresource.GenerateDataSourceHashID("AiDocumentModelsDataSource-", AiDocumentModelsDataSource(), s.D))
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		items = tfresource.ApplyFiltersInCollection(f.(*schema.Set), items, AiDocumentModelsDataSource().Schema["model_collection"].Elem.(*schema.Resource).Schema)
		model["items"] = items
	}
