...
func DatabaseAutonomousDatabasesDataSource() *schema.Resource {
	return &schema.Resource{
		ReadContext: readDatabaseAutonomousDatabasesWithContext,
...
func readDatabaseAutonomousDatabasesWithContext(ctx context.Context, d *schema.ResourceData, m interface{}) diag.Diagnostics {
...
func (s *DatabaseAutonomousDatabasesDataSourceCrud) GetWithContext(ctx context.Context) error {
	...
	response, err := s.Client.ListAutonomousDatabases(ctx, request)
	if err != nil {
		return err
	}
	...
	for request.Page != nil {
		listResponse, err := s.Client.ListAutonomousDatabases(ctx, request)
		if err != nil {
			return err
		}
...
func (s *DatabaseAutonomousDatabasesDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.AdScheduledForUpdate != nil {
			autonomousDatabase["ad_scheduled_for_update"] = *r.AdScheduledForUpdate
		}
		...
		if r.AutoRefreshFrequencyInSeconds != nil {
			autonomousDatabase["auto_refresh_frequency_in_seconds"] = *r.AutoRefreshFrequencyInSeconds
		}

		if r.AutoRefreshPointLagInSeconds != nil {
			autonomousDatabase["auto_refresh_point_lag_in_seconds"] = *r.AutoRefreshPointLagInSeconds
		}
		...
		if r.AutonomousDatabaseMaintenanceWindow != nil {
			autonomousDatabase["autonomous_database_maintenance_window"] = []interface{}{AutonomousDatabaseMaintenanceWindowSummaryToMap(r.AutonomousDatabaseMaintenanceWindow)}
		} else {
			autonomousDatabase["autonomous_database_maintenance_window"] = nil
		}
		...
		if r.LocalAdgResourcePoolLeaderId != nil {
			autonomousDatabase["local_adg_resource_pool_leader_id"] = *r.LocalAdgResourcePoolLeaderId
		}
		...
		if r.TimeEarliestAvailableAdUpdate != nil {
			autonomousDatabase["time_earliest_available_ad_update"] = r.TimeEarliestAvailableAdUpdate.String()
		}
		...
		if r.TimeLatestAvailableAdUpdate != nil {
			autonomousDatabase["time_latest_available_ad_update"] = r.TimeLatestAvailableAdUpdate.String()
		}
		...
		if r.TimeMaintenancePauseUntil != nil {
			autonomousDatabase["time_maintenance_pause_until"] = r.TimeMaintenancePauseUntil.String()
		}

		if r.TimeOfAutoRefreshStart != nil {
			autonomousDatabase["time_of_auto_refresh_start"] = r.TimeOfAutoRefreshStart.Format(time.RFC3339Nano)
		}
		...
		if r.TimeScheduledAdUpdate != nil {
			autonomousDatabase["time_scheduled_ad_update"] = r.TimeScheduledAdUpdate.String()
		}
