...
// Create an Autonomous JSON Database insight managed by Operations Insights
resource "oci_opsi_database_insight" "test_database_insight" {
  ...
  credential_details {
    ...
    # When using CREDENTIALS_BY_VAULT, configure the user credentials stored in OCI Vault.
    # For autonomous JSON databases this typically references the ADB wallet secret.
  }
  connection_details {
  ...
  defined_tags = {
    "${oci_identity_tag_namespace.tag-namespace1.name}.${oci_identity_tag.tag1.name}" = var.database_insight_defined_tags_value
  }
  # Optionally provide freeform tags similar to the test coverage.
...
data "oci_opsi_database_insights" "test_database_insights" {
  ...
  id                           = oci_opsi_database_insight.test_database_insight.id

  filter {
    name   = "id"
    values = [oci_opsi_database_insight.test_database_insight.id]
  }
}
