...
resource "oci_ocvp_sddc" "test_sddc" {
  ...
  // Optional
  #defined_tags  = "${map("${oci_identity_tag_namespace.tag-namespace1.name}.${oci_identity_tag.tag1.name}", "${var.sddc_defined_tags_value}")}"
...
resource "oci_ocvp_esxi_host" "test_esxi_host" {
  ...
  #Optional
  #defined_tags  = "${map("${oci_identity_tag_namespace.tag-namespace1.name}.${oci_identity_tag.tag1.name}", "${var.esxihost_defined_tags_value}")}"
...
data "oci_ocvp_sddc" "test_esxi_host" {
