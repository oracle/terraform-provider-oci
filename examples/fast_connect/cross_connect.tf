...
resource "oci_core_cross_connect" "cross_connect_on_device" {
  ...
  location_name         = data.oci_core_cross_connect_locations.cross_connect_locations.cross_connect_locations[1].name
...
resource "oci_core_cross_connect" "cross_connect" {
  ...
  # Optional LOA updates. Uncomment only when you intend to update the Letter of Authority.
  # loa_properties {
  #   authorized_agent = "authorizedAgent"
  #
  #   # Increase by 1 from the current value to request one LOA expiry extension.
  #   # expiry_extension_count = 1
  # }
}
...
output "cross_connects" {
  value = [
    for cross_connect in data.oci_core_cross_connects.cross_connects.cross_connects : {
      id           = cross_connect.id
      display_name = cross_connect.display_name
      state        = cross_connect.state
    }
  ]
}
