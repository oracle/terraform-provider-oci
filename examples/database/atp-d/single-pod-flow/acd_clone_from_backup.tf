...
# backup OCID in public cloud. It reuses the source Cloud Exadata Infrastructure,
# subnet, source ACD, source ADB, and OBJECT_STORE backup config from this
# example directory.
#
# Execution order:
...
# 2. This file creates a peer Cloud AVM cluster on the same Cloud Exadata
#    Infrastructure. This peer AVM is the full clone target.
# 3. This data source lists active local public-cloud ACD backups for the source
#    ACD after the source ADB exists.
# 4. The clone resource uses the first returned backup OCID to create a full ACD
#    clone on the peer Cloud AVM cluster.
# 5. The final data source lists ADBs inside the cloned ACD for validation.

resource "oci_database_cloud_autonomous_vm_cluster" "peer_cloud_autonomous_vm_cluster" {
  cloud_exadata_infrastructure_id = oci_database_cloud_exadata_infrastructure.test_cloud_exadata_infrastructure.id
  compartment_id                  = var.compartment_ocid
  display_name                    = "PeerCloudAutonomousVmCluster"
...
resource "oci_database_autonomous_container_database" "acd_clone_from_backup" {
  ...
  cloud_autonomous_vm_cluster_id          = oci_database_cloud_autonomous_vm_cluster.peer_cloud_autonomous_vm_cluster.id
  clone_type                              = "FULL"
  compartment_id                          = var.compartment_ocid
