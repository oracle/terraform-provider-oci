resource "oci_database_cloud_vm_cluster" "gcp_cluster" {
...
resource "oci_database_db_home" "test_db_home_vm_cluster_no_db" {
  vm_cluster_id = oci_database_cloud_vm_cluster.gcp_cluster.id
...
resource "oci_database_database" "test_database_create" {
  ...
  database {
    ...
    encryption_key_location_details {
        #Required
        azure_encryption_key_id  =  var.gcp_encryption_key
        provider_type = "GCP"
    }
  }
...
/*
    encryption_key_location_details {
        #Required
        gcp_encryption_key_id  = var.gcp_encryption_key
        provider_type = "GCP"
    }
*/
  }
