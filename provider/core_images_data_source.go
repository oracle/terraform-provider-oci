...
func (s *ImagesDataSourceCrud) Get() error {
	...
	if operatingSystem, ok := s.D.GetOk("operating_system"); ok {
		tmp := operatingSystem.(string)
		request.OperatingSystem = &tmp
	}

	if operatingSystemVersion, ok := s.D.GetOk("operating_system_version"); ok {
...
func (s *ImagesDataSourceCrud) SetData() {
	...
	for _, r := range s.Res.Items {
		...
		// The spec marks compartmentId as a required field, but the service doesn't always return it.
