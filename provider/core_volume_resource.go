...
func (s *VolumeResourceCrud) Create() error {
	...
	if sizeInGBs, ok := s.D.GetOk("size_in_gbs"); ok {
		tmp := sizeInGBs.(int)
		request.SizeInGBs = &tmp
	}

	if sizeInMBs, ok := s.D.GetOk("size_in_mbs"); ok {
		tmp := sizeInMBs.(int)
	...
	if sourceDetails, ok := s.D.GetOk("source_details"); ok {
		tmp := mapToVolumeSourceDetails(sourceDetails.([]interface{}))
		request.SourceDetails = &tmp
	}

	if volumeBackupId, ok := s.D.GetOk("volume_backup_id"); ok {
