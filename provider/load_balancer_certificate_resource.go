...
func (s *CertificateResourceCrud) Create() error {
	...
	if caCertificate, ok := s.D.GetOk("ca_certificate"); ok {
		tmp := caCertificate.(string)
		request.CaCertificate = &tmp
	}

	if certificateName, ok := s.D.GetOk("certificate_name"); ok {
	...
	if passphrase, ok := s.D.GetOk("passphrase"); ok {
		tmp := passphrase.(string)
		request.Passphrase = &tmp
	}

	if privateKey, ok := s.D.GetOk("private_key"); ok {
		tmp := privateKey.(string)
		request.PrivateKey = &tmp
	}

	if publicCertificate, ok := s.D.GetOk("public_certificate"); ok {
