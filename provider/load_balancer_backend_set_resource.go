...
func (s *BackendSetResourceCrud) Create() error {
	...
	if backend, ok := s.D.GetOk("backend"); ok {
	...
	*/
	...
	if healthChecker, ok := s.D.GetOk("health_checker"); ok {
	...
	if policy, ok := s.D.GetOk("policy"); ok {
		tmp := policy.(string)
		request.Policy = &tmp
	}

	if sessionPersistenceConfiguration, ok := s.D.GetOk("session_persistence_configuration"); ok {
	...
	if sslConfiguration, ok := s.D.GetOk("ssl_configuration"); ok {
...
func (s *BackendSetResourceCrud) Get() error {
	...
	if backendSetName, ok := s.D.GetOk("name"); ok {
