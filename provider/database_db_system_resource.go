...
func (s *DbSystemResourceCrud) Create() error {
	...
	if cpuCoreCount, ok := s.D.GetOk("cpu_core_count"); ok {
	...
	if dataStorageSizeInGB, ok := s.D.GetOk("data_storage_size_in_gb"); ok {
	...
	if sshPublicKeys, ok := s.D.GetOk("ssh_public_keys"); ok {
