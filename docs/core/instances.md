...
* `private_ip` - The private IP address of instance VNIC. To set the private IP address, use the `private_ip` argument in create_vnic_details. This field is NOT available in the oci_core_instances data source.
* `public_ip` - The public IP address of instance VNIC (if enabled). This field is NOT available in the oci_core_instances data source.
...
* `extended_metadata` - (Optional) Additional metadata key/value pairs that you provide.  They serve a similar purpose and functionality from fields in the 'metadata' object.
