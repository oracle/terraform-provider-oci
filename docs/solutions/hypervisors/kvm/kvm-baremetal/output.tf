...
output "SSH_TUNNEL_TO_GUEST" {
  value = "ssh -i ${var.ssh_user_private_key_path} -L ${var.kvm_guest_vnc_port}:localhost:${var.kvm_guest_vnc_port} opc@${data.oci_core_vnic.kvm-host-vnic.public_ip_address}"
}
