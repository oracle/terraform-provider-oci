...
resource "oci_core_instance" "dcos_gpu_agent" {
    count      = "${var.count}"
    availability_domain = "${var.availability_domain}"
    compartment_id = "${var.compartment_ocid}"
    display_name   = "${var.dcos_cluster_name}-gpu-agent-${var.display_name_prefix}-${count.index}"
    image = "${var.image}"
    shape = "${var.shape}"
    create_vnic_details {
  ...
  connection {
      host = "${oci_core_instance.dcos_gpu_agent.public_ip}"
      type = "ssh"
      user = "opc"
