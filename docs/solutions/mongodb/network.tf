resource "oci_core_virtual_network" "MongoDB" {
...
resource "oci_core_route_table" "MongoDB" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id         = "${oci_core_virtual_network.MongoDB.id}"
  display_name   = "MongoDB"
  ...
  route_rules {
    ...
    network_entity_id = "${oci_core_internet_gateway.MongoDB.id}"
  }
}
...
resource "oci_core_security_list" "PrivateSubnets" {
  compartment_id = "${var.compartment_ocid}"
